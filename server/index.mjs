...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/areas/bulk-zone-group') {const payload=(await readJson(request)).payload;return sendJson(response,200,{items:moveAreasToZone(payload.areaIds,Number(payload.zoneGroupId),payload)})}
