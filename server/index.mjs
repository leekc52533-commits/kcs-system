...
async function reverseGeocode(latitude,longitude){const lat=Number(latitude),lon=Number(longitude);if(!Number.isFinite(lat)||lat < -90||lat>90||!Number.isFinite(lon)||lon < -180||lon>180)throw Object.assign(new Error('Invalid GPS latitude or longitude'),{statusCode:400});const endpoint=new URL('https://nominatim.openstreetmap.org/reverse');endpoint.search=new URLSearchParams({format:'jsonv2',lat:String(lat),lon:String(lon),addressdetails:'1'});const result=await fetch(endpoint,{headers:{'User-Agent':'KCS-Dispatch-System/1.0 (reverse geocoding)','Accept-Language':'en'}});if(!result.ok)throw Object.assign(new Error('Address lookup is temporarily unavailable. GPS coordinates are still available.'),{statusCode:502});const data=await result.json(),address=data.address||{};return{address:data.display_name||'',state:address.state||'',street:address.road||address.pedestrian||address.footway||'',city:address.city||address.town||address.village||address.municipality||'',streetNumber:address.house_number||'',postalCode:address.postcode||'',provider:'OpenStreetMap Nominatim'}}
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/gps-collection/reverse-geocode') return sendJson(response,200,await reverseGeocode(url.searchParams.get('latitude'),url.searchParams.get('longitude')))
