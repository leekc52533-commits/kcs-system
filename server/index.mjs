...
import {geocodeGoogleAddress,reverseGeocodeGoogle} from './googleGeocodingService.mjs'
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/api/gps-collection/geocode') return sendJson(response,200,await geocodeGoogleAddress(url.searchParams.get('address')))
    ...
    if (request.method === 'PATCH' && /^\/api\/default-vehicle\/(zone|area)\/\d+$/.test(url.pathname)) {if(!canManageSchedules(session))return sendJson(response,403,{error:'Schedule management permission is required.'});const parts=url.pathname.split('/'),payload=(await readJson(request)).payload;return sendJson(response,200,setDefaultVehicle(parts[3],Number(parts[4]),{...payload,changedBy:session.employeeName}))}
    if (request.method === 'PUT' && /^\/api\/zone-groups\/\d+\/default-vehicles$/.test(url.pathname)) {if(!canManageSchedules(session))return sendJson(response,403,{error:'Schedule management permission is required.'});const zoneId=Number(url.pathname.split('/')[3]),payload=(await readJson(request)).payload;return sendJson(response,200,setZoneDefaultVehicles(zoneId,{...payload,changedBy:session.employeeName}))}
    ...
    if (request.method === 'POST' && /^\/api\/areas\/\d+\/refinement\/analyze$/.test(url.pathname)) {if(!canManageSchedules(session))return sendJson(response,403,{error:'Area management permission is required'});const payload=(await readJson(request)).payload;return sendJson(response,201,await analyzeArea(Number(url.pathname.split('/')[3]),{...payload,createdBy:session.employeeName}))}
    if (request.method === 'POST' && /^\/api\/zone-groups\/\d+\/area-refinement\/analyze$/.test(url.pathname)) {if(!canManageSchedules(session))return sendJson(response,403,{error:'Area management permission is required'});const payload=(await readJson(request)).payload;return sendJson(response,201,await analyzeZoneAreas(Number(url.pathname.split('/')[3]),{...payload,createdBy:session.employeeName}))}
