...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'PATCH' && /^\/api\/vehicles\/\d+$/.test(url.pathname)) {if(!accountCan(session,'vehicle_manage'))return sendJson(response,403,{error:'Vehicle management permission is required'});return sendJson(response,200,updateVehicle(Number(url.pathname.split('/').at(-1)),{...((await readJson(request)).payload),changedBy:session.employeeName}))}
