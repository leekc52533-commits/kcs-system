...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/customers') {const payload=(await readJson(request)).payload;if(Array.isArray(payload.materialPricing)&&payload.materialPricing.length&&!accountCan(session,'price_manage'))return sendJson(response,403,{error:'没有 Customer Pricing 管理权限'});return sendJson(response,201,createCustomer(payload))}
    ...
    if (request.method === 'PATCH' && /^\/api\/customers\/[^/]+$/.test(url.pathname)) {const payload=(await readJson(request)).payload;if(Array.isArray(payload.materialPricing)&&!accountCan(session,'price_manage'))return sendJson(response,403,{error:'没有 Customer Pricing 管理权限'});return sendJson(response,200,updateCustomer(decodeURIComponent(url.pathname.split('/').at(-1)),payload))}
