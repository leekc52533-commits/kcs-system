...
import {assignProductBranches,changeProductGroupPrice,createCategory,deleteEntity,entityPreview,getCategory,getPriceGroup,getProduct,listCategories,mergeEntities,moveProductBranches,moveProductCategory,setEntityVisibility} from './materialCatalogService.mjs'
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'POST' && url.pathname === '/api/master-transfer/preview') {const payload=(await readJson(request)).payload;if(payload.module==='employee'&&!canManageEmployees(session)&&!accountCan(session,'employee_sensitive_import'))return sendJson(response,403,{error:'没有 Employee 导入权限'});return sendJson(response,200,previewMasterImport(payload))}
    if (request.method === 'POST' && url.pathname === '/api/master-transfer/commit') {const payload=(await readJson(request)).payload;const batch=db.prepare('SELECT source FROM import_batches WHERE id=?').get(payload.batchId);if(batch?.source==='kcs_master_employee'&&!canManageEmployees(session)&&!accountCan(session,'employee_sensitive_import'))return sendJson(response,403,{error:'没有 Employee 导入权限'});return sendJson(response,200,commitMasterImport(payload.batchId,{...payload,changedBy:session.employeeName}))}
