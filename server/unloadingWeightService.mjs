...
export function mobileWeightContext(context={},database=defaultDb){
  ...
  return{available:Boolean(trips.length),trip:trips[0]||null,recent:recent.map(row=>({...row,code:unloadingCode({...row,serviceDate})}))}
}
...
export async function recognizeUnloadingWeight(payload={},context={},database=defaultDb,{uploadsRoot}={}){
  ...
  try{const result=await ocr(absolute),weighedAt=nowKuching();database.exec('SAVEPOINT unloading_capture');try{const insert=database.prepare(`INSERT INTO unloading_weight_records(dispatch_trip_id,dispatch_day_id,vehicle_id,driver_employee_id,service_date,trip_number,vehicle_code_snapshot,registration_number_snapshot,driver_name_snapshot,crew_names_snapshot,unloading_location_name_snapshot,unloading_address_snapshot,estimated_weight_kg,gross_weight_kg,tare_weight_kg,recognized_weight_kg,ocr_text,ocr_candidates_json,ocr_status,photo_storage_key,photo_original_name,photo_content_type,photo_size_bytes,latitude,longitude,accuracy_m,device_captured_at,weighed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(trip.tripId,trip.dayId,trip.vehicleId,context.employeeId,trip.serviceDate,trip.tripNumber,trip.vehicleCode,trip.registrationNumber,trip.driverName,trip.crewNames,trip.locationName,trip.locationAddress,trip.estimatedWeightKg,result.grossWeightKg,result.tareWeightKg,result.recognizedWeightKg,result.text,JSON.stringify(result.candidates),result.status,`unloading-weights/${name}`,String(payload.photo?.name||`weight-ticket.${photo.extension}`),photo.type,photo.bytes.length,payload.latitude||null,payload.longitude||null,payload.accuracyM||null,payload.deviceCapturedAt||null,weighedAt);captureUnloadingRoute(database,Number(insert.lastInsertRowid),trip);database.exec('RELEASE unloading_capture');return{id:Number(insert.lastInsertRowid),code:unloadingCode({id:Number(insert.lastInsertRowid),serviceDate:trip.serviceDate}),recognizedWeightKg:result.recognizedWeightKg,grossWeightKg:result.grossWeightKg,tareWeightKg:result.tareWeightKg,estimatedWeightKg:trip.estimatedWeightKg,ocrStatus:result.status,candidates:result.candidates,vehicleCode:trip.vehicleCode,registrationNumber:trip.registrationNumber,locationName:trip.locationName}}catch(error){database.exec('ROLLBACK TO unloading_capture; RELEASE unloading_capture');throw error}}
