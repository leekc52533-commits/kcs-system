...
function buildReport(db,access,date){
 ...
 const sections={}
 sections.vehicles=[...new Set([...byDay('dispatches','dispatch_date').map(r=>r.vehicle_id),...stops.map(r=>r.vehicle_id),...trips.map(r=>r.vehicle_id),...issued.map(r=>r.vehicle_id)].filter(v=>v!=null))].map(id=>row(id,vehicles.get(id),'vehicle',runVehicleIds.has(id)?'running':'not_started',sum(kg.filter(r=>r.vehicle_id===id),'quantity'),null,'','',{trips:activeTrips.filter(t=>t.vehicle_id===id).length,collectedBranches:unique(issued.filter(b=>b.vehicle_id===id),'branch_id'),plannedBranches:unique(currentStops.filter(s=>s.vehicle_id===id),'branch_id'),pendingBranches:unique(pendingStops.filter(s=>s.vehicle_id===id),'branch_id'),noGoodsBranches:unique(noGoods.filter(s=>s.vehicle_id===id),'branch_id'),cancelledBranches:unique(stops.filter(s=>s.vehicle_id===id&&s.status==='cancelled'),'branch_id'),targetKg:null,achievement:null}))
 const stopDateRequest=tables.has('driver_date_requests')?db.prepare("SELECT * FROM driver_date_requests WHERE dispatch_stop_id=? AND status='approved' ORDER BY reviewed_at DESC,id DESC LIMIT 1"):null
 const stopDateReview=tables.has('driver_date_reviews')?db.prepare('SELECT approved_date FROM driver_date_reviews WHERE request_id=?'):null
 sections.stops=stops.map(s=>{const request=s.status==='cancelled'&&s.override_note==='driver_date_approved'?stopDateRequest?.get(s.id):null;const review=request?stopDateReview?.get(request.id):null;return row(s.id,s.branch_name,'branch',s.superseded_by_stop_id?'superseded':s.status==='completed'&&['no_goods','no_goods_notice'].includes(s.completion_outcome)?'no_goods':s.status,null,null,employees.get(s.driver_id)||'',s.completed_at||s.arrived_at||'',{branchCode:s.jodoo_branch_id,vehicleId:s.vehicle_id,vehicle:vehicles.get(s.vehicle_id),tripId:s.dispatch_trip_id,serviceDate:date,reason:s.override_reason||s.superseded_reason||(tables.has('driver_no_goods_proofs')?db.prepare('SELECT reason FROM driver_no_goods_proofs WHERE dispatch_stop_id=?').get(s.id)?.reason:null)||(tables.has('no_goods_notices')?db.prepare('SELECT reason FROM no_goods_notices WHERE dispatch_stop_id=? AND restored_at IS NULL').get(s.id)?.reason:null)||'',hasIssuedBill:issued.some(b=>b.dispatch_stop_id===s.id),...(request?{reason:request.reason,reviewReason:request.review_reason,requestedAt:request.requested_at,reviewedAt:request.reviewed_at,sourceDate:request.source_date,targetDate:request.target_date,approvedDate:review?.approved_date}:{} )})})
