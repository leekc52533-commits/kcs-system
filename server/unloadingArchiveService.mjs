...
import {filterUnloading,unloadingColumns,unloadingLabels,unloadingStatus,unloadingOrder} from '../shared/unloadingArchive.js'
...
export function listUnloadingArchive(query={},db=defaultDb){
 ...
 const corrections=db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='unloading_corrections'").get()?new Map(db.prepare('SELECT record_id,COUNT(*) n FROM unloading_corrections GROUP BY record_id').all().map(r=>[r.record_id,r.n])):new Map()
...
export async function unloadingArchiveWorkbook(query={},db=defaultDb){
 ...
 const order=unloadingOrder(query.order)
 sheet.columns=order.map(key=>({key,header:unloadingLabels[lang][unloadingColumns.indexOf(key)],width:key==='code'||key==='driverName'||key==='locationName'?30:20}))
