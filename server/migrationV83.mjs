...
export function applyV83Migration(db){
 const version=Number(db.prepare('SELECT COALESCE(MAX(version),0) v FROM schema_meta').get().v)
 if(version>=83)return{noOp:true}
 if(version!==82)throw Error('Schema 82 required')
 ...
 try{
  ...
  for(const row of catalog){
   ...
   if(name!==row.description){
    ...
    if(winner){
     db.prepare('INSERT INTO sale_price_audit(catalog_id,actor,before_json,after_json) VALUES(?,?,?,NULL)').run(row.id,'Material name normalization',JSON.stringify(row))
    ...
    db.prepare('INSERT INTO sale_price_audit(catalog_id,actor,before_json,after_json) VALUES(?,?,?,?)').run(row.id,'Material name normalization',JSON.stringify(row),JSON.stringify({...row,description:name,description_key:name.toLowerCase()}))
   }
   kept.set(key,row.id)
  }
  ...
  db.prepare('INSERT INTO schema_meta(version) VALUES(83)').run()
  db.exec('COMMIT')
  return{schemaVersion:83}
