...
function employeeRows(database) {
  return database.prepare(`SELECT e.id,e.employee_code employeeCode,e.name,e.phone,e.job_role jobRole,e.is_active isActive,
    ...
    e.employment_type employmentType,
    (SELECT h.start_date FROM employee_employment_history h WHERE h.employee_id=e.id AND h.end_date IS NULL AND h.employment_status='active' ORDER BY h.id DESC LIMIT 1) employmentStartDate,
    e.employment_start_date legacyEmploymentStartDate,e.employment_end_date employmentEndDate,e.last_working_day lastWorkingDay,e.resignation_termination_reason resignationTerminationReason,
    e.national_id_number nationalIdNumber,e.bank_name bankName,e.bank_account_number bankAccountNumber,e.bank_account_holder_name bankAccountHolderName,e.epf_number epfNumber,e.socso_number socsoNumber,e.driving_licence_expiry_date drivingLicenceExpiryDate,e.gdl_expiry_date gdlExpiryDate,
    CASE WHEN e.home_latitude IS NOT NULL AND e.home_longitude IS NOT NULL THEN 'Set' ELSE 'Not Set' END homeGpsStatus,
    e.default_area_id defaultAreaId,a.name defaultArea,aa.id accountId,aa.username,CASE WHEN COALESCE(aa.system_role,aa.role)='admin' THEN 'owner_admin' ELSE COALESCE(aa.system_role,aa.role) END systemRole,aa.is_active accountActive,aa.must_change_password mustChangePassword,
    ...
    GROUP BY e.id ORDER BY e.name`).all().map(item=>({...item,
      additionalRoles:item.additionalRoles?item.additionalRoles.split('|').filter(role=>role!==item.jobRole):[],
      ...
      hasNationalId:Boolean(item.nationalIdNumber),nationalIdSuffix:item.nationalIdNumber?String(item.nationalIdNumber).slice(-4):null,hasBankAccount:Boolean(item.bankAccountNumber),hasEpf:Boolean(item.epfNumber),hasSocso:Boolean(item.socsoNumber),
...
const EMPLOYMENT_STATUSES=new Set(['active','on_leave','inactive','resigned','terminated','contract_end','suspended'])
...
const normalizeEmploymentStatus=value=>{const result=text(value).toLowerCase()||'active',mapped={active:'active','on leave':'on_leave',on_leave:'on_leave',inactive:'inactive',resigned:'resigned',terminated:'terminated','contract end':'contract_end',contract_end:'contract_end',suspended:'suspended'}[result];if(!mapped||!EMPLOYMENT_STATUSES.has(mapped))throw new Error('Invalid Employment Status');return mapped}
...
function employeeHomeCoordinates(payload,before={}){const latitude=payload.homeLatitude===undefined?(before.home_latitude??null):(payload.homeLatitude===''||payload.homeLatitude==null?null:Number(payload.homeLatitude)),longitude=payload.homeLongitude===undefined?(before.home_longitude??null):(payload.homeLongitude===''||payload.homeLongitude==null?null:Number(payload.homeLongitude));if((latitude==null)!==(longitude==null))throw new Error('Home Latitude and Home Longitude must be set or cleared together');if(latitude!=null&&(!Number.isFinite(latitude)||latitude < -90||latitude > 90))throw new Error('Invalid Home Latitude');if(longitude!=null&&(!Number.isFinite(longitude)||longitude < -180||longitude > 180))throw new Error('Invalid Home Longitude');return{latitude,longitude}}
const auditValue=value=>value==null?'':String(value)
function currentEmploymentPeriod(database,employeeId){
  const periods=database.prepare("SELECT * FROM employee_employment_history WHERE employee_id=? AND end_date IS NULL AND employment_status='active' ORDER BY id").all(employeeId)
  if(periods.length>1)throw new Error('Employee has multiple active Employment Periods')
  return periods[0]||null
}
function auditEmploymentPeriodStart(database,employeeId,periodId,before,after,reason,actor){
  if(auditValue(before)===auditValue(after))return
  database.prepare('INSERT INTO employee_change_history(employee_id,field_name,old_value,new_value,reason,changed_by) VALUES(?,?,?,?,?,?)').run(employeeId,`employment_period_start_date:${periodId}`,auditValue(before)||null,auditValue(after)||null,text(reason),actor)
}
function auditEmployeeChanges(database,id,before,next,payload,actor){const tracked=[['employee_code','employeeCode'],['name','name'],['phone','phone'],['job_role','jobRole'],['employment_type','employmentType'],['employment_detail_status','employmentStatus'],['employment_start_date','employmentStartDate'],['employment_end_date','employmentEndDate'],['last_working_day','lastWorkingDay'],['resignation_termination_reason','resignationTerminationReason'],['driving_licence_expiry_date','drivingLicenceExpiryDate'],['gdl_expiry_date','gdlExpiryDate'],['national_id_number','nationalIdNumber'],['bank_name','bankName'],['bank_account_number','bankAccountNumber'],['bank_account_holder_name','bankAccountHolderName'],['epf_number','epfNumber'],['socso_number','socsoNumber'],['default_base_location_id','defaultBaseLocationId'],['home_address','homeAddress'],['home_latitude','homeLatitude'],['home_longitude','homeLongitude'],['home_gps_remark','homeGpsRemark']];const changes=tracked.filter(([column,key])=>Object.hasOwn(payload,key)&&auditValue(before?.[column])!==auditValue(next[column]));if(changes.length&&!text(payload.reason))throw new Error('修改员工关键资料必须填写原因');const insert=database.prepare('INSERT INTO employee_change_history(employee_id,field_name,old_value,new_value,reason,changed_by) VALUES(?,?,?,?,?,?)'),sensitive=new Set(['national_id_number','bank_name','bank_account_number','bank_account_holder_name','epf_number','socso_number']);for(const[column]of changes){insert.run(id,column,auditValue(before?.[column])||null,auditValue(next[column])||null,text(payload.reason),actor);if(sensitive.has(column))database.prepare(`INSERT INTO employee_sensitive_access_logs(employee_id,action,field_name,reason,actor) VALUES(?,'modify',?,?,?)`).run(id,column,text(payload.reason),actor)}}
...
export function getEmployeeMaster(id,database=defaultDb){const item=employeeRows(database).find(item=>item.id===Number(id))||null;if(!item)return null;const home=database.prepare('SELECT home_address homeAddress,home_latitude homeLatitude,home_longitude homeLongitude,home_gps_remark homeGpsRemark FROM employees WHERE id=?').get(id);return{...item,...home}}
...
const zoneRow=(database,id)=>withZoneDefaults(database,database.prepare(`${zoneStatsSql} WHERE z.id=?`).get(id))
...
export function updateVehicle(id, payload, database = defaultDb) {
  ...
  database.exec('BEGIN IMMEDIATE')
  try {
    const protectedFields=[['registrationNumber','registration_number','Registration plate'],['registeredOwner','registered_owner','Registered owner'],['chassisNumber','chassis_number','Chassis number'],['engineNumber','engine_number','Engine number']]
    for(const [key,column,label] of protectedFields)if(payload[key]!==undefined&&text(before[column])&&text(payload[key])!==text(before[column]))throw new Error(`${label} conflicts with the existing verified value; review is required`)
    ...
    database.prepare(`UPDATE vehicles SET vehicle_code=?,vehicle_name=?,registration_number=?,capacity_kg=?,default_base_location_id=?,status=?,operational_status=?,brand=?,model=?,manufacture_year=?,registration_date=?,vehicle_type=?,chassis_number=?,engine_number=?,gross_vehicle_weight_kg=?,unladen_weight_kg=?,registered_owner=?,fuel_type=?,engine_capacity_cc=?,vehicle_origin=?,vehicle_class=?,remark=?,is_common=?,sold_at=CASE WHEN ?='sold' THEN COALESCE(sold_at,CURRENT_TIMESTAMP) ELSE sold_at END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(
    ...
    const after=database.prepare('SELECT * FROM vehicles WHERE id=?').get(id),changedFields=Object.keys(after).filter(key=>!['updated_at'].includes(key)&&before[key]!==after[key])
    if(changedFields.length)database.prepare(`INSERT INTO master_change_history(entity_type,entity_id,change_type,field_name,before_json,after_json,reason,changed_by) VALUES('vehicle',?,'update',?,?,?,?,?)`).run(String(id),changedFields.join(','),JSON.stringify(before),JSON.stringify(after),text(payload.reason||payload.statusReason)||'Vehicle master update',text(payload.changedBy)||'Supervisor')
    database.exec('COMMIT')
...
export function createEmployee(payload, database = defaultDb) {
  ...
  database.exec('SAVEPOINT employee_create');try{
    const home=employeeHomeCoordinates(payload)
    const result = database.prepare(`INSERT INTO employees(employee_code,name,phone,job_role,employment_status,employment_detail_status,employment_type,employment_start_date,employment_end_date,last_working_day,resignation_termination_reason,national_id_number,bank_name,bank_account_number,bank_account_holder_name,epf_number,socso_number,driving_licence_expiry_date,gdl_expiry_date,default_base_location_id,default_area_id,home_address,home_latitude,home_longitude,home_gps_remark,is_active) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      employeeCode,text(payload.name),text(payload.phone)||null,primaryRole,operationalStatus(detailStatus),detailStatus,type,payload.employmentStartDate||null,payload.employmentEndDate||null,payload.lastWorkingDay||null,text(payload.resignationTerminationReason)||null,text(payload.nationalIdNumber)||null,text(payload.bankName)||null,text(payload.bankAccountNumber)||null,text(payload.bankAccountHolderName)||null,text(payload.epfNumber)||null,text(payload.socsoNumber)||null,payload.drivingLicenceExpiryDate||null,payload.gdlExpiryDate||null,validateEmployeeBase(database,payload.defaultBaseLocationId),idOrNull(payload.defaultAreaId),text(payload.homeAddress)||null,home.latitude,home.longitude,text(payload.homeGpsRemark)||null,detailStatus==='active'&&payload.isActive!==false?1:0)
...
export function updateEmployee(id, payload, database = defaultDb) {
  ...
  if (payload.employeeCode !== undefined && text(payload.employeeCode) !== before.employee_code) throw new Error('Employee Code cannot be changed')
  const primaryRole=normalizeJobRole(payload.jobRole??before.job_role),oldRoles=database.prepare('SELECT role FROM employee_job_roles WHERE employee_id=? AND is_active=1').all(id).map(item=>item.role),actor=text(payload.changedBy)||'Supervisor',detailStatus=normalizeEmploymentStatus(payload.employmentStatus??before.employment_detail_status??before.employment_status),type=normalizeEmploymentType(payload.employmentType??before.employment_type),terminal=['inactive','resigned','terminated','contract_end','suspended'].includes(detailStatus),effectiveEndDate=payload.employmentEndDate===undefined?(before.employment_end_date||(terminal?kuchingDate():null)):payload.employmentEndDate||null,currentPeriodBefore=currentEmploymentPeriod(database,id),startDate=payload.employmentStartDate===undefined?(currentPeriodBefore?.start_date??before.employment_start_date):payload.employmentStartDate||null,compatibilityStartDate=payload.employmentStartDate===undefined?before.employment_start_date:startDate
  ...
  if(terminal&&database.prepare("SELECT 1 FROM auth_accounts WHERE employee_id=? AND is_active=1 AND COALESCE(system_role,role) IN ('owner_admin','admin')").get(id)&&database.prepare("SELECT COUNT(*) count FROM auth_accounts WHERE is_active=1 AND COALESCE(system_role,role) IN ('owner_admin','admin')").get().count<=1)throw new Error('不可停用最后一个 Owner Admin')
  const home=employeeHomeCoordinates(payload,before)
  database.exec('SAVEPOINT employee_update');try{
  database.prepare(`UPDATE employees SET employee_code=?,name=?,phone=?,job_role=?,employment_status=?,employment_detail_status=?,employment_type=?,employment_start_date=?,employment_end_date=?,last_working_day=?,resignation_termination_reason=?,national_id_number=?,bank_name=?,bank_account_number=?,bank_account_holder_name=?,epf_number=?,socso_number=?,driving_licence_expiry_date=?,gdl_expiry_date=?,default_base_location_id=?,default_area_id=?,home_address=?,home_latitude=?,home_longitude=?,home_gps_remark=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(
    values.employeeCode,text(payload.name??before.name),payload.phone===undefined?before.phone:text(payload.phone)||null,primaryRole,operationalStatus(detailStatus),detailStatus,type,compatibilityStartDate,effectiveEndDate,payload.lastWorkingDay===undefined?before.last_working_day:payload.lastWorkingDay||null,payload.resignationTerminationReason===undefined?before.resignation_termination_reason:text(payload.resignationTerminationReason)||null,values.nationalIdNumber||null,payload.bankName===undefined?before.bank_name:text(payload.bankName)||null,payload.bankAccountNumber===undefined?before.bank_account_number:text(payload.bankAccountNumber)||null,payload.bankAccountHolderName===undefined?before.bank_account_holder_name:text(payload.bankAccountHolderName)||null,values.epfNumber||null,values.socsoNumber||null,payload.drivingLicenceExpiryDate===undefined?before.driving_licence_expiry_date:payload.drivingLicenceExpiryDate||null,payload.gdlExpiryDate===undefined?before.gdl_expiry_date:payload.gdlExpiryDate||null,payload.defaultBaseLocationId===undefined?before.default_base_location_id:validateEmployeeBase(database,payload.defaultBaseLocationId),payload.defaultAreaId===undefined?before.default_area_id:idOrNull(payload.defaultAreaId),payload.homeAddress===undefined?before.home_address:text(payload.homeAddress)||null,home.latitude,home.longitude,payload.homeGpsRemark===undefined?before.home_gps_remark:text(payload.homeGpsRemark)||null,detailStatus==='active'&&(payload.isActive===undefined?before.is_active:Number(Boolean(payload.isActive)))?1:0,id)
  ...
  let period=currentPeriodBefore
  if(!period&&detailStatus==='active'&&Object.hasOwn(payload,'employmentStartDate')){
    const created=database.prepare(`INSERT INTO employee_employment_history(employee_id,start_date,employment_status,employment_type,primary_job_role,secondary_job_roles,rehire_flag,created_by) VALUES(?,?,'active',?,?,?,?,?)`).run(id,startDate,type,primaryRole,(payload.additionalRoles??oldRoles.filter(role=>role!==primaryRole)).join('|')||null,0,actor)
    period=database.prepare('SELECT * FROM employee_employment_history WHERE id=?').get(created.lastInsertRowid)
  }
  if(period){
    const periodStart=Object.hasOwn(payload,'employmentStartDate')?startDate:period.start_date
    database.prepare(`UPDATE employee_employment_history SET start_date=?,end_date=?,last_working_day=?,employment_status=?,employment_type=?,primary_job_role=?,secondary_job_roles=?,resignation_or_termination_reason=?,updated_by=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(periodStart,after.employment_end_date,after.last_working_day,detailStatus,type,primaryRole,(payload.additionalRoles??oldRoles.filter(role=>role!==primaryRole)).join('|')||null,after.resignation_termination_reason,actor,period.id)
    if(Object.hasOwn(payload,'employmentStartDate'))auditEmploymentPeriodStart(database,id,period.id,period.start_date,periodStart,payload.reason,actor)
  }
  if(['inactive','resigned','terminated','contract_end','suspended'].includes(detailStatus)){database.prepare('UPDATE auth_accounts SET is_active=0,disabled_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE employee_id=?').run(id);database.prepare('UPDATE auth_sessions SET revoked_at=CURRENT_TIMESTAMP WHERE account_id IN(SELECT id FROM auth_accounts WHERE employee_id=?) AND revoked_at IS NULL').run(id)}
  if (roleChanged||detailStatus!=='active'||payload.isActive===false) {
    const dates = database.prepare(`SELECT DISTINCT dd.dispatch_date FROM dispatch_days dd
      ...
      WHERE (d.driver_id=? OR d.assistant_id=? OR dva.employee_id=?) AND dd.dispatch_date>=date('now','+8 hours')`).all(id, id, id)
...
export function endEmployeeEmployment(id,payload,database=defaultDb){const status=normalizeEmploymentStatus(payload.employmentStatus);if(!['resigned','terminated','contract_end'].includes(status))throw new Error('Leaving status must be Resigned, Terminated or Contract End');if(!payload.lastWorkingDay)throw new Error('Last Working Day is required');if(!payload.employmentEndDate)throw new Error('Employment End Date is required');if(!text(payload.resignationTerminationReason))throw new Error('Leaving Reason is required');return updateEmployee(id,{employeeId:Number(id),employmentStatus:status,lastWorkingDay:payload.lastWorkingDay,employmentEndDate:payload.employmentEndDate,resignationTerminationReason:payload.resignationTerminationReason,reason:payload.reason||payload.resignationTerminationReason,changedBy:payload.changedBy},database)}
