...
export function customerBranches(params, database = db) {
  ...
  const items = database.prepare(`SELECT b.jodoo_branch_id branchId,c.name customerName,b.branch_name branchName,z.name zoneGroup,a.name area,a.jodoo_area_id areaId,COALESCE(b.payment_type,c.payment_type) paymentType,b.gps_status gpsStatus,b.latitude,b.longitude,COUNT(DISTINCT s.id) scheduleCount,(SELECT COUNT(*) FROM customer_material_pricing cmp WHERE cmp.customer_id=b.customer_id AND cmp.status='active' AND cmp.resolution_state='ready') materialCount,GROUP_CONCAT(DISTINCT s.days_of_week) daysOfWeek,GROUP_CONCAT(DISTINCT s.frequency) frequency ${base} LEFT JOIN branch_schedules s ON s.branch_id=b.id WHERE ${where.join(' AND ')} GROUP BY b.id ORDER BY c.name,b.branch_name LIMIT ? OFFSET ?`).all(...args,pageSize,(page-1)*pageSize)
...
export function schedules(params, database = db) {
  ...
  return database.prepare(`SELECT s.id internalScheduleId,s.jodoo_schedule_id scheduleId,s.source_branch_id branchId,COALESCE(b.branch_name,'') branch,s.frequency,s.days_of_week dayOfWeek,s.take_date takeDate,s.next_take_date nextTakeDate,s.recurrence_type recurrenceType,s.interval_weeks intervalWeeks,s.anchor_date anchorDate,s.effective_date effectiveDate,s.monthly_occurrence monthlyOccurrence,s.fixed_weekday fixedWeekday,s.next_collection_date nextCollectionDate,s.branch_id IS NULL unmatched,(SELECT COUNT(*) FROM branch_schedules sx WHERE sx.source_branch_id=s.source_branch_id)>1 multipleSchedules,z.name zoneGroup,a.name area FROM branch_schedules s LEFT JOIN branches b ON b.id=s.branch_id LEFT JOIN areas a ON a.id=b.area_id LEFT JOIN zone_groups z ON z.id=a.zone_group_id WHERE ${where.join(' AND ')} ORDER BY s.source_branch_id,s.days_of_week`).all(...args)
}

export function dataQualitySummary(database = db) {
  const groups = database.prepare(`SELECT b.jodoo_branch_id branchId,c.name customerName,b.branch_name branchName,z.name zoneGroup,a.name area,b.is_active isActive,b.status,
