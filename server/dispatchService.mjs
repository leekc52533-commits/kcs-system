...
function assignExistingUnassignedStops(database,day){
  ...
  let assigned=0
  for(const row of rows){const snapshot=branchZoneSnapshot(database,row.branchId),vehicleId=availableDefaultVehicleId(database,snapshot,day);if(!vehicleId)continue;const target=ensureVehicleTrip(database,day,vehicleId,1),sequence=database.prepare("SELECT COALESCE(MAX(stop_sequence),0)+1 value FROM dispatch_stops WHERE dispatch_id=? AND status<>'cancelled'").get(target.dispatch_id).value;database.prepare('UPDATE dispatch_stops SET dispatch_id=?,dispatch_trip_id=?,stop_sequence=? WHERE id=?').run(target.dispatch_id,target.id,sequence,row.id);assigned+=1}
  return assigned
}
