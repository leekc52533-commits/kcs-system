...
export function updateTrip(id,payload,database=defaultDb){return withImmediateTransaction(database,()=>{const before=database.prepare(`SELECT dt.*,dd.dispatch_date,d.* FROM dispatch_trips dt JOIN dispatch_days dd ON dd.id=dt.dispatch_day_id JOIN dispatches d ON d.id=dt.dispatch_id WHERE dt.id=?`).get(id);if(!before)throw new Error('Trip not found');database.prepare(`UPDATE dispatches SET vehicle_id=?,driver_id=?,assistant_id=?,end_location_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(payload.vehicleId??before.vehicle_id,payload.driverId??before.driver_id,payload.assistantId??before.assistant_id,payload.endLocationId??before.end_location_id,before.dispatch_id);if(payload.startLocation){const resolved=resolveStartLocation(payload.startLocation,{driverId:payload.driverId??before.driver_id,canViewEmployeeHome:Boolean(payload.canViewEmployeeHome)},database);writeStartLocationSnapshot(database,before.dispatch_id,resolved)}database.prepare('UPDATE dispatch_trips SET trip_number=COALESCE(?,trip_number),estimated_weight_kg=COALESCE(?,estimated_weight_kg),updated_at=CURRENT_TIMESTAMP WHERE id=?').run(payload.tripNumber??null,payload.estimatedWeightKg??null,id);const after=database.prepare('SELECT * FROM dispatches WHERE id=?').get(before.dispatch_id),changeType=payload.startLocation?'trip_start_location_updated':'trip_updated';invalidateDispatchDay(database,before.dispatch_date,changeType,'dispatch_trip',id,before,{...after,reason:payload.reason||null},payload.changedBy);return database.prepare('SELECT * FROM dispatch_trips WHERE id=?').get(id)})}
...
export function transferVehicleDay(date,sourceVehicleId,payload,database=defaultDb){
  ...
  const sourceTrips=database.prepare(`SELECT dt.*,d.driver_id,d.assistant_id,d.start_location_id,d.start_location_type,d.start_location_reference_type,d.start_location_reference_id,d.start_location_name,d.start_address,d.start_latitude,d.start_longitude,d.end_location_id FROM dispatch_trips dt JOIN dispatches d ON d.id=dt.dispatch_id WHERE dt.dispatch_day_id=? AND d.vehicle_id=? ORDER BY dt.trip_number,dt.id`).all(day.id,sourceVehicleId)
  ...
  try{
    for(const sourceTrip of sourceTrips){
      ...
      if(payload.transferDriver!==false){database.prepare(`UPDATE dispatches SET driver_id=COALESCE(?,driver_id),driver_employment_period_id=COALESCE(?,driver_employment_period_id),assistant_id=COALESCE(?,assistant_id),assistant_employment_period_id=COALESCE(?,assistant_employment_period_id),end_location_id=COALESCE(?,end_location_id),updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(sourceTrip.driver_id,currentEmploymentPeriod(database,sourceTrip.driver_id),sourceTrip.assistant_id,currentEmploymentPeriod(database,sourceTrip.assistant_id),sourceTrip.end_location_id,targetTrip.dispatch_id);if(sourceTrip.start_location_type)writeStartLocationSnapshot(database,targetTrip.dispatch_id,{startLocationId:sourceTrip.start_location_id,startLocationType:sourceTrip.start_location_type,startLocationReferenceType:sourceTrip.start_location_reference_type,startLocationReferenceId:sourceTrip.start_location_reference_id,startLocationName:sourceTrip.start_location_name,startAddress:sourceTrip.start_address,startLatitude:sourceTrip.start_latitude,startLongitude:sourceTrip.start_longitude})}
...
export function driverToday({employeeId,role,today=kuchingDate()}={},database=defaultDb){
  ...
  const trips=database.prepare(`SELECT dt.id,dt.trip_number tripNumber,dt.execution_status executionStatus,dt.started_at startedAt${tripCompletion},d.vehicle_id vehicleId,v.vehicle_code vehicleCode,v.vehicle_name vehicleName,v.registration_number registrationNumber
    ...
    ORDER BY v.vehicle_code,dt.trip_number,dt.id`).all(...params).map(trip=>({...trip,stops:database.prepare(`SELECT ds.id,ds.stop_sequence stopSequence,ds.status${stopCompletion},b.jodoo_branch_id branchId,b.branch_name branchName,c.name customerName,b.address,
      ...
      ORDER BY ds.stop_sequence,ds.id`).all(trip.id).map(stop=>({...stop,gpsAvailable:Boolean(stop.gpsAvailable),invoiceConfirmed:Boolean(stop.invoiceConfirmed)}))})).map((trip,index,trips)=>{const current=trip.executionStatus==='in_progress'?trip.stops.find(stop=>!['completed','cancelled'].includes(stop.status)):null,earlierOpen=trips.some(other=>other.vehicleId===trip.vehicleId&&other.tripNumber<trip.tripNumber&&other.stops.length&&other.executionStatus!=='completed'),finished=trip.stops.filter(stop=>stop.status==='completed').length;return{...trip,completedCount:finished,totalCount:trip.stops.length,canComplete:trip.executionStatus==='in_progress'&&finished===trip.stops.length,canStart:['approved','in_progress'].includes(day.status)&&trip.executionStatus==='not_started'&&!earlierOpen,currentStopId:current?.id||null,stops:trip.stops.map(stop=>({...stop,canArrive:Boolean(current&&current.id===stop.id&&!stop.arrivedAt),canFinish:Boolean(current&&current.id===stop.id&&stop.arrivedAt&&stop.status==='active')}))}})
