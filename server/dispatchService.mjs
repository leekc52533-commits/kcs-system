...
export function handoverRoute(date,routeNumber,payload={},context={},database=defaultDb){
  ...
  return withImmediateTransaction(database,()=>{
    ...
    const currentAssistants=database.prepare('SELECT employee_id id FROM dispatch_vehicle_assistants WHERE dispatch_day_id=? AND vehicle_id=? ORDER BY employee_id').all(day.id,assignment.vehicleId).map(row=>row.id)
    if(payload.assistantIds!==undefined&&!Array.isArray(payload.assistantIds))throw fail('跟车员选择无效')
    const assistantIds=payload.assistantIds===undefined?currentAssistants:payload.assistantIds.map(Number)
    if(assistantIds.length>MAX_ASSIGNED_CREW||new Set(assistantIds).size!==assistantIds.length||assistantIds.some(id=>!Number.isInteger(id)||id<=0||id===driverId))throw fail('跟车员选择无效')
    for(const employeeId of assistantIds){
      const attendant=database.prepare(`SELECT 1 FROM employees e WHERE e.id=? AND e.is_active=1 AND e.employment_status='active' AND (lower(e.job_role) IN ('assistant','crew','attendant / crew') OR EXISTS(SELECT 1 FROM employee_job_roles r WHERE r.employee_id=e.id AND r.role='Attendant / Crew' AND r.is_active=1)) AND NOT EXISTS(SELECT 1 FROM route_employee_availability a WHERE a.employee_id=e.id AND a.availability_date=? AND a.status<>'available')`).get(employeeId,date)
      if(!attendant||database.prepare('SELECT 1 FROM dispatch_vehicle_assistants WHERE dispatch_day_id=? AND employee_id=? AND vehicle_id<>?').get(day.id,employeeId,assignment.vehicleId)||database.prepare('SELECT 1 FROM dispatch_trips dt JOIN dispatches d ON d.id=dt.dispatch_id WHERE dt.dispatch_day_id=? AND d.driver_id=? AND d.vehicle_id<>?').get(day.id,employeeId,assignment.vehicleId))throw fail('跟车员当天不可用或已有其他任务')
    }
    ...
    if(trips.every(t=>t.vehicle_id===vehicleId&&t.driver_id===driverId)&&assistantIds.length===currentAssistants.length&&assistantIds.every(id=>currentAssistants.includes(id)))throw fail('当天车辆、司机和跟车员没有改变')
    const before={vehicleId:assignment.vehicleId,trips,targetTrips,assistantIds:currentAssistants}
    ...
    if(payload.assistantIds!==undefined){
      database.prepare('DELETE FROM dispatch_vehicle_assistants WHERE dispatch_day_id=? AND vehicle_id=?').run(day.id,vehicleId)
      const insert=database.prepare('INSERT INTO dispatch_vehicle_assistants(dispatch_day_id,vehicle_id,employee_id,employment_period_id) VALUES(?,?,?,?)')
      for(const employeeId of assistantIds)insert.run(day.id,vehicleId,employeeId,currentEmploymentPeriod(database,employeeId))
      for(const trip of trips)database.prepare('UPDATE dispatches SET assistant_id=?,assistant_employment_period_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(assistantIds[0]||null,currentEmploymentPeriod(database,assistantIds[0]||null),trip.id)
    }
    ...
    database.prepare(`INSERT INTO dispatch_change_logs(dispatch_day_id,actor,change_type,entity_type,entity_id,before_json,after_json,requires_reapproval) VALUES(?,?,'route_day_handover','route',?,?,?,0)`).run(day.id,actor(context.employeeName),String(route),json(before),json({vehicleId,driverId,assistantIds,reason,date,temporary:true}))
    return getDispatchDay(date,database)
  })
}
...
function driverRouteForDate({employeeId,role,date,preview=false},database){
  ...
  const trips=database.prepare(`SELECT dt.id,dt.trip_number tripNumber,dt.execution_status executionStatus,dt.started_at startedAt${tripCompletion},d.vehicle_id vehicleId,v.vehicle_code vehicleCode,v.vehicle_name vehicleName,v.registration_number registrationNumber
    ...
    ORDER BY v.vehicle_code,dt.trip_number,dt.id`).all(...params).map(trip=>({...trip,stops:database.prepare(`SELECT ds.id,ds.route_number routeNumber,ds.stop_sequence stopSequence,ds.status,CASE WHEN ds.override_note='driver_deferred' THEN 1 ELSE 0 END deferred,ds.override_reason deferReason${stopCompletion},b.jodoo_branch_id branchId,b.branch_name branchName,c.name customerName,b.address,b.latitude,b.longitude,
      ...
      ORDER BY ds.stop_sequence,ds.id`).all(trip.id).map(stop=>({...stop,nextScheduledDate:nextBranchCollectionDate(database,stop.id,date),flexibleCollection:flexibleExecution(database,stop.id),noGoodsNotice:activeNoGoodsNotice(database,stop.id),arrangementRequests:database.prepare('SELECT id,kind,status,reason,review_reason reviewReason FROM driver_arrangement_requests WHERE dispatch_stop_id=? AND id IN (SELECT MAX(id) FROM driver_arrangement_requests WHERE dispatch_stop_id=? GROUP BY kind)').all(stop.id,stop.id),dateRequest:database.prepare('SELECT id,status,target_date targetDate,review_reason reviewReason FROM driver_date_requests WHERE dispatch_stop_id=? ORDER BY id DESC LIMIT 1').get(stop.id)||null,deferred:Boolean(stop.deferred),gpsAvailable:Boolean(stop.gpsAvailable),billCreated:Boolean(stop.billCreated),paymentProofUploaded:Boolean(stop.paymentProofUploaded)}))})).filter(trip=>trip.stops.length).map((trip,index,trips)=>{const tripApproved=legacyWholeDayApproval||trip.stops.every(stop=>approvedRoutes.has(Number(stop.routeNumber))),current=tripApproved&&trip.executionStatus==='in_progress'?trip.stops.find(stop=>!stop.deferred&&!['completed','cancelled'].includes(stop.status)):null,earlierOpen=trips.some(other=>other.vehicleId===trip.vehicleId&&other.tripNumber<trip.tripNumber&&other.stops.length&&other.executionStatus!=='completed'),finished=trip.stops.filter(stop=>stop.status==='completed').length;return{...trip,canAttemptComplete:!preview&&isDriver&&trip.executionStatus==='in_progress',approved:tripApproved,completedCount:trip.stops.filter(stop=>stop.status==='completed'&&!['no_goods','no_goods_notice'].includes(stop.completionOutcome)).length,noGoodsCount:trip.stops.filter(stop=>['no_goods','no_goods_notice'].includes(stop.completionOutcome)).length,totalCount:trip.stops.length,canComplete:!preview&&tripApproved&&trip.executionStatus==='in_progress'&&finished===trip.stops.length,canStart:!preview&&tripApproved&&['approved','reapproval_required','in_progress'].includes(day.status)&&trip.executionStatus==='not_started'&&!earlierOpen,currentStopId:current?.id||null,stops:trip.stops.map(stop=>({...stop,noGoodsApprovalRequired:requiresDriverApproval(date)&&!stop.verifiedArrival,canReportNoGoods:!preview&&!['completed','cancelled'].includes(stop.status)&&!stop.billCreated&&trip.executionStatus!=='completed',canArrive:Boolean(!preview&&!['completed','cancelled'].includes(stop.status)&&tripApproved&&(stop.deferred||(trip.executionStatus==='in_progress'&&stop.flexibleCollection)||current&&current.id===stop.id)&&!stop.arrivedAt),canFinish:Boolean(!preview&&tripApproved&&(stop.deferred||(trip.executionStatus==='in_progress'&&stop.flexibleCollection)||current&&current.id===stop.id)&&stop.arrivedAt&&stop.status==='active'&&stop.deferApprovalStatus!=='pending')}))}})
  ...
  return{date,weekday,preview,driverApprovalRequired:requiresDriverApproval(date),trialOrderEnabled:!preview&&isRouteTrialDate(date)&&trips.some(trip=>trip.approved&&trip.executionStatus==='in_progress'),status:day.status,approved:trips.every(trip=>trip.approved),routeAvailable:true,jodooUrl:String(process.env.JODOO_FORM_URL||'https://www.jodoo.com/'),trips,vehicles,totalStops:stops.length,noGoodsCount:stops.filter(stop=>['no_goods','no_goods_notice'].includes(stop.completionOutcome)).length,completedStops:stops.filter(stop=>stop.status==='completed'&&!['no_goods','no_goods_notice'].includes(stop.completionOutcome)).length,pendingStops:stops.filter(stop=>stop.status!=='completed').length}
}
