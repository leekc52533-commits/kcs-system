...
export function listPurchaseBillArchive(filters={},database=defaultDb){
  const query=where(filters),headers=database.prepare(`SELECT pb.id,pb.bill_number billNumber,pb.service_date serviceDate,pb.total_cents totalCents,pb.payment_method paymentMethod,
    ...
    COALESCE((SELECT GROUP_CONCAT(e.name, ', ') FROM dispatch_vehicle_assistants dva JOIN employees e ON e.id=dva.employee_id WHERE dva.dispatch_day_id=pb.dispatch_day_id AND dva.vehicle_id=pb.vehicle_id),
      (SELECT e.name FROM dispatch_trips dt JOIN dispatches d ON d.id=dt.dispatch_id LEFT JOIN employees e ON e.id=d.assistant_id WHERE dt.id=pb.dispatch_trip_id),'') crew,
...
export function purchaseBillRows(filters={},database=defaultDb){
  ...
  return{...archive,rows:archive.items.flatMap(bill=>bill.items.map(item=>({Date:excelDate(bill.serviceDate),'PO No.':bill.billNumber,PaymentMethod:bill.paymentMethod,'Customer Name':bill.customerName,Branch:bill.branchName,Item:item.shortForm||item.item,Quantity:item.quantity,Price:item.unitPriceCents/100,'Item Total':item.itemTotalCents/100,Total:bill.totalCents/100,'Issued By':bill.issuedBy,Crew:bill.crew||'',Car:bill.registrationNumber||bill.vehicleCode||'','Payment Gambar':bill.proofId?`${bill.billNumber}_${safeFile(bill.proofName)}`:'','Void No.':bill.status==='voided'?bill.billNumber:''})))}
}
...
export async function purchaseBillsWorkbook(filters={},database=defaultDb,{uploadsRoot}={}){
  ...
  for(let row=2;row<=sheet.rowCount;row++){sheet.getCell(row,1).numFmt='dd-mm-yy';for(const column of [8,9,10])sheet.getCell(row,column).numFmt='0.00';sheet.getCell(row,7).numFmt='0.00####'}
