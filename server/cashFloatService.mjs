...
const expenseCategory=description=>EXPENSE_CATEGORIES.includes(String(description||''))?String(description):'Other'

export function listExpenseRecords(filters={},database=defaultDb){
  const clauses=["t.transaction_type='expense'"],params=[],from=String(filters.from||''),to=String(filters.to||''),category=String(filters.category||''),search=String(filters.search||'').trim()
  if(/^\d{4}-\d{2}-\d{2}$/.test(from)){clauses.push('t.service_date>=?');params.push(from)}
  if(/^\d{4}-\d{2}-\d{2}$/.test(to)){clauses.push('t.service_date<=?');params.push(to)}
  const employeeId=Number(filters.employeeId);if(employeeId>0){clauses.push('t.employee_id=?');params.push(employeeId)}
  if(EXPENSE_CATEGORIES.includes(category)){clauses.push('t.description=?');params.push(category)}
  else if(category==='Other'){clauses.push(`COALESCE(t.description,'') NOT IN (${EXPENSE_CATEGORIES.map(()=>'?').join(',')})`);params.push(...EXPENSE_CATEGORIES)}
  if(search){clauses.push("(lower(e.name) LIKE ? OR lower(COALESCE(t.description,'')) LIKE ? OR lower(COALESCE(t.reference_number,'')) LIKE ? OR lower(COALESCE(t.created_by_name_snapshot,'')) LIKE ?)");const term=`%${search.toLowerCase()}%`;params.push(term,term,term,term)}
  const items=database.prepare(`SELECT t.id,t.employee_id employeeId,e.employee_code employeeCode,e.name employeeName,t.amount_cents amountCents,t.service_date serviceDate,t.description,t.reference_number referenceNumber,t.proof_storage_key proofStorageKey,t.proof_content_type proofContentType,t.created_by_name_snapshot createdBy,t.created_at createdAt FROM cash_float_transactions t JOIN employees e ON e.id=t.employee_id WHERE ${clauses.join(' AND ')} ORDER BY t.service_date DESC,t.id DESC LIMIT 2000`).all(...params).map(item=>({...item,id:Number(item.id),employeeId:Number(item.employeeId),amountCents:Math.abs(Number(item.amountCents)),category:expenseCategory(item.description),hasProof:Boolean(item.proofStorageKey)}))
  const employees=database.prepare(`SELECT e.id employeeId,e.employee_code employeeCode,e.name employeeName,CASE WHEN a.is_active=1 AND m.is_selected=1 THEN 1 ELSE 0 END configured FROM employees e LEFT JOIN cash_float_accounts a ON a.employee_id=e.id LEFT JOIN cash_float_members m ON m.employee_id=e.id WHERE EXISTS(SELECT 1 FROM cash_float_transactions t WHERE t.employee_id=e.id AND t.transaction_type='expense') OR (e.is_active=1 AND e.employment_status='active' AND a.is_active=1 AND m.is_selected=1) ORDER BY e.name`).all().map(item=>({...item,employeeId:Number(item.employeeId),configured:Boolean(item.configured)}))
...
export async function expenseRecordsWorkbook(filters={},database=defaultDb,{uploadsRoot}={}){
  ...
  const records=workbook.addWorksheet('Expense Records');records.columns=[{header:'Date',key:'date',width:12},{header:'Employee ID',key:'code',width:15},{header:'Employee',key:'employee',width:27},{header:'Category',key:'category',width:18},{header:'Description',key:'description',width:35},{header:'Amount',key:'amount',width:15},{header:'Reference',key:'reference',width:20},{header:'Entered By',key:'createdBy',width:24},{header:'Entered Time',key:'createdAt',width:22},{header:'Receipt',key:'receipt',width:15}]
  for(const item of data.items)records.addRow({date:item.serviceDate,code:item.employeeCode||'',employee:item.employeeName,category:item.category,description:item.description||'',amount:item.amountCents/100,reference:item.referenceNumber||'',createdBy:item.createdBy||'',createdAt:item.createdAt,receipt:item.hasProof?'Receipt attached':''})
  records.addRow({description:'TOTAL',amount:data.totalCents/100});records.getRow(records.rowCount).font={bold:true}
  records.autoFilter={from:'A1',to:{row:Math.max(1,records.rowCount-1),column:records.columnCount}};records.views=[{state:'frozen',ySplit:1}];records.getRow(1).eachCell(cell=>{cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF176B5B'}}});for(let row=2;row<=records.rowCount;row++){records.getCell(row,1).numFmt='dd-mm-yy';records.getCell(row,6).numFmt='RM #,##0.00'}
