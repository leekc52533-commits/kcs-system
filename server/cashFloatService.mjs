...
export function addAdminExpense(payload={},context={},database=defaultDb,options={}){
  const category=String(payload.category||''),description=String(payload.description||'').trim(),paymentMethod=String(payload.paymentMethod||'')
...
export function listExpenseRecords(filters={},database=defaultDb){
  ...
  if(search){clauses.push("(lower(COALESCE(r.employeeName,'')) LIKE ? OR lower(r.description) LIKE ? OR lower(COALESCE(r.referenceNumber,'')) LIKE ? OR lower(r.createdBy) LIKE ?)");const term=`%${search.toLowerCase()}%`;params.push(term,term,term,term)}
  const items=database.prepare(`SELECT * FROM (
    SELECT 'employee-'||t.id recordKey,'employee' expenseType,t.id sourceId,t.employee_id employeeId,e.employee_code employeeCode,e.name employeeName,ABS(t.amount_cents) amountCents,t.service_date serviceDate,CASE WHEN t.description IN (${known}) THEN t.description ELSE 'Other' END category,t.description,NULL paymentMethod,t.reference_number referenceNumber,t.proof_storage_key proofStorageKey,t.proof_content_type proofContentType,t.created_by_name_snapshot createdBy,t.created_at createdAt
    FROM cash_float_transactions t JOIN employees e ON e.id=t.employee_id WHERE t.transaction_type='expense'
    UNION ALL
  ...
  return{items,employees,totalCents:items.reduce((sum,item)=>sum+item.amountCents,0)}
}
