...
export function listExpenseRecords(filters={},database=defaultDb){
  ...
  ) r WHERE ${clauses.join(' AND ')} ORDER BY r.serviceDate DESC,r.createdAt DESC LIMIT 2000`).all(...params).map(item=>({...item,sourceId:Number(item.sourceId),employeeId:item.employeeId==null?null:Number(item.employeeId),amountCents:Number(item.amountCents),hasProof:Boolean(item.proofStorageKey)}))
  ...
  return{items,employees,vehicles:expenseVehicles(database),totalCents:items.reduce((sum,item)=>sum+item.amountCents,0)}
}
