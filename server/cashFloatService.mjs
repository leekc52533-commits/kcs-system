import {allocateDocumentNumber,documentNumberMap,documentNumber} from './documentNumbers.mjs'
...
function addTransaction(employeeId,type,signedAmount,payload,context,database,uploadsRoot){
  ...
  try{return withImmediateTransaction(database,()=>{const result=database.prepare(`INSERT INTO cash_float_transactions(employee_id,transaction_type,amount_cents,service_date,payment_channel,description,reference_number,proof_storage_key,proof_original_name,proof_content_type,proof_size_bytes,created_by_employee_id,created_by_name_snapshot,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(employeeId,type,signedAmount,validDate(payload.serviceDate),payload.paymentChannel||'System',description||null,reference,proof.storageKey||null,proof.originalName||null,proof.contentType||null,proof.sizeBytes||null,actorId,actorName,when);if(type==='expense')allocateDocumentNumber(database,'E','employee-'+Number(result.lastInsertRowid),context.now||new Date());if(type==='expense')saveExpenseDetails(database,'employee',Number(result.lastInsertRowid),payload.expenseDetails);const alertId=refreshAlert(database,employeeId,when);return{documentNumber:type==='expense'?documentNumber(database,'employee-'+Number(result.lastInsertRowid)):null,transactionId:Number(result.lastInsertRowid),alertId,account:serializeAccount(accountRow(database,employeeId),database)}})}catch(error){if(proof.absolute&&fs.existsSync(proof.absolute))fs.unlinkSync(proof.absolute);throw error}
}
...
export function addAdminExpense(payload={},context={},database=defaultDb,options={}){
  ...
  const details=normalizeExpenseDetails(payload,database),{category,description}=details,paymentMethod=String(payload.paymentMethod||'')
...
export function listExpenseRecords(filters={},database=defaultDb){
  ...
  if(/^\d{4}-\d{2}-\d{2}$/.test(from)){clauses.push('r.serviceDate>=?');params.push(from)}
  if(/^\d{4}-\d{2}-\d{2}$/.test(to)){clauses.push('r.serviceDate<=?');params.push(to)}
  ...
  if(search){clauses.push("(lower(COALESCE(r.employeeName,'')) LIKE ? OR lower(r.description) LIKE ? OR lower(COALESCE(r.referenceNumber,'')) LIKE ? OR lower(r.createdBy) LIKE ? OR lower(COALESCE(r.vehiclePlate,'')) LIKE ? OR lower(COALESCE(r.companyName,'')) LIKE ? OR lower(COALESCE(r.tinNumber,'')) LIKE ? OR lower(COALESCE(r.remarks,'')) LIKE ?)");const term=`%${search.toLowerCase()}%`;params.push(term,term,term,term,term,term,term,term)}
  const items=database.prepare(`SELECT * FROM (
    SELECT 'employee-'||t.id recordKey,'employee' expenseType,t.id sourceId,t.employee_id employeeId,e.employee_code employeeCode,e.name employeeName,ABS(t.amount_cents) amountCents,t.service_date serviceDate,COALESCE(x.category,CASE WHEN t.description IN (${known}) THEN t.description ELSE 'Other' END) category,t.description,NULL paymentMethod,t.reference_number referenceNumber,t.proof_storage_key proofStorageKey,t.proof_content_type proofContentType,t.created_by_name_snapshot createdBy,t.created_at createdAt,x.vehicle_id vehicleId,x.vehicle_plate vehiclePlate,x.odometer_km odometerKm,x.company_name companyName,x.tin_number tinNumber,x.remarks
    FROM cash_float_transactions t JOIN employees e ON e.id=t.employee_id LEFT JOIN expense_details x ON x.employee_transaction_id=t.id WHERE t.transaction_type='expense'
    UNION ALL
    SELECT 'admin-'||a.id recordKey,'admin' expenseType,a.id sourceId,NULL employeeId,NULL employeeCode,'Admin / Company' employeeName,a.amount_cents amountCents,a.service_date serviceDate,a.category,a.description,a.payment_method paymentMethod,a.reference_number referenceNumber,a.receipt_storage_key proofStorageKey,a.receipt_content_type proofContentType,a.created_by_name_snapshot createdBy,a.created_at createdAt,x.vehicle_id vehicleId,x.vehicle_plate vehiclePlate,x.odometer_km odometerKm,x.company_name companyName,x.tin_number tinNumber,x.remarks
    FROM admin_expense_records a LEFT JOIN expense_details x ON x.admin_expense_id=a.id
  ) r WHERE ${clauses.join(' AND ')} ORDER BY r.serviceDate DESC,r.createdAt DESC`).all(...params).map(item=>({...item,sourceId:Number(item.sourceId),employeeId:item.employeeId==null?null:Number(item.employeeId),amountCents:Number(item.amountCents),hasProof:Boolean(item.proofStorageKey)}))
  const employees=database.prepare(`SELECT e.id employeeId,e.employee_code employeeCode,e.name employeeName,CASE WHEN a.is_active=1 AND m.is_selected=1 THEN 1 ELSE 0 END configured FROM employees e LEFT JOIN cash_float_accounts a ON a.employee_id=e.id LEFT JOIN cash_float_members m ON m.employee_id=e.id WHERE EXISTS(SELECT 1 FROM cash_float_transactions t WHERE t.employee_id=e.id AND t.transaction_type='expense') OR (e.is_active=1 AND e.employment_status='active' AND m.is_selected=1) ORDER BY e.name`).all().map(item=>({...item,employeeId:Number(item.employeeId),configured:Boolean(item.configured)}))
  const correctionCounts=database.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='expense_amount_corrections'").get()?new Map(database.prepare('SELECT record_key,COUNT(*) n FROM expense_amount_corrections GROUP BY record_key').all().map(r=>[r.record_key,r.n])):new Map()
  const numbers=documentNumberMap(database)
  for(const item of items){item.correctionCount=correctionCounts.get(item.recordKey)||0;item.documentNumber=numbers.get(item.recordKey)||null}
  const table=applyArchiveColumns('expense',items,filters);return{items:table.items,filterOptions:table.filterOptions,employees,vehicles:expenseVehicles(database),totalCents:table.items.reduce((sum,item)=>sum+item.amountCents,0)}
}
...
export function cashFloatProofFile(transactionId,database=defaultDb){return database.prepare(`SELECT COALESCE(t.proof_storage_key,p.storage_key) storageKey,CASE WHEN t.proof_storage_key IS NOT NULL THEN t.proof_content_type ELSE p.content_type END contentType FROM cash_float_transactions t LEFT JOIN purchase_payment_proofs p ON p.purchase_bill_id=t.purchase_bill_id WHERE t.id=? AND COALESCE(t.proof_storage_key,p.storage_key) IS NOT NULL`).get(Number(transactionId))||null}
export function expenseReceiptFile(recordKey,database=defaultDb){const match=/^(employee|admin)-(\d+)$/.exec(String(recordKey||''));if(!match)return null;return match[1]==='employee'?cashFloatProofFile(Number(match[2]),database):database.prepare('SELECT receipt_storage_key storageKey,receipt_content_type contentType FROM admin_expense_records WHERE id=? AND receipt_storage_key IS NOT NULL').get(Number(match[2]))||null}

export async function cashFloatWorkbook(filters={},database=defaultDb){if(filters.from||filters.to){const from=expenseServiceDate(filters.from),to=expenseServiceDate(filters.to);if(from>to)throw fail('Invalid date range.','CASH_DATE_INVALID');}const transactions=listCashFloatTransactions(filters,database).items,workbook=new ExcelJS.Workbook();workbook.creator='KCS Dispatch System';const ledger=workbook.addWorksheet('Cash Float Ledger');ledger.columns=[{header:'Date',key:'date',width:12},{header:'Employee',key:'employee',width:26},{header:'Type',key:'type',width:18},{header:'Amount',key:'amount',width:14},{header:'Channel',key:'channel',width:16},{header:'PO No.',key:'bill',width:20},{header:'Description',key:'description',width:35},{header:'Reference',key:'reference',width:20},{header:'Entered By',key:'createdBy',width:24},{header:'Time',key:'time',width:22}];transactions.forEach(item=>ledger.addRow({date:item.serviceDate,employee:item.employeeName,type:item.transactionType,amount:item.amountCents/100,channel:item.paymentChannel||'',bill:item.billNumber||'',description:item.description||'',reference:item.referenceNumber||'',createdBy:item.createdBy,time:item.createdAt}));for(const sheet of [ledger]){sheet.autoFilter={from:'A1',to:{row:Math.max(1,sheet.rowCount),column:sheet.columnCount}};sheet.views=[{state:'frozen',ySplit:1}];sheet.getRow(1).eachCell(cell=>{cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF176B5B'}}})}for(let row=2;row<=ledger.rowCount;row++)ledger.getCell(row,4).numFmt='"RM" #,##0.00' ;return Buffer.from(await workbook.xlsx.writeBuffer())}

export async function expenseRecordsWorkbook(filters={},database=defaultDb,{uploadsRoot}={}){
  const data=listExpenseRecords(filters,database),workbook=new ExcelJS.Workbook();workbook.creator='KCS Dispatch System'
  const records=workbook.addWorksheet('Expense Records');records.columns=[{header:'Date',key:'date',width:12},{header:'Type',key:'type',width:16},{header:'Employee ID',key:'code',width:15},{header:'Employee / Admin',key:'employee',width:27},{header:'Category',key:'category',width:18},{header:'Description',key:'description',width:35},{header:'Amount',key:'amount',width:15},{header:'Payment Method',key:'paymentMethod',width:18},{header:'Reference',key:'reference',width:20},{header:'Entered By',key:'createdBy',width:24},{header:'Entered Time',key:'createdAt',width:22},{header:'Receipt',key:'receipt',width:15},{header:'Vehicle',key:'vehiclePlate',width:18},{header:'Odometer (km)',key:'odometerKm',width:18},{header:'Company Name',key:'companyName',width:30},{header:'TIN',key:'tinNumber',width:22},{header:'Remarks',key:'remarks',width:40},{header:'Expense No.',key:'expenseNumber',width:22},{header:'Correction history',key:'correctionStatus',width:25}]
  for(const item of data.items)records.addRow({expenseNumber:item.documentNumber||`EXP-${item.expenseType==='employee'?'E':'A'}-${String(item.sourceId).padStart(6,'0')}`,correctionStatus:item.correctionCount?`Corrected (${item.correctionCount})`:'Unchanged',vehiclePlate:item.vehiclePlate,odometerKm:item.odometerKm,companyName:item.companyName,tinNumber:item.tinNumber,remarks:item.remarks,date:item.serviceDate,type:item.expenseType==='admin'?'Admin Expense':'Employee Expense',code:item.employeeCode||'',employee:item.employeeName,category:item.category,description:item.description||'',amount:item.amountCents/100,paymentMethod:item.paymentMethod||'',reference:item.referenceNumber||'',createdBy:item.createdBy||'',createdAt:item.createdAt,receipt:item.hasProof?'Receipt attached':''})
  records.addRow({description:'TOTAL',amount:data.totalCents/100});records.getRow(records.rowCount).font={bold:true}
  records.autoFilter={from:'A1',to:{row:Math.max(1,records.rowCount-1),column:records.columnCount}};records.views=[{state:'frozen',ySplit:1}];records.getRow(1).eachCell(cell=>{cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF176B5B'}}});for(let row=2;row<=records.rowCount;row++){records.getCell(row,1).numFmt='dd-mm-yy';records.getCell(row,7).numFmt='"RM" #,##0.00'}
  const photos=workbook.addWorksheet('Receipt Photos');photos.columns=[{header:'Date',key:'date',width:12},{header:'Employee',key:'employee',width:27},{header:'Receipt Photo',key:'photo',width:58}];photos.getRow(1).eachCell(cell=>{cell.font={bold:true,color:{argb:'FFFFFFFF'}};cell.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF176B5B'}}});photos.views=[{state:'frozen',ySplit:1}]
