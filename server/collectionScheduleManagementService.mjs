...
export function saveCollectionScheduleManagement(id,payload={},database=defaultDb){const reason=String(payload.reason||'').trim();if(!reason)throw new Error('Reason is required.');const initial=branch(database,id);if(!initial)throw Object.assign(new Error('Active Branch not found.'),{statusCode:404});let schedules=activeSchedules(database,initial.id);if(schedules.length>1)throw Object.assign(new Error('Blocked: Multiple active Schedules exist for this Branch.'),{statusCode:409});const before=scheduleState(schedules[0])||{frequency:initial.collection_frequency,weekdays:days(initial.assigned_weekdays)};const after=normalized(payload,before,initial),impact=futureStopWarning(database,initial,after);if(payload.dryRun===true)return{dryRun:true,action:schedules.length?'update':'create',before,after,...impact}
