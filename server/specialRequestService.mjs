...
export function adoptTemporaryLocation(id,{adoptedBy='Supervisor',adoptedByAccountId=null,reason=''}={},database=defaultDb){const location=database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(id);if(!location)throw new Error('Temporary location not found');if(location.verification_status!=='pending_supervisor')throw new Error('Temporary GPS is no longer pending approval');let branchId=location.branch_id;if(!branchId&&location.special_request_id)branchId=database.prepare('SELECT existing_branch_id id FROM special_collection_requests WHERE id=?').get(location.special_request_id)?.id;if(!branchId)throw new Error('临时位置尚未连接正式 Branch');const official=database.prepare('SELECT latitude,longitude FROM branches WHERE id=?').get(branchId);if(official?.latitude!=null&&official?.longitude!=null)throw new Error('Official GPS already exists for this Branch. Withdraw it before approving another GPS.');const distance=null;database.exec('BEGIN IMMEDIATE');try{database.prepare(`UPDATE branches SET latitude=?,longitude=?,gps_status='Supervisor Confirmed',gps_verified_at=CURRENT_TIMESTAMP,gps_address=?,gps_state=?,gps_street=?,gps_city=?,gps_street_number=?,gps_postal_code=?,gps_remark=?,gps_reverse_geocode_provider=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(location.latitude,location.longitude,location.address,location.state,location.street,location.city,location.street_number,location.postal_code,location.remark,location.reverse_geocode_provider,branchId);database.prepare(`INSERT INTO branch_gps_history(branch_id,action,latitude,longitude,address,state,street,city,street_number,postal_code,remark,reverse_geocode_provider,actor,reason) VALUES(?,'approved',?,?,?,?,?,?,?,?,?,?,?,?)`).run(branchId,location.latitude,location.longitude,location.address,location.state,location.street,location.city,location.street_number,location.postal_code,location.remark,location.reverse_geocode_provider,adoptedBy,clean(reason)||null);database.prepare(`UPDATE temporary_locations SET verification_status='adopted',review_decision='adopt',review_reason=?,reviewed_by_account_id=?,reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP,distance_from_official_m=?,adopted_by=?,adopted_at=CURRENT_TIMESTAMP WHERE id=?`).run(clean(reason)||null,adoptedByAccountId,adoptedBy,distance,adoptedBy,id);database.prepare(`INSERT INTO audit_logs(action,entity_type,entity_id,before_json,after_json) VALUES('temporary_gps_reviewed','branch',?,?,?)`).run(String(branchId),JSON.stringify({officialGps:official}),JSON.stringify({suggestedGps:{latitude:location.latitude,longitude:location.longitude,address:location.address,state:location.state,city:location.city,street:location.street,streetNumber:location.street_number,postalCode:location.postal_code,remark:location.remark},decision:'adopt',decidedBy:adoptedBy,reason:clean(reason)||null}));const dates=database.prepare(`SELECT DISTINCT dd.dispatch_date FROM dispatch_days dd JOIN dispatch_trips dt ON dt.dispatch_day_id=dd.id JOIN dispatch_stops ds ON ds.dispatch_trip_id=dt.id WHERE ds.branch_id=? AND dd.dispatch_date>=date('now','+8 hours')`).all(branchId);for(const day of dates)invalidateDispatchDay(database,day.dispatch_date,'official_gps_adopted','branch',branchId,official,{latitude:location.latitude,longitude:location.longitude},adoptedBy);database.exec('COMMIT')}catch(error){database.exec('ROLLBACK');throw error}recalculateRecommendations({branchIds:[branchId],changedBy:adoptedBy},database);return{...database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(id),distanceWarning:false}}
