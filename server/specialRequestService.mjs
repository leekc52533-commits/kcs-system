...
export function addTemporaryLocation(payload,database=defaultDb){const latitude=Number(payload.latitude),longitude=Number(payload.longitude),capturedLatitude=Number(payload.capturedLatitude??payload.latitude),capturedLongitude=Number(payload.capturedLongitude??payload.longitude),capturedAccuracy=payload.capturedAccuracyM??payload.accuracyM??null,manuallyAdjusted=Boolean(payload.manuallyAdjusted),adjustmentDistance=manuallyAdjusted?distanceMeters(capturedLatitude,capturedLongitude,latitude,longitude):0;if(!Number.isFinite(latitude)||latitude < -90||latitude>90||!Number.isFinite(longitude)||longitude < -180||longitude>180||!Number.isFinite(capturedLatitude)||!Number.isFinite(capturedLongitude))throw new Error('Invalid GPS latitude or longitude');const photo=saveGpsPhoto(payload.photo),periodId=payload.employeeId?database.prepare('SELECT id FROM employee_employment_history WHERE employee_id=? ORDER BY id DESC LIMIT 1').get(payload.employeeId)?.id||null:null;try{const result=database.prepare(`INSERT INTO temporary_locations(special_request_id,branch_id,latitude,longitude,location_source,location_link,verification_status,captured_by,accuracy_m,device_captured_at,server_received_at,employee_id,employment_period_id,dispatch_id,dispatch_stop_id,photo_storage_key,photo_original_name,photo_content_type,remark,address,state,street,city,street_number,postal_code,reverse_geocode_provider,captured_latitude,captured_longitude,captured_accuracy_m,manually_adjusted,adjusted_by,adjusted_at,adjustment_reason,adjustment_distance_m) VALUES(?,?,?,?,?,?,'pending_supervisor',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(payload.specialRequestId||null,payload.branchId||null,latitude,longitude,payload.locationSource||'Driver Captured',payload.locationLink||null,payload.capturedBy||'Driver',capturedAccuracy,payload.deviceCapturedAt||null,new Date().toISOString(),payload.employeeId||null,periodId,payload.dispatchId||null,payload.dispatchStopId||null,photo.photoStorageKey||null,photo.photoOriginalName||null,photo.photoContentType||null,clean(payload.remark)||null,clean(payload.address)||null,clean(payload.state)||null,clean(payload.street)||null,clean(payload.city)||null,clean(payload.streetNumber)||null,clean(payload.postalCode)||null,clean(payload.reverseGeocodeProvider)||null,capturedLatitude,capturedLongitude,capturedAccuracy,manuallyAdjusted?1:0,manuallyAdjusted?clean(payload.adjustedBy)||clean(payload.capturedBy)||null:null,manuallyAdjusted?new Date().toISOString():null,manuallyAdjusted?clean(payload.adjustmentReason)||clean(payload.remark)||null:null,adjustmentDistance);return database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(result.lastInsertRowid)}catch(error){if(photo.photoStorageKey)fs.rmSync(path.join(uploadsDir,photo.photoStorageKey),{force:true});throw error}}
...
export function adoptTemporaryLocation(id,{adoptedBy='Supervisor',adoptedByAccountId=null,reason=''}={},database=defaultDb){
  const location=database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(id)
  if(!location)throw new Error('Temporary location not found')
  if(location.verification_status!=='pending_supervisor')throw new Error('Temporary GPS is no longer pending approval')
  let branchId=location.branch_id
  if(!branchId&&location.special_request_id)branchId=database.prepare('SELECT existing_branch_id id FROM special_collection_requests WHERE id=?').get(location.special_request_id)?.id
  if(!branchId)throw new Error('临时位置尚未连接正式 Branch')
  const official=database.prepare('SELECT jodoo_branch_id,latitude,longitude FROM branches WHERE id=?').get(branchId)
  const sourceRow=database.prepare("SELECT after_json FROM audit_logs WHERE action='gps_change_source' AND entity_id=? ORDER BY id DESC LIMIT 1").get(String(id))
  const source=sourceRow?JSON.parse(sourceRow.after_json):null
  if(source){
    const currentHistory=database.prepare('SELECT MAX(id) id FROM branch_gps_history WHERE branch_id=?').get(branchId).id
    if(source.latitude!==official.latitude||source.longitude!==official.longitude||source.historyId!==currentHistory)throw new Error('Official GPS changed after this request. Reject and capture again.')
    if(!clean(reason))throw new Error('A supervisor reason is required to change GPS.')
  }else if(official?.latitude!=null&&official?.longitude!=null)throw new Error('Official GPS already exists for this Branch. Withdraw it before approving another GPS.')
  const reviewReason=clean(reason)||null,reviewerAccountId=adoptedByAccountId==null?null:Number(adoptedByAccountId),distance=null
  const proposed={latitude:location.latitude,longitude:location.longitude,address:location.address,state:location.state,city:location.city,street:location.street,streetNumber:location.street_number,postalCode:location.postal_code,remark:location.remark}
  database.exec('BEGIN IMMEDIATE')
  try{
    const live=database.prepare('SELECT latitude,longitude FROM branches WHERE id=?').get(branchId)
    if(live.latitude!==official.latitude||live.longitude!==official.longitude||database.prepare('SELECT verification_status FROM temporary_locations WHERE id=?').get(id)?.verification_status!=='pending_supervisor')throw new Error('GPS changed during approval. Reload and review again.')
    database.prepare(`UPDATE branches SET latitude=?,longitude=?,gps_status='Supervisor Confirmed',gps_verified_at=CURRENT_TIMESTAMP,gps_address=?,gps_state=?,gps_street=?,gps_city=?,gps_street_number=?,gps_postal_code=?,gps_remark=?,gps_reverse_geocode_provider=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(location.latitude,location.longitude,location.address,location.state,location.street,location.city,location.street_number,location.postal_code,location.remark,location.reverse_geocode_provider,branchId)
    database.prepare(`INSERT INTO branch_gps_history(branch_id,action,latitude,longitude,address,state,street,city,street_number,postal_code,remark,reverse_geocode_provider,actor,reason) VALUES(?,'approved',?,?,?,?,?,?,?,?,?,?,?,?)`).run(branchId,location.latitude,location.longitude,location.address,location.state,location.street,location.city,location.street_number,location.postal_code,location.remark,location.reverse_geocode_provider,adoptedBy,reviewReason)
    database.prepare(`UPDATE temporary_locations SET verification_status='adopted',review_decision='adopt',review_reason=?,reviewed_by_account_id=?,reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP,distance_from_official_m=?,adopted_by=?,adopted_at=CURRENT_TIMESTAMP WHERE id=?`).run(reviewReason,reviewerAccountId,adoptedBy,distance,adoptedBy,id)
    database.prepare(`INSERT INTO master_change_history(entity_type,entity_id,change_type,before_json,after_json,reason,changed_by) VALUES('branch',?,'official_gps_adopted',?,?,?,?)`).run(String(official.jodoo_branch_id),JSON.stringify({latitude:official.latitude,longitude:official.longitude}),JSON.stringify({...proposed,distanceWarning:false}),reviewReason,adoptedBy)
    database.prepare(`INSERT INTO audit_logs(action,entity_type,entity_id,before_json,after_json) VALUES('temporary_gps_reviewed','branch',?,?,?)`).run(String(branchId),JSON.stringify({officialGps:{latitude:official.latitude,longitude:official.longitude}}),JSON.stringify({temporaryLocationId:id,suggestedGps:proposed,decision:'adopt',decidedBy:adoptedBy,reviewedByAccountId:reviewerAccountId,reason:reviewReason}))
    const dates=database.prepare(`SELECT DISTINCT dd.dispatch_date FROM dispatch_days dd JOIN dispatch_trips dt ON dt.dispatch_day_id=dd.id JOIN dispatch_stops ds ON ds.dispatch_trip_id=dt.id WHERE ds.branch_id=? AND dd.dispatch_date>=date('now','+8 hours')`).all(branchId)
    for(const day of dates)invalidateDispatchDay(database,day.dispatch_date,'official_gps_adopted','branch',branchId,official,{latitude:location.latitude,longitude:location.longitude},adoptedBy)
    database.exec('COMMIT')
  }catch(error){database.exec('ROLLBACK');throw error}
  recalculateRecommendations({branchIds:[branchId],changedBy:adoptedBy},database)
  return{...database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(id),distanceWarning:false}
}
export function reviewTemporaryLocation(id,payload,database=defaultDb){const decision=clean(payload.decision),location=database.prepare('SELECT tl.*,b.latitude official_latitude,b.longitude official_longitude FROM temporary_locations tl LEFT JOIN branches b ON b.id=tl.branch_id WHERE tl.id=?').get(id);if(!location)throw new Error('Temporary location not found');if(location.verification_status!=='pending_supervisor')throw new Error('Temporary GPS is no longer pending approval');if(decision==='adopt')return adoptTemporaryLocation(id,{adoptedBy:payload.reviewedBy||'Supervisor',adoptedByAccountId:payload.reviewedByAccountId,reason:payload.reason},database);if(!['reject','recapture','keep_official','later'].includes(decision))throw new Error('Invalid GPS review decision');const status={reject:'rejected',recapture:'recapture_requested',keep_official:'kept_official',later:'pending_supervisor'}[decision],actor=payload.reviewedBy||'Supervisor';database.exec('BEGIN IMMEDIATE');try{database.prepare(`UPDATE temporary_locations SET verification_status=?,review_decision=?,review_reason=?,reviewed_by_account_id=?,reviewed_by=?,reviewed_at=CURRENT_TIMESTAMP WHERE id=?`).run(status,decision,clean(payload.reason)||null,payload.reviewedByAccountId||null,actor,id);database.prepare(`INSERT INTO audit_logs(action,entity_type,entity_id,before_json,after_json) VALUES('temporary_gps_reviewed','branch',?,?,?)`).run(String(location.branch_id||''),JSON.stringify({officialGps:{latitude:location.official_latitude,longitude:location.official_longitude}}),JSON.stringify({suggestedGps:{latitude:location.latitude,longitude:location.longitude},decision,decidedBy:actor,reason:clean(payload.reason)||null}));database.exec('COMMIT')}catch(error){database.exec('ROLLBACK');throw error}return database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(id)}
