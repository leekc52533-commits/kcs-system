...
export function createSpecialRequest(payload,database=defaultDb){
  ...
  if(payload.temporaryLatitude!=null&&payload.temporaryLongitude!=null)database.prepare(`INSERT INTO temporary_locations(special_request_id,latitude,longitude,location_source,location_link,verification_status,captured_by,employee_id,accuracy_m,device_captured_at,server_received_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(result.lastInsertRowid,payload.temporaryLatitude,payload.temporaryLongitude,payload.locationSource||'Manual Entry',payload.locationLink||null,payload.verificationStatus||'pending_supervisor',payload.createdBy||'Office',payload.employeeId||null,payload.accuracyM??null,payload.deviceCapturedAt||null,new Date().toISOString())
...
export function addTemporaryLocation(payload,database=defaultDb){const latitude=Number(payload.latitude),longitude=Number(payload.longitude);if(!Number.isFinite(latitude)||latitude < -90||latitude>90||!Number.isFinite(longitude)||longitude < -180||longitude>180)throw new Error('Invalid GPS latitude or longitude');const photo=saveGpsPhoto(payload.photo);const result=database.prepare(`INSERT INTO temporary_locations(special_request_id,branch_id,latitude,longitude,location_source,location_link,verification_status,captured_by,accuracy_m,device_captured_at,server_received_at,employee_id,dispatch_id,dispatch_stop_id,photo_storage_key,photo_original_name,photo_content_type,remark) VALUES(?,?,?,?,?,?,'pending_supervisor',?,?,?,?,?,?,?,?,?,?,?)`).run(payload.specialRequestId||null,payload.branchId||null,latitude,longitude,payload.locationSource||'Driver Captured',payload.locationLink||null,payload.capturedBy||'Driver',payload.accuracyM??null,payload.deviceCapturedAt||null,new Date().toISOString(),payload.employeeId||null,payload.dispatchId||null,payload.dispatchStopId||null,photo.photoStorageKey||null,photo.photoOriginalName||null,photo.photoContentType||null,clean(payload.remark)||null);return database.prepare('SELECT * FROM temporary_locations WHERE id=?').get(result.lastInsertRowid)}
