...
export function captureBranchGps(branchId,payload,database=defaultDb){const branch=database.prepare('SELECT id,jodoo_branch_id FROM branches WHERE jodoo_branch_id=?').get(branchId);if(!branch)throw new Error('Please select a valid Customer Branch first');const latitude=Number(payload.latitude),longitude=Number(payload.longitude);if(!Number.isFinite(latitude)||latitude < -90||latitude>90||!Number.isFinite(longitude)||longitude < -180||longitude>180)throw new Error('Invalid GPS latitude or longitude');const item=addTemporaryLocation({branchId:branch.id,latitude,longitude,locationSource:payload.locationSource||'Driver Captured',locationLink:payload.locationLink,capturedBy:payload.capturedBy||payload.changedBy||'Employee'},database);history(database,'branch',branchId,'temporary_gps_captured',null,item,{changedBy:payload.capturedBy||payload.changedBy,reason:payload.reason});return item}
