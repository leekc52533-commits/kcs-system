...
export function listBuyers(params={},database=defaultDb){const where=['1=1'],args=[];if(params.search){const search=text(params.search),q=`%${search}%`;if(/^[Bb]?\d+$/.test(search)){where.push('(id=? OR buyer_code LIKE ? OR buyer_name LIKE ? OR location_name LIKE ? OR phone LIKE ?)');args.push(Number(parseTypedId(search,'buyer')),q,q,q,q)}else{where.push('(buyer_code LIKE ? OR buyer_name LIKE ? OR location_name LIKE ? OR phone LIKE ?)');args.push(q,q,q,q)}}if(params.status){where.push('status=?');args.push(params.status)}return database.prepare(`SELECT id,buyer_code buyerId,buyer_name buyerName,location_name locationName,address,latitude officialLatitude,longitude officialLongitude,contact_person contactPerson,phone,material_accepted materialAccepted,operating_hours operatingHours,unloading_restrictions unloadingRestrictions,pricing_notes pricingNotes,status,notes,created_by createdBy,created_at createdAt,updated_at updatedAt FROM buyers WHERE ${where.join(' AND ')} ORDER BY buyer_name`).all(...args)}
