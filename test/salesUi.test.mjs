...
test('three-language review form requires confirmation and blocks a mismatched total',async()=>{
 ...
 const price=document.querySelectorAll('fieldset input, fieldset select')[4];await change(price,'0.60')
 assert.equal(check().checked,false);assert.equal(document.querySelectorAll('fieldset input, fieldset select')[5].value,'12.00');assert.equal(save().disabled,true)
...
test('several prices for a material require choosing the actual price',async()=>{
 ...
 assert.equal(document.querySelectorAll('fieldset label input, fieldset label select').length,6)
 assert.ok([...document.querySelectorAll('fieldset select option')].some(option=>option.value==='MIX PAPER'))
 await change(document.querySelectorAll('fieldset input, fieldset select')[2],'MIX PAPER')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[4].value,'')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[5].value,'')
 assert.deepEqual([...document.querySelectorAll('#sales-prices-0 option')].map(option=>option.value),['0.250','0.300'])
 await change(document.querySelectorAll('fieldset input, fieldset select')[4],'0.30')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[4].value,'0.30')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[5].value,'6.00')
 await change(document.querySelectorAll('fieldset input, fieldset select')[4],'0.28')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[5].value,'5.60')
 await change(document.querySelectorAll('fieldset input, fieldset select')[2],'CARDBOARD')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[4].value,'0.170')
 assert.equal(document.querySelectorAll('fieldset input, fieldset select')[5].value,'3.40')
