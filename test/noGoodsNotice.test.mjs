...
test('pending date request is superseded and an already-arrived stop retains arrival evidence',t=>{
 ...
 test('closed untouched hidden stop is cancelled with audit when driver finishes, never counted as collected',t=>{
 const{db,ids,trip,uploadsRoot}=setup(t)
 for(const id of ids.slice(0,2))submitNoGoodsNotice(id,payload,context,db,{uploadsRoot})
 db.prepare("UPDATE branches SET status='closed' WHERE id=(SELECT branch_id FROM dispatch_stops WHERE id=?)").run(ids[2])
 const view=driverToday(context,db);assert.equal(view.pendingStops,0);assert.equal(view.trips[0].canComplete,true)
 assert.equal(completeDriverTrip(trip,context,db).status,'completed')
 const stop=db.prepare('SELECT * FROM dispatch_stops WHERE id=?').get(ids[2]);assert.equal(stop.status,'cancelled');assert.equal(stop.completion_outcome,null);assert.equal(stop.completed_at,null)
 assert.equal(db.prepare("SELECT COUNT(*) n FROM dispatch_change_logs WHERE change_type='closed_branch_stop_retired'").get().n,1)
 assert.equal(completeDriverTrip(trip,context,db).idempotent,true)
 })
 test('active unfinished stop still blocks and rolls back closed-stop retirement',t=>{
 const{db,ids,trip}=setup(t)
 db.prepare("UPDATE branches SET status='closed' WHERE id=(SELECT branch_id FROM dispatch_stops WHERE id=?)").run(ids[2])
 assert.throws(()=>completeDriverTrip(trip,context,db),e=>e.code==='UNFINISHED_STOPS')
 assert.notEqual(db.prepare('SELECT status FROM dispatch_stops WHERE id=?').get(ids[2]).status,'cancelled')
 assert.equal(db.prepare("SELECT COUNT(*) n FROM dispatch_change_logs WHERE change_type='closed_branch_stop_retired'").get().n,0)
 })
 test('closed branch with arrival evidence remains protected and blocks completion',t=>{
 const{db,ids,trip,uploadsRoot}=setup(t)
 for(const id of ids.slice(0,2))submitNoGoodsNotice(id,payload,context,db,{uploadsRoot})
 db.prepare("UPDATE branches SET status='closed' WHERE id=(SELECT branch_id FROM dispatch_stops WHERE id=?)").run(ids[2])
 db.prepare("UPDATE dispatch_stops SET arrived_at='2026-09-10T12:00:00+08:00' WHERE id=?").run(ids[2])
 assert.throws(()=>completeDriverTrip(trip,context,db),e=>e.code==='UNFINISHED_STOPS')
 assert.notEqual(db.prepare('SELECT status FROM dispatch_stops WHERE id=?').get(ids[2]).status,'cancelled')
 })

import {listTripExceptions,cancelExceptionStop,completeExceptionTrip} from '../server/tripExceptions.mjs'
test('exception center exposes all hidden stops and records reasoned management cancellation and completion',t=>{
 const{db,ids,trip}=setup(t);db.exec("UPDATE branches SET status='closed'")
 const before=listTripExceptions(today,office,db);assert.equal(before.items[0].stops.length,3);assert.equal(before.items[0].canComplete,false)
 for(const id of ids)cancelExceptionStop(id,{reason:'Confirmed business closed'},office,db)
 assert.equal(listTripExceptions(today,office,db).items[0].canComplete,true)
 completeExceptionTrip(trip,{reason:'All closed branches reviewed'},office,db)
 const after=listTripExceptions(today,office,db);assert.equal(after.items.length,0);assert.equal(after.history.length,4)
 assert.equal(after.history[0].details.reason,'All closed branches reviewed');assert.equal(after.history[0].actor,'3')
 assert.equal(db.prepare('SELECT completed_by_employee_id e FROM dispatch_trips WHERE id=?').get(trip).e,3)
 assert.equal(db.prepare("SELECT COUNT(*) n FROM dispatch_stops WHERE dispatch_trip_id=? AND status='cancelled' AND completed_at IS NULL").get(trip).n,3)
 assert.throws(()=>completeExceptionTrip(trip,{reason:'Repeat'},office,db),e=>e.code==='INVALID_STATUS');assert.equal(listTripExceptions(today,office,db).history.length,4)
 assert.deepEqual(db.prepare('PRAGMA foreign_key_check').all(),[])
})
test('exception center denies employees, blank reason and active branch cancellation; completion cannot bypass blockers',t=>{
 const{db,ids,trip}=setup(t)
 for(const role of ['driver','crew']){const ctx={...context,role};assert.throws(()=>listTripExceptions(today,ctx,db),e=>e.code==='PERMISSION_DENIED');assert.throws(()=>cancelExceptionStop(ids[0],{reason:'x'},ctx,db),e=>e.code==='PERMISSION_DENIED');assert.throws(()=>completeExceptionTrip(trip,{reason:'x'},ctx,db),e=>e.code==='PERMISSION_DENIED')}
 assert.throws(()=>cancelExceptionStop(ids[0],{reason:' '},office,db),e=>e.code==='REQUIRED_FIELD')
 assert.throws(()=>cancelExceptionStop(ids[0],{reason:'x'},office,db),e=>e.code==='CONFLICT')
 assert.throws(()=>completeExceptionTrip(trip,{reason:'x'},office,db),e=>e.code==='CONFLICT')
 assert.throws(()=>listTripExceptions('2026-02-31',office,db),e=>e.code==='REQUIRED_FIELD')
 assert.equal(listTripExceptions(today,office,db).history.length,0)
})
test('exception cancellation revalidates arrival, reactivation and pending approval since preview',t=>{
 const{db,ids}=setup(t);db.exec("UPDATE branches SET status='closed'")
 assert.ok(listTripExceptions(today,office,db).items[0].stops.every(s=>s.canCancel))
 db.prepare("UPDATE dispatch_stops SET arrived_at='2026-09-10T12:00:00+08:00' WHERE id=?").run(ids[0])
 assert.throws(()=>cancelExceptionStop(ids[0],{reason:'x'},office,db),e=>e.code==='CONFLICT')
 db.prepare("UPDATE branches SET status='active' WHERE id=(SELECT branch_id FROM dispatch_stops WHERE id=?)").run(ids[1])
 assert.throws(()=>cancelExceptionStop(ids[1],{reason:'x'},office,db),e=>e.code==='CONFLICT')
 db.prepare("INSERT INTO driver_date_requests(dispatch_stop_id,employee_id,source_date,target_date,reason) VALUES(?,1,?,'2026-09-17','Later')").run(ids[2],today)
 assert.throws(()=>cancelExceptionStop(ids[2],{reason:'x'},office,db),e=>e.code==='CONFLICT')
 assert.equal(listTripExceptions(today,office,db).items[0].stops[2].issue,'approval')
})
test('exception cancellation rolls back on audit failure and never closes other trips',t=>{
 const{db,ids,trip}=setup(t);db.exec("UPDATE branches SET status='closed'")
 db.exec("CREATE TRIGGER exception_audit_failure BEFORE INSERT ON dispatch_change_logs WHEN NEW.change_type='trip_exception_cancel' BEGIN SELECT RAISE(ABORT,'audit failed'); END")
 assert.throws(()=>cancelExceptionStop(ids[0],{reason:'x'},office,db),/audit failed/)
 assert.notEqual(db.prepare('SELECT status FROM dispatch_stops WHERE id=?').get(ids[0]).status,'cancelled')
 db.exec('DROP TRIGGER exception_audit_failure')
 for(const id of ids)cancelExceptionStop(id,{reason:'Closed'},office,db)
 const original=db.prepare('SELECT * FROM dispatch_trips WHERE id=?').get(trip)
 const other=Number(db.prepare("INSERT INTO dispatches(dispatch_date,vehicle_id,driver_id,status) VALUES(?,1,1,'in_progress')").run(today).lastInsertRowid)
 db.prepare("INSERT INTO dispatch_trips(dispatch_id,dispatch_day_id,trip_number,execution_status) VALUES(?,?,2,'in_progress')").run(other,original.dispatch_day_id)
 completeExceptionTrip(trip,{reason:'Reviewed'},office,db)
 assert.equal(db.prepare('SELECT status FROM dispatches WHERE id=?').get(other).status,'in_progress')
