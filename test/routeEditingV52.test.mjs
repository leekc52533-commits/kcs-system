...
test('moving a customer changes only the selected date, preserving other Mondays',()=>{const db=fixture();generateDay({startDate:'2026-09-07'},db);generateDay({startDate:'2026-09-14'},db);let day=getDispatchDay('2026-09-07',db),third=day.routeBoards[0].stops[2];reorderRouteStop('2026-09-07',1,{stopId:third.id,direction:'up',changedBy:'Planner'},db);assert.deepEqual(getDispatchDay('2026-09-07',db).routeBoards[0].stops.map(row=>row.branchId),['B1','B3','B2']);assert.deepEqual(getDispatchDay('2026-09-14',db).routeBoards[0].stops.map(row=>row.branchId),['B1','B2','B3']);db.prepare("UPDATE dispatch_days SET status='approved' WHERE dispatch_date='2026-09-07'").run();day=getDispatchDay('2026-09-07',db);assert.throws(()=>reorderRouteStop('2026-09-07',1,{stopId:day.routeBoards[0].stops[1].id,direction:'up'},db),/protected/)})
