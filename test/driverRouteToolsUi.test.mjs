...
for(const language of ['en','ms','zh'])test(`${language}: date reason selection, other required and draft retained on error`,async()=>{
 ...
 assert.equal(document.querySelectorAll('select option').length,14)
 assert.equal(document.querySelector('option[value=business_closed]').textContent,{en:'Business closed',ms:'Berhenti business',zh:'不再营业'}[language])
 await set(document.querySelector('select'),'business_closed');assert.equal(submit().disabled,true);assert.equal(document.querySelector('textarea'),null)
