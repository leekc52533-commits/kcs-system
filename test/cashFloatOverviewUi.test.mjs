...
test('cash cards show arithmetic and suggested topup while settings sit inside More in three languages',async()=>{
 ...
 globalThis.fetch=async url=>({ok:true,json:async()=>String(url).includes('?date=')?{spending:{from:'2026-09-10',to:'2026-09-10',purchaseCents:12000,expenseCents:3000,voidCents:0,creditCents:0,totalCents:15000},items:[{employeeId:1,employeeName:'Test Employee',configured:true,targetFloatCents:50000,lowBalanceThresholdCents:20000,suggestedTopUpCents:25000,balanceCents:25000,day:{date:'2026-09-10',openingCents:30000,topUpCents:10000,purchaseCents:12000,expenseCents:3000,otherCents:0,closingCents:25000}}]}:{items:[]}})
 ...
 const card=document.querySelector('.cash-account-grid article');assert.equal(card.querySelector('.cash-line-total b').textContent,'RM 250.00');assert.equal(card.querySelectorAll('.cash-arithmetic .cash-line').length,6)
