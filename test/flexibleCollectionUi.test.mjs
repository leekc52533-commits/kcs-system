...
test('owner switches use live revision and translated labels; supervisors retain dispatch without toggle controls',async()=>{
 for(const language of ['zh','en','ms'])for(const canEdit of [true,false]){
  ...
  try{await act(async()=>root.render(React.createElement(I18nProvider,{language},React.createElement(Page))))
   assert.match(document.body.textContent,/Kuching MPKS/);assert.equal(document.body.textContent.includes('flex.'),false)
   ...
   if(canEdit){await act(async()=>document.querySelector('[role=switch]').click());assert.equal(document.querySelector('[role=switch]').getAttribute('aria-checked'),'true');assert.equal(calls.filter(c=>c[1].method==='PATCH').length,1)}
