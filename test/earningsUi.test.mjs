...
test('management report renders employees and opens rate settings with effective date and tiers',async()=>{const root=createRoot(document.getElementById('root'));globalThis.fetch=async url=>({ok:true,json:async()=>url.endsWith('/settings')?{revision:0,versions:[],defaults:{driver:[{from:0,rate:.03},{from:25000,rate:.04}],crewRate:.03}}:{...report,period:periodFor(url)}});try{await act(async()=>root.render(React.createElement(I18nProvider,{language:'zh'},React.createElement(Page))));assert.equal(document.querySelectorAll('table').length,2);assert.equal(document.querySelector('.earnings-page h1'),null);assert.equal(document.querySelectorAll('.earnings-navigation button').length,2);await act(async()=>[...document.querySelectorAll('button')].find(b=>b.textContent===words.zh.settings).click());assert.equal(document.querySelectorAll('table').length,0);assert.equal(document.querySelector('.earnings-toolbar'),null);assert.equal(document.querySelectorAll('.earnings-tier').length,2);assert.ok(document.body.textContent.includes(words.zh.effective));assert.ok(document.body.textContent.includes(words.zh.crewRate));await act(async()=>document.querySelector('.earnings-settings .earnings-back').click());assert.equal(document.querySelector('.earnings-settings'),null);assert.equal(document.querySelectorAll('table').length,2)}finally{await act(async()=>root.unmount())}})
