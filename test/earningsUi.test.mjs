...
test('management report renders employees and opens rate settings with effective date and tiers',async()=>{const root=createRoot(document.getElementById('root'));globalThis.fetch=async url=>({ok:true,json:async()=>url.endsWith('/settings')?{revision:0,versions:[],defaults:{driver:[{from:0,rate:.03},{from:25000,rate:.04}],crewRate:.03}}:{...report,period:periodFor(url)}});try{await act(async()=>root.render(React.createElement(I18nProvider,{language:'zh'},React.createElement(Page))));assert.equal(document.querySelectorAll('table').length,1);assert.equal(document.querySelector('.earnings-page h1'),null);assert.equal(document.querySelectorAll('.earnings-navigation button').length,2);await act(async()=>[...document.querySelectorAll('button')].find(b=>b.textContent===words.zh.settings).click());assert.equal(document.querySelectorAll('table').length,0);assert.equal(document.querySelector('.earnings-toolbar'),null);assert.equal(document.querySelectorAll('.earnings-tier').length,2);assert.ok(document.body.textContent.includes(words.zh.effective));assert.ok(document.body.textContent.includes(words.zh.crewRate));await act(async()=>document.querySelector('.earnings-settings .earnings-back').click());assert.equal(document.querySelector('.earnings-settings'),null);assert.equal(document.querySelectorAll('table').length,1)}finally{await act(async()=>root.unmount())}})
...
test('previous period is a compact button; dialog retains prior data and nested details, Escape restores focus',async()=>{
 for(const language of ['zh','en','ms'])for(const personal of [false,true]){
  const root=createRoot(document.getElementById('root'));let calls=0
  globalThis.fetch=async url=>{const prior=calls++%2===1;return{ok:true,json:async()=>({...report,period:{...periodFor(url),due:prior?'2026-09-20':'2026-10-05'},items:[{...report.items[0],name:prior?'Previous Employee':'Current Employee',crewKg:prior?5020:1080,amount:prior?150.6:32.4}]})}}
  try{
   await act(async()=>root.render(React.createElement(I18nProvider,{language},React.createElement(Page,{personal}))))
   assert.equal(document.querySelectorAll('.earnings-period').length,1)
   assert.ok(document.body.textContent.includes('Current Employee'));assert.ok(!document.body.textContent.includes('Previous Employee'))
   ...
   const detail=dialog.querySelector('dialog');assert.ok(detail);assert.match(detail.textContent,/TN-1/)
   await act(async()=>detail.dispatchEvent(new Event('cancel',{bubbles:true,cancelable:true})))
   assert.ok(document.querySelector('.earnings-previous-dialog'));assert.equal(dialog.querySelector('dialog'),null);assert.equal(document.activeElement,name)
   await act(async()=>dialog.dispatchEvent(new Event('cancel',{bubbles:true,cancelable:true})))
   assert.equal(document.querySelector('.earnings-previous-dialog'),null);assert.equal(document.querySelectorAll('.earnings-period').length,1);assert.equal(document.activeElement,trigger)
  }finally{await act(async()=>root.unmount())}
 }
})
