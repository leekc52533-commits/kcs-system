...
test('mobile UI keeps Weight direct, moves GPS and Temporary Customer into More, and spaces navigation clearly',()=>{const source=fs.readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8'),css=fs.readFileSync(new URL('../src/App.css',import.meta.url),'utf8');assert.match(source,/const tabs=\[\['today','mobile\.today'\].*\['weight','mobile\.weight'\].*\['more','mobile\.more'\]\]/);assert.doesNotMatch(source,/\['mine','mobile\.mine'\]/);assert.match(source,/mobile-more/);assert.match(source,/setTab\('gps'\)/);assert.match(source,/setTab\('new'\)/);assert.match(source,/function WeightView/);assert.match(source,/capture="environment"/);assert.match(source,/weightConfirm/);assert.match(css,/grid-auto-columns:1fr/);assert.match(css,/border-left:1px solid/)})
