...
function fixture(){
  ...
  db.prepare("INSERT INTO employees(id,employee_code,name,job_role) VALUES(9,'C009','CREW MEMBER','Attendant / Crew')").run();db.prepare('INSERT INTO dispatch_vehicle_assistants(dispatch_day_id,vehicle_id,employee_id) VALUES(1,1,9)').run()
...
test('one Excel workbook uses the requested order, split crew and car columns, compact dates, embedded photo and proof link',async()=>{const{db,root}=fixture(),file=await purchaseBillsWorkbook({from:'2026-09-02',to:'2026-09-03'},db,{uploadsRoot:root}),workbook=new ExcelJS.Workbook();await workbook.xlsx.load(file.buffer);const sheet=workbook.getWorksheet('Purchase Bills'),proofSheet=workbook.getWorksheet('Payment Proofs');assert.deepEqual(sheet.getRow(1).values.slice(1),['Date','PO No.','PaymentMethod','Customer Name','Branch','Item','Quantity','Price','Item Total','Total','Issued By','Crew','Car','Payment Gambar','Void No.']);assert.equal(sheet.getCell('A2').numFmt,'dd-mm-yy');assert.equal(sheet.getCell('B2').value,'P20260903-000002');assert.equal(sheet.getCell('J2').value,22);assert.equal(sheet.getCell('K2').value,'BOBOY');assert.equal(sheet.getCell('L2').value,'CREW MEMBER');assert.equal(sheet.getCell('M2').value,'QAA4293N');assert.equal(sheet.rowCount,4);assert.equal(proofSheet.getCell('A2').value,'P20260902-000001');assert.equal(proofSheet.getCell('C2').numFmt,'dd-mm-yy');assert.equal(proofSheet.getImages().length,1);assert.equal(sheet.getCell('N3').value.text,'View Payment Proof');assert.equal(sheet.getCell('N3').value.hyperlink,"#'Payment Proofs'!A2");assert.equal(file.proofCount,1);fs.rmSync(root,{recursive:true,force:true})})

test('admin UI uses DD-MM-YY, compact triangle filters, separate crew and car columns, and one combined download',()=>{const source=fs.readFileSync(new URL('../src/PurchaseBillsPage.jsx',import.meta.url),'utf8'),css=fs.readFileSync(new URL('../src/PurchaseBillsPage.css',import.meta.url),'utf8');assert.match(source,/From Date/);assert.match(source,/To Date/);assert.match(source,/displayDate/);assert.match(source,/\['crew','Crew'\]/);assert.match(source,/\['car','Car'\]/);assert.match(source,/<FilterHeader/);assert.match(source,/Download Excel with Payment Proofs/);assert.match(css,/compact-filter select\{[^}]*opacity:0/);assert.doesNotMatch(source,/Download Monthly Excel|Download Payment Proofs ZIP|Print Choice|Whether Printed|Daily Summary|Employee Summary/);assert.doesNotMatch(css,/archive-summary/)})

test('archive multi-column filters and sort also control Excel rows and embedded proofs',async()=>{
 const{db,root}=fixture()
 try{
 const query={month:'2026-09',columns:JSON.stringify({customerName:['ECO','HNL'],paymentMethod:['Cash']}),sortKey:'totalLabel',sortDirection:'asc'}
 const result=listPurchaseBillArchive(query,db);assert.deepEqual(result.items.map(x=>x.id),[1]);assert.ok(result.filterOptions.customerName.includes('HNL'))
 const file=await purchaseBillsWorkbook(query,db,{uploadsRoot:root}),book=new ExcelJS.Workbook();await book.xlsx.load(file.buffer)
 assert.equal(book.getWorksheet('Purchase Bills').rowCount,3);assert.equal(file.proofCount,1)
