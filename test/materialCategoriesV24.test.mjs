...
test('legacy Product Branch moves remain atomic while Customer-level membership stays authoritative',()=>{const{db,source,target}=fixture();applyV24Migration(db);const result=moveProductBranches(source,target,[1,2,3,4,5],{reason:'Approved test',changedBy:'Owner'},db);assert.equal(result.changedCount,5);assert.equal(db.prepare('SELECT COUNT(*) count FROM branch_product_price_assignments WHERE price_level_id=?').get(source).count,0);assert.equal(db.prepare('SELECT COUNT(*) count FROM branch_product_price_assignments WHERE price_level_id=?').get(target).count,5);assert.equal(getPriceGroup(source,db).customer_count,1);assert.equal(getPriceGroup(source,db).branch_count,5);assert.equal(getPriceGroup(target,db).customer_count,0);assert.equal(db.prepare("SELECT COUNT(*) count FROM branch_product_price_assignment_history WHERE action='move'").get().count,5);assert.throws(()=>moveProductBranches(source,target,[1],{reason:'stale',changedBy:'Owner'},db),/no longer belong/)})
