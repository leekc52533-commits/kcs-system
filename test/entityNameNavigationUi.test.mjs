...
test('customer and branch names are primary actions while the quick preview remains available',()=>{
  ...
  assert.match(master,/key:'branchName'.*className="entity-name-link".*openFromName/s)
  assert.match(master,/actor\.canManageMaster\)void setEditing\(item\);else table\.toggleDetails\(\)/)
