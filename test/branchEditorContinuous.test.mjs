...
test('Branch modal lifecycle prevents stale data and closes only after a successful save', () => {
  ...
  const editor = readFileSync(new URL('../src/BranchEditor.jsx', import.meta.url), 'utf8')
  assert.match(master, /key=\{`branch-\$\{editing\.branchId\|\|'new'\}`\}/)
  assert.match(master, /const detail=await api\(`\$\{endpoint\}\/\$\{encodeURIComponent\(item\[idKey\]\)\}`\)/)
  assert.match(master, /const saved=await api\(url,\{method,body:JSON\.stringify/)
  assert.match(master, /catch\(item\)\{fail\(item\.message\);return\{ok:false,error:item\.message\}\}/)
  assert.match(editor, /collectBranchOptionalFields\(data\)/)
  assert.match(editor, /if\(!result\?\.ok\).*return/)
  assert.match(editor, /setSaved\(true\);successTimer\.current=window\.setTimeout\(onClose,1400\)/)
  assert.match(editor, /name=\{key\}.*onChange=/)
})
