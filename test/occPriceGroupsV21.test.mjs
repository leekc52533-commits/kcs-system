...
test('custom OCC group can be added while fixed groups cannot be repriced through an update API',()=>{const db=database(),before=db.prepare('SELECT COUNT(*) n FROM occ_price_groups').get().n;for(const action of [()=>createOccPriceGroup({priceAmount:.7,reason:'Legacy'},db),()=>setOccPriceGroupStatus(1,'inactive',{reason:'Legacy'},db)])assert.throws(action,error=>error.statusCode===410&&error.code==='OCC_LEGACY_READ_ONLY');assert.equal(db.prepare('SELECT COUNT(*) n FROM occ_price_groups').get().n,before)})

test('selected Branches transfer between groups atomically with audit and deselection support',()=>{const db=database(),{source,target}=legacyAssignments(db),before=db.prepare('SELECT * FROM branch_occ_price_assignments').all();assert.throws(()=>assignBranchesToOccPriceGroup(target.id,[1],{reason:'No longer allowed'},db),/read-only/);assert.throws(()=>bulkTransferOccBranches(source.id,target.id,[1],{reason:'No longer allowed'},db),/read-only/);assert.deepEqual(db.prepare('SELECT * FROM branch_occ_price_assignments').all(),before);assert.equal(db.prepare('SELECT COUNT(*) n FROM branch_occ_price_assignment_history').get().n,0)})

test('five Branches move to an existing stable Group ID and counts refresh from the database',()=>{const db=database(),{source,target}=legacyAssignments(db),counts=listOccPriceGroups(db).items;assert.throws(()=>bulkTransferOccBranches(source.id,target.id,[1,2,3,4,5],{reason:'Forbidden'},db),/read-only/);assert.equal(listOccPriceGroups(db).items.find(x=>x.id===source.id).branchCount,1);assert.deepEqual(listOccPriceGroups(db).items.map(x=>x.id),counts.map(x=>x.id))})

test('v22 converted Branches move by stable OCC assignment without a legacy price-list row',()=>{const db=database(),{source,target}=legacyAssignments(db);db.prepare('DELETE FROM branch_material_prices').run();assert.throws(()=>bulkTransferOccBranches(source.id,target.id,[1],{reason:'Forbidden'},db),/read-only/);assert.equal(db.prepare('SELECT occ_price_group_id id FROM branch_occ_price_assignments WHERE branch_id=1').get().id,source.id)})

test('audit insertion failure rolls back every converted Branch assignment',()=>{const db=database(),{source,target}=legacyAssignments(db);db.exec("CREATE TRIGGER reject_occ_audit BEFORE INSERT ON branch_occ_price_assignment_history BEGIN SELECT RAISE(ABORT,'audit unavailable'); END");assert.throws(()=>bulkTransferOccBranches(source.id,target.id,[1],{reason:'Forbidden'},db),/read-only/);assert.equal(db.prepare('SELECT COUNT(*) n FROM branch_occ_price_assignment_history').get().n,0)})

test('bulk move validation returns stable codes and stale membership rolls back the entire batch',()=>{const db=database(),{source,target}=legacyAssignments(db);for(const ids of [[],[1]])assert.throws(()=>bulkTransferOccBranches(source.id,target.id,ids,{reason:'Any'},db),/read-only/);assert.equal(db.prepare('SELECT occ_price_group_id id FROM branch_occ_price_assignments WHERE branch_id=1').get().id,source.id)})

test('used group cannot be hidden and invalid source transfer rolls back',()=>{const db=database(),{source,target}=legacyAssignments(db);assert.throws(()=>setOccPriceGroupStatus(source.id,'inactive',{reason:'Hide'},db),/read-only/);assert.throws(()=>bulkTransferOccBranches(source.id,target.id,[1],{reason:'Forbidden'},db),/read-only/);assert.equal(db.prepare('SELECT occ_price_group_id id FROM branch_occ_price_assignments WHERE branch_id=1').get().id,source.id)})
