...
test('driver Purchase Bill UI is English-only, hides weight method and omits Company from receipt',()=>{const source=fs.readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8'),start=source.indexOf('function PurchaseReceipt'),end=source.indexOf('const stopMapUrl'),billing=source.slice(start,end);assert.match(billing,/ELECTRONIC PURCHASE BILL/);assert.match(billing,/Branch ID:/);assert.match(billing,/Paper option/);assert.doesNotMatch(billing,/Company:/);assert.doesNotMatch(billing,/purchase\./);assert.doesNotMatch(billing,/weightMethod,setWeightMethod|How weight was determined|重量取得方式/);assert.match(billing,/weightMethod:'on_site'/)})
