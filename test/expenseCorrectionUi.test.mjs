...
test('correction requires a reason, previews refund and sends original version only after confirmation',async()=>{
 ...
 globalThis.fetch=async(url,options={})=>{if(options.method==='POST'){posts++;body=JSON.parse(options.body);return{ok:true,json:async()=>({...current,amountCents:15000,revision:1,history:[{id:1,oldAmountCents:150000,newAmountCents:15000,reason:body.reason,actorName:'Kc',createdAt:'2026-09-14T00:00:00Z'}]})}}return{ok:true,json:async()=>current}}
 ...
 assert.equal(posts,1);assert.equal(saved,1);assert.equal(body.expectedAmountCents,150000);assert.equal(body.revision,0);assert.equal(body.amount,'150');assert.equal(document.querySelector('button[type="submit"]').disabled,true);assert.match(document.body.textContent,/Kc/)
