...
test('driver UI connects only implemented execution actions',()=>{const source=readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8');const view=source.slice(source.indexOf('function TodayView'));assert.match(view,/driver-stop/);assert.match(view,/estimatedWeightKg==null/);assert.match(view,/mobile\.startTrip/);assert.match(view,/mobile\.arrive/);assert.match(source,/PurchaseBillPanel/);assert.match(source,/purchase\.create/);assert.match(source,/purchase\.uploadProof/);assert.match(source,/window\.print/);assert.match(view,/No Goods/);assert.match(view,/Complete Trip/);assert.match(source,/\/bills/);assert.match(source,/\/payment-proof/);assert.match(view,/\/no-goods/);assert.match(view,/\/complete/);assert.match(source,/api\('\/api\/mobile\/today'\)/);assert.doesNotMatch(view,/Open Jodoo|\/invoice-confirm|Skip Stop/)})
