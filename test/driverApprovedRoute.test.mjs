...
test('driver UI connects only implemented execution actions',()=>{const source=readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8');const view=source.slice(source.indexOf('function TodayView'));assert.match(view,/driver-stop/);assert.match(view,/estimatedWeightKg==null/);assert.match(view,/mobile\.startTrip/);assert.match(view,/mobile\.arrive/);assert.match(view,/Open Jodoo/);assert.match(view,/Invoice completed/);assert.match(view,/Complete Stop/);assert.match(view,/No Goods/);assert.match(view,/Complete Trip/);assert.match(view,/\/invoice-confirm/);assert.match(view,/\/no-goods/);assert.match(view,/\/complete/);assert.match(source,/api\('\/api\/mobile\/today'\)/);assert.doesNotMatch(view,/Skip Stop/)})
