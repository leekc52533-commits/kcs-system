...
test('driver UI renders approved route as read-only details',()=>{const source=readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8');const view=source.slice(source.indexOf('function TodayView'));assert.match(view,/driver-stop/);assert.match(view,/estimatedWeightKg==null/);assert.doesNotMatch(view,/onClick/);assert.doesNotMatch(view,/<button/);assert.match(source,/api\('\/api\/mobile\/today'\)/)})
