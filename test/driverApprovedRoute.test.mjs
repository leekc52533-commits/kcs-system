...
test('assigned Draft and withdrawn plans remain visible without execution permission',()=>{const db=fixture();let view=driverToday({employeeId:1,role:'driver',today:date},db);assert.equal(view.routeAvailable,true);assert.equal(view.approved,false);assert.ok(view.trips.every(t=>!t.canStart));approved(db);db.prepare("UPDATE dispatch_days SET status='draft' WHERE dispatch_date=?").run(date);view=driverToday({employeeId:1,role:'driver',today:date},db);assert.ok(view.trips.length>0);assert.ok(view.trips.every(t=>!t.canStart));assert.ok(view.trips.flatMap(t=>t.stops).every(s=>!s.canArrive&&!s.canFinish))})
...
test('NULL weight and map coordinates remain visible without commercial pricing fields',()=>{const db=fixture();approved(db);const stops=driverToday({employeeId:1,role:'driver',today:date},db).trips.flatMap(item=>item.stops);assert.equal(stops[0].estimatedWeightKg,null);assert.equal(stops[0].gpsAvailable,false);assert.equal(stops[0].latitude,null);assert.equal(stops[1].gpsAvailable,true);assert.equal(stops[1].latitude,3.1);assert.equal(stops[1].longitude,101.6);for(const stop of stops){assert.equal('paymentType' in stop,false);assert.equal('occPrice' in stop,false)}})
...
test('driver UI uses a single open Stop, compact rows, expandable completed Bills and no-key Google Maps links',()=>{const source=readFileSync(new URL('../src/AuthPages.jsx',import.meta.url),'utf8'),view=source.slice(source.indexOf('function TodayView'));assert.match(view,/openStopId,setOpenStopId/);assert.match(view,/setOpenStopId\(current=>current===id\?null:id\)/);assert.doesNotMatch(view,/expandedStops|setExpandedStops/);assert.match(view,/collapsed-stop/);assert.match(view,/stop\.billCreated/);assert.match(view,/readOnly/);assert.match(source,/google\.com\/maps\/dir\/\?api=1&destination=/);assert.match(view,/target="_blank"/);assert.match(view,/mobile\.startTrip/);assert.match(view,/mobile\.arrive/);assert.match(source,/PurchaseBillPanel/);assert.match(source,/window\.print/);assert.match(view,/NoGoodsButton/);assert.match(view,/Complete Trip/);assert.match(source,/\/bills/);assert.match(source,/\/payment-proof/);assert.match(view,/no-goods-archive/);assert.match(view,/\/complete/);assert.match(source,/api\('\/api\/mobile\/today'\)/);assert.doesNotMatch(view,/Open Jodoo|\/invoice-confirm|Skip Stop/)})
