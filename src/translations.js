...
export const messages = {
  en: {
    ...
    'nav.workspace':'Workspace','nav.dashboard':'Dashboard','nav.dispatch':'Weekly Dispatch','nav.special':'Special Collection Requests','nav.buyers':'Buyer Management','nav.companySettings':'Company Settings',
  ...
  ms: {
    ...
    'branchLifecycle.stopWarning':'{stops} hentian dalam {dispatches} tugasan akan disemak. Jeda atau tutup membatalkan jadual automatik yang belum dibuat; rekod kerja dan aturan khas dikekalkan.','branchLifecycle.preserveHistory':'Jadual tetap, GPS, harga, bil, bayaran dan sejarah kutipan dikekalkan.','branchLifecycle.reviewTitle':'Semakan Cawangan Tidak Aktif','branchLifecycle.reviewHelp':'Semak Cawangan yang tidak beroperasi tanpa memadam sebarang sejarah.','branchLifecycle.operationalStatus':'Status Operasi','branchLifecycle.changeStatus':'Tukar Status','branchLifecycle.openReview':'Buka Semakan Cawangan Tidak Aktif','branchLifecycle.currentStatus':'Status Semasa','branchLifecycle.newStatus':'Status Baharu','branchLifecycle.reason':'Sebab','branchLifecycle.changedAt':'Tarikh Status Diubah','branchLifecycle.changedBy':'Diubah Oleh','branchLifecycle.replacedBy':'Digantikan Oleh Cawangan','branchLifecycle.replacementSearch':'Cari Cawangan pengganti','branchLifecycle.search':'Cari ID Cawangan, Cawangan, Pelanggan, Area atau alamat','branchLifecycle.allInactive':'Semua Tidak Aktif','branchLifecycle.withReplacement':'Dengan Pengganti','branchLifecycle.withoutReplacement':'Tanpa Pengganti','branchLifecycle.lastCollection':'Tarikh Kutipan Terakhir','branchLifecycle.noCollection':'Tiada kutipan selesai','branchLifecycle.viewEdit':'Lihat / Edit Cawangan','branchLifecycle.restoreActive':'Pulihkan Aktif','branchLifecycle.restoreReason':'Sebab pemulihan','branchLifecycle.empty':'Tiada Cawangan sepadan untuk semakan ini.','branchLifecycle.status.ACTIVE':'Aktif','branchLifecycle.status.TEMPORARILY_PAUSED':'Kutipan Dijeda','branchLifecycle.status.CLOSED':'Tutup / Tidak Lagi Beroperasi','branchLifecycle.status.DUPLICATE_REPLACED':'Pendua / Digantikan','branchLifecycle.status.NOT_COLLECTING':'Tidak Mengutip / Belum Pernah Bermula','branchLifecycle.status.TEST_INVALID':'Ujian / Rekod Tidak Sah',
    ...
    'nav.workspace':'Ruang kerja','nav.dashboard':'Ringkasan','nav.dispatch':'Penghantaran Mingguan','nav.special':'Permintaan Kutipan Khas','nav.buyers':'Pengurusan Buyer','nav.companySettings':'Tetapan Syarikat',
...
const moduleMessages={
  en:{
    ...
    'list.selected':'{count} selected','list.selectAll':'Select all visible rows','list.selectRow':'Select row','list.sortAsc':'Sort ascending','list.sortDesc':'Sort descending','list.clearSort':'Clear sort','list.clearFilter':'Clear filter','list.actions':'Actions','list.details':'Details','list.expand':'Expand details','list.collapse':'Collapse details','list.copy':'Copy','list.copied':'Copied','list.columns':'Columns','list.closeColumns':'Close column chooser','list.resetColumns':'Reset Columns','list.noResults':'No records match the current search or filters.','list.activeOnly':'Active only','list.includeInactive':'Include non-operating Branches','list.customerId':'Customer ID','list.customerName':'Customer Name','list.branchId':'Branch ID','list.branchName':'Branch Name','list.status':'Status','apiError.date_evidence_required':'Complete the required evidence and confirm it has been checked before approval.','list.branchCount':'Operating Branches','list.phone':'Phone','list.payment':'Payment','list.customer':'Customer','list.area':'Area','list.lifecycleStatus':'Branch Status','list.gpsStatus':'GPS Status','list.materialsCount':'Materials Count','list.contact':'Contact Person','list.address':'Address','list.notes':'Notes','list.updatedAt':'Updated Date','list.employeeId':'Employee ID','list.employeeName':'Employee Name','list.mainJobRole':'Main Job Role','list.accountStatus':'Account Status','list.permissions':'Permissions','list.createdAt':'Created Date','list.lastLogin':'Last Login','list.audit':'Audit Summary','list.failedLogins':'failed logins','list.searchAccounts':'Search Employee ID, employee, username or role','branchEditor.unsavedLeave':'There are unsaved changes. Are you sure you want to leave?',
...
export function translate(language,key,variables={}){
  if(key==='earnings.title')return (earningsWords[language]||earningsWords.en).title
...
Object.assign(messages.en,{"intake.title": "Temporary customers", "intake.help": "Enter customer details and GPS once, then choose on-site billing or a collection request.", "intake.action": "Action", "intake.collectNow": "Collect and issue bill now", "intake.requestOnly": "Request collection arrangement", "intake.extra": "More details (optional)"})

Object.assign(messages.ms,{"intake.title": "Pelanggan sementara", "intake.help": "Isi butiran pelanggan dan GPS sekali, kemudian pilih bil di lokasi atau permintaan kutipan.", "intake.action": "Tindakan", "intake.collectNow": "Kutip dan keluarkan bil sekarang", "intake.requestOnly": "Mohon aturan kutipan", "intake.extra": "Butiran lanjut (pilihan)"})

Object.assign(messages.zh,{"intake.title": "临时客户", "intake.help": "客户资料和 GPS 只填一次，再选择现场开单或提交收货请求。", "intake.action": "处理方式", "intake.collectNow": "现场收货并开单", "intake.requestOnly": "申请安排收货", "intake.extra": "更多资料（选填）"})

Object.assign(messages.en,{
  "intake.existing": "Existing customer",
  "pickup.searchHelp": "Type part of a company or branch name, code or phone number. Select the matching existing customer.",
  "pickup.searching": "Searching…",
  "pickup.matches": "Matching existing customers",
  "pickup.noMatch": "No matching active customer. Check the name before creating a temporary customer.",
  "pickup.existing": "Existing customer selected",
  "pickup.masterDetails": "Original details, GPS and prices are used. Collection still requires live arrival GPS. Fixed schedules stay unchanged.",
  "pickup.ownVehicle": "Already on your vehicle today",
  "pickup.otherVehicle": "Assigned to another vehicle today",
  "pickup.unscheduled": "Not scheduled today — one-time collection available",
  "pickup.protected": "Work recorded; transfer unavailable",
  "pickup.stop.locked": "Not started",
  "pickup.stop.available": "Ready",
  "pickup.stop.active": "In progress",
  "pickup.stop.completed": "Completed",
  "pickup.stop.overridden": "Skipped / changed",
  "pickup.payment.cash": "Cash — payment proof required",
  "pickup.payment.credit": "Credit — use original account",
  "pickup.reason": "Reason for transfer",
  "pickup.requestTransfer": "Request vehicle transfer",
  "pickup.collectExisting": "Open existing customer collection",
  "pickup.submitted": "Transfer requested. The customer stays on the original vehicle until a supervisor approves.",
  "pickup.myTransfers": "My transfer requests",
  "pickup.pending": "Awaiting supervisor approval",
  "pickup.approved": "Transfer approved",
  "pickup.rejected": "Transfer rejected",
  "pickup.open": "Open collection",
  "pickup.requester": "Requested by",
  "pickup.expired": "This request is no longer for today. Reject it and ask the employee to submit a new request.",
  "pickup.approve": "Approve transfer",
  "pickup.reject": "Reject transfer",
  "pickup.reviewTitle": "Customer vehicle transfers",
  "pickup.reviewHelp": "Approval moves this collection only. If the original vehicle has arrived or recorded work, transfer is blocked.",
  "apiError.pickup_branch": "This customer is unavailable. Search again.",
  "apiError.pickup_own_trip": "This customer is on another trip of your vehicle. Open the original trip.",
  "apiError.pickup_protected": "The original stop has started or has records. It cannot be transferred or duplicated.",
  "apiError.pickup_reason": "Enter a reason (up to 1,000 characters).",
  "apiError.pickup_price": "Ask the office to complete the customer payment method and product prices first.",
  "apiError.pickup_gps": "This customer has no valid GPS. Ask the office to update the location, or submit a collection request.",
  "apiError.pickup_pending": "Another transfer request is already pending for this customer.",
  "apiError.pickup_stale": "The date, assignment or request has changed. Refresh and check before continuing."
})

Object.assign(messages.ms,{
  "intake.existing": "Pelanggan sedia ada",
  "pickup.searchHelp": "Taip sebahagian nama syarikat atau cawangan, kod atau telefon. Pilih pelanggan sedia ada yang sepadan.",
  "pickup.searching": "Mencari…",
  "pickup.matches": "Pelanggan sedia ada yang sepadan",
  "pickup.noMatch": "Tiada pelanggan aktif sepadan. Semak nama sebelum mencipta pelanggan sementara.",
  "pickup.existing": "Pelanggan sedia ada dipilih",
  "pickup.masterDetails": "Butiran, GPS dan harga asal digunakan. Kutipan masih memerlukan GPS ketibaan langsung. Jadual tetap tidak berubah.",
  "pickup.ownVehicle": "Sudah di kenderaan anda hari ini",
  "pickup.otherVehicle": "Ditugaskan ke kenderaan lain hari ini",
  "pickup.unscheduled": "Tidak dijadualkan hari ini — kutipan sekali sahaja tersedia",
  "pickup.protected": "Kerja telah direkodkan; tidak boleh dipindah",
  "pickup.stop.locked": "Belum bermula",
  "pickup.stop.available": "Sedia",
  "pickup.stop.active": "Sedang berjalan",
  "pickup.stop.completed": "Selesai",
  "pickup.stop.overridden": "Dilangkau / diubah",
  "pickup.payment.cash": "Tunai — bukti bayaran diperlukan",
  "pickup.payment.credit": "Kredit — guna akaun asal",
  "pickup.reason": "Sebab pemindahan",
  "pickup.requestTransfer": "Mohon pindah kenderaan",
  "pickup.collectExisting": "Buka kutipan pelanggan sedia ada",
  "pickup.submitted": "Permohonan dihantar. Pelanggan kekal di kenderaan asal sehingga diluluskan penyelia.",
  "pickup.myTransfers": "Permohonan pemindahan saya",
  "pickup.pending": "Menunggu kelulusan penyelia",
  "pickup.approved": "Pemindahan diluluskan",
  "pickup.rejected": "Pemindahan ditolak",
  "pickup.open": "Buka kutipan",
  "pickup.requester": "Pemohon",
  "pickup.expired": "Permohonan ini bukan lagi untuk hari ini. Tolak dan minta pekerja menghantar permohonan baharu.",
  "pickup.approve": "Luluskan pemindahan",
  "pickup.reject": "Tolak pemindahan",
  "pickup.reviewTitle": "Pemindahan kenderaan pelanggan",
  "pickup.reviewHelp": "Kelulusan memindahkan kutipan ini sahaja. Jika kenderaan asal sudah tiba atau merekodkan kerja, pemindahan disekat.",
  "apiError.pickup_branch": "Pelanggan ini tidak tersedia. Cari semula.",
  "apiError.pickup_own_trip": "Pelanggan ini dalam perjalanan lain kenderaan anda. Buka perjalanan asal.",
  "apiError.pickup_protected": "Hentian asal telah bermula atau mempunyai rekod. Tidak boleh dipindah atau diduplikasi.",
  "apiError.pickup_reason": "Masukkan sebab (sehingga 1,000 aksara).",
  "apiError.pickup_price": "Minta pejabat melengkapkan kaedah bayaran dan harga produk pelanggan dahulu.",
  "apiError.pickup_gps": "Pelanggan ini tiada GPS sah. Minta pejabat mengemas kini lokasi atau hantar permohonan kutipan.",
  "apiError.pickup_pending": "Permohonan pemindahan lain sedang menunggu untuk pelanggan ini.",
  "apiError.pickup_stale": "Tarikh, tugasan atau permohonan telah berubah. Muat semula dan semak sebelum meneruskan."
})

Object.assign(messages.zh,{
  "intake.existing": "旧顾客",
  "pickup.searchHelp": "输入公司／分店名称、编号或电话的一部分，选择相符的旧顾客。",
  "pickup.searching": "正在查找…",
  "pickup.matches": "相符的旧顾客",
  "pickup.noMatch": "没有找到相符的有效顾客，请检查名称后再建立临时客户。",
  "pickup.existing": "已选择旧顾客",
  "pickup.masterDetails": "使用原有资料、GPS 和价格；到店时仍需现场定位验证，固定收货安排保持不变。",
  "pickup.ownVehicle": "今天已安排在本车",
  "pickup.otherVehicle": "今天已安排在别车",
  "pickup.unscheduled": "今天未安排，可一次性收货",
  "pickup.protected": "已有执行记录，不能转车",
  "pickup.stop.locked": "未开始",
  "pickup.stop.available": "待收货",
  "pickup.stop.active": "收货中",
  "pickup.stop.completed": "已完成",
  "pickup.stop.overridden": "已跳过／调整",
  "pickup.payment.cash": "现金：需要付款证明",
  "pickup.payment.credit": "赊账：使用原有户口",
  "pickup.reason": "转车申请原因",
  "pickup.requestTransfer": "申请转车",
  "pickup.collectExisting": "进入旧顾客收货",
  "pickup.submitted": "转车申请已提交；主管批准前，顾客仍留在原车。",
  "pickup.myTransfers": "我的转车申请",
  "pickup.pending": "等待主管批准",
  "pickup.approved": "转车已批准",
  "pickup.rejected": "转车已拒绝",
  "pickup.open": "进入收货",
  "pickup.requester": "申请员工",
  "pickup.expired": "申请日期已过，不能批准；请拒绝并让员工重新申请。",
  "pickup.approve": "批准转车",
  "pickup.reject": "拒绝转车",
  "pickup.reviewTitle": "顾客转车审批",
  "pickup.reviewHelp": "批准只调整本次收货；原车若已到店或有执行记录，系统会阻止转车。",
  "apiError.pickup_branch": "此顾客已不可用，请重新搜索。",
  "apiError.pickup_own_trip": "顾客在本车的另一趟行程，请从原行程进入。",
  "apiError.pickup_protected": "原收货任务已开始或已有记录，不能转车或重复建立。",
  "apiError.pickup_reason": "请填写原因，最多 1000 字。",
  "apiError.pickup_price": "请先由办公室补齐顾客付款方式和有效产品价格。",
  "apiError.pickup_gps": "顾客尚无有效 GPS，请办公室补齐定位，或先提交收货申请。",
...
Object.assign(messages.en,{
  ...
  "notice.manage": "Employee Notice Board",
  "notice.new": "New notice",
  "notice.acknowledge": "I have read this",
  ...
  "notice.loadFailed": "Unable to refresh notices. Please check your connection and try again.",
  "notice.receipts": "View read status",
  "notice.hideReceipts": "Hide read status",
  "notice.readMeaning": "Read means the employee pressed “I have read this”.",
  "notice.help": "Publish a notice to all current active employees or selected employees. Unread notices appear when they open or use the mobile page; history stays in More.",
  "notice.moreHelp": "View all notices sent to you and your read history.",
  "notice.heading": "Notice title",
  "notice.content": "Message",
  "notice.priority": "Priority",
  "notice.audience": "Recipients",
  "notice.all": "All current active employees",
  "notice.selected": "Selected employees",
  "notice.findEmployee": "Find employee",
  "notice.recipientCount": "Recipient count",
  "notice.preview": "Preview notice",
  "notice.publishHelp": "Check the message and recipients before publishing. Published notices and read records are retained.",
  "notice.publish": "Publish notice",
  "notice.editDraft": "Back to edit",
  "notice.published": "Notice published. Employees will see it when they open or use the mobile page.",
  "notice.history": "Published notices",
  "apiError.notice_access": "You cannot access or publish this notice.",
  "apiError.notice_fields": "Enter a title of up to 120 characters and a message of up to 5,000 characters.",
  "apiError.notice_recipients": "Select active employees. The employee list may have changed; refresh and check again.",
  "apiError.notice_retry": "This submission differs from the original. Return to edit and submit again.",
  "apiError.notice_missing": "This notice was not found."
})

Object.assign(messages.ms,{
  "notice.title": "Papan Notis",
  "notice.manage": "Papan Notis Pekerja",
  "notice.new": "Notis baharu",
  "notice.acknowledge": "Saya sudah membaca",
  "notice.normal": "Biasa",
  "notice.urgent": "Segera",
  "notice.read": "Sudah dibaca",
  "notice.unread": "Belum dibaca",
  "notice.refresh": "Muat semula",
  "notice.search": "Cari notis",
  "notice.empty": "Belum ada notis.",
  "notice.loadFailed": "Tidak dapat memuat semula notis. Semak sambungan dan cuba lagi.",
  "notice.receipts": "Lihat status bacaan",
  "notice.hideReceipts": "Sembunyikan status bacaan",
  "notice.readMeaning": "Sudah dibaca bermaksud pekerja menekan “Saya sudah membaca”.",
  "notice.help": "Terbitkan notis kepada semua pekerja aktif semasa atau pekerja terpilih. Notis belum dibaca muncul apabila mereka membuka atau menggunakan halaman mudah alih; sejarah disimpan dalam Lagi.",
  "notice.moreHelp": "Lihat semua notis untuk anda dan sejarah bacaan.",
  "notice.heading": "Tajuk notis",
  "notice.content": "Kandungan",
  "notice.priority": "Keutamaan",
  "notice.audience": "Penerima",
  "notice.all": "Semua pekerja aktif semasa",
  "notice.selected": "Pekerja terpilih",
  "notice.findEmployee": "Cari pekerja",
  "notice.recipientCount": "Bilangan penerima",
  "notice.preview": "Pratonton notis",
  "notice.publishHelp": "Semak kandungan dan penerima sebelum menerbitkan. Notis terbitan dan rekod bacaan disimpan.",
  "notice.publish": "Terbitkan notis",
  "notice.editDraft": "Kembali menyunting",
  "notice.published": "Notis diterbitkan. Pekerja akan melihatnya apabila membuka atau menggunakan halaman mudah alih.",
  "notice.history": "Notis diterbitkan",
  "apiError.notice_access": "Anda tidak boleh mengakses atau menerbitkan notis ini.",
  "apiError.notice_fields": "Masukkan tajuk sehingga 120 aksara dan kandungan sehingga 5,000 aksara.",
  "apiError.notice_recipients": "Pilih pekerja aktif. Senarai pekerja mungkin berubah; muat semula dan semak lagi.",
  "apiError.notice_retry": "Penghantaran ini berbeza daripada asal. Kembali menyunting dan hantar semula.",
  "apiError.notice_missing": "Notis ini tidak ditemui."
})

Object.assign(messages.zh,{
  "notice.title": "通知公告栏",
  "notice.manage": "员工通知公告栏",
  "notice.new": "新通知",
  "notice.acknowledge": "我已阅读",
  "notice.normal": "普通通知",
  "notice.urgent": "紧急通知",
  "notice.read": "已读",
  "notice.unread": "未读",
  ...
  "notice.search": "搜索通知",
  "notice.empty": "暂无通知。",
  "notice.loadFailed": "暂时无法更新通知，请检查网络后重试。",
  "notice.receipts": "查看已读／未读员工",
  "notice.hideReceipts": "收起阅读状态",
  "notice.readMeaning": "“已读”表示员工亲自按下了“我已阅读”。",
  "notice.help": "可通知当前全体在职员工或指定员工。员工打开或使用手机页面时会弹出未读通知，历史保留在 More。",
  "notice.moreHelp": "查看发给你的全部通知及阅读记录。",
  "notice.heading": "通知标题",
  "notice.content": "通知内容",
  "notice.priority": "通知级别",
  "notice.audience": "接收员工",
  "notice.all": "当前全体在职员工",
  "notice.selected": "指定员工",
  "notice.findEmployee": "搜索员工",
  "notice.recipientCount": "接收人数",
  "notice.preview": "预览通知",
  "notice.publishHelp": "请核对内容和接收员工后发布，已发布通知和阅读记录将保留。",
  "notice.publish": "发布通知",
  "notice.editDraft": "返回修改",
  "notice.published": "通知已发布，员工打开或使用手机页面时即可看到。",
  "notice.history": "已发布通知",
  "apiError.notice_access": "你无权查看、确认或发布此通知。",
  "apiError.notice_fields": "请填写通知标题（最多 120 字）及内容（最多 5000 字）。",
  "apiError.notice_recipients": "请选择在职员工；员工名单可能已改变，请刷新后重新核对。",
...
Object.assign(messages.en,{
  ...
  "guide.welcome": "Driver guide — please read before starting",
  "guide.where": "Keep this guide: More → User Guide. You can open it again at any time.",
  "guide.effective": "These approval rules apply from 14 September 2026, 00:00 Malaysia time.",
  "guide.workflow": "Your daily collection steps",
  "guide.step.route": "Check Today’s Route",
  "guide.step.route.help": "Check the date, assigned vehicle and customers. Tomorrow is a preview; execute today’s assigned route.",
  "guide.step.start": "Start your trip",
  "guide.step.start.help": "Start your assigned, approved trip before collecting. If approval is pending, wait for your supervisor.",
  "guide.step.arrive": "Verify arrival at the customer",
  "guide.step.arrive.help": "Open the current customer and confirm arrival while on site. Allow location access and wait for GPS verification. Reuse the existing customer record; do not create another account.",
  "guide.step.bill": "Weigh and issue the bill",
  "guide.step.bill.help": "Choose the weighing method, enter actual weight and check customer, goods, price and amount before issuing the bill.",
  "guide.step.proof": "Attach payment proof when required",
  "guide.step.proof.help": "Cash bills require payment proof. Credit bills do not require Cash payment proof. For photos, capture or choose from the gallery, check the preview and submit.",
  "guide.step.finish": "Complete the customer, then the trip",
  "guide.step.finish.help": "After the bill and required proof are saved, complete this customer and proceed to the next. Finish the trip when all customers have been handled.",
  "guide.rules": "When do you need approval?",
  "guide.rule.order": "Change customer order: approval first",
  "guide.rule.order.help": "Submit the requested order and reason. Until approved, follow the original order. Submitting a request does not change the route.",
  "guide.rule.advance": "No Goods before arrival: wait for approval",
  "guide.rule.advance.help": "Record the contact method, reason and proof. Submit for supervisor review; the customer stays on your route until approved.",
  "guide.rule.onsite": "Verified arrival + No Goods proof: skip directly",
  "guide.rule.onsite.help": "After the system verifies your arrival, submit the No Goods reason and proof. Selecting “on site” alone is not arrival verification.",
  "guide.rule.temporary": "Temporary customer collection remains available",
  "guide.rule.temporary.help": "Open More → Temporary Customer during your running trip. Search the name first: select an existing customer if found; otherwise enter the new customer name and GPS. The supervisor reviews the temporary customer after billing.",
  "guide.rule.transfer": "Another vehicle’s customer: request transfer",
  "guide.rule.transfer.help": "Request a transfer and wait for supervisor approval. A stop already arrived at, billed or completed cannot be transferred.",
  "guide.rule.return": "Return later or change date: request approval",
  "guide.rule.return.help": "Give the reason and requested time or date. Wait for approval; do not treat a pending request as a completed change.",
  "guide.blocked": "If you cannot continue",
  "guide.help.noRoute": "No route? Check today’s date and your login, then ask the office to check your vehicle and assignment.",
  "guide.help.noTrip": "No trip in Temporary Customer? Start your assigned trip in Today’s Route. A completed trip cannot be used for new collections.",
  "guide.help.gps": "GPS failed? Enable location permission and retry at the customer’s site. If it still fails, contact your supervisor.",
  "guide.help.waiting": "Pending approval means no change yet. Follow the original arrangement and check the decision; avoid duplicate requests.",
  "guide.help.role": "Use the controls available to your role. Crew without execution controls should ask the assigned driver or supervisor.",
  "guide.understood": "I have read this — continue",
  "guide.moreHelp": "Daily steps, approval rules and troubleshooting",
  "guide.next": "Next step:",
  "guide.next.preview": "Tomorrow’s plan is a preview. Use Today’s Route to collect.",
  "guide.next.done": "This trip is complete.",
  "guide.next.approval": "Wait for supervisor departure approval.",
  "guide.next.waiting": "A return-later request is awaiting approval. Check its decision before continuing.",
  "guide.next.start": "Start this trip.",
  "guide.next.earlier": "Check the preceding trip and its completion status.",
  "guide.next.finishTrip": "All customers are handled. Finish this trip.",
  "guide.next.refresh": "Refresh the route; contact the office if no current customer appears.",
  "guide.next.arrive": "Go to the current customer and verify arrival on site.",
  "guide.next.bill": "Enter the actual weight and issue the collection bill.",
  "guide.next.proof": "Upload the Cash payment proof.",
  "guide.next.finish": "Check the saved bill and proof, then complete this customer.",
  "apiError.guide_stale": "Please refresh the guide and try again after it becomes active."
})

Object.assign(messages.ms,{
  "guide.title": "Panduan Penggunaan",
  "guide.welcome": "Panduan pemandu — baca sebelum bermula",
  "guide.where": "Panduan ini kekal di More → Panduan Penggunaan. Boleh dibuka semula pada bila-bila masa.",
  "guide.effective": "Peraturan kelulusan ini berkuat kuasa pada 14 September 2026, 00:00 waktu Malaysia.",
  "guide.workflow": "Langkah kutipan harian",
  "guide.step.route": "Semak Laluan Hari Ini",
  "guide.step.route.help": "Semak tarikh, kenderaan dan pelanggan yang ditugaskan. Esok ialah pratonton; jalankan laluan hari ini.",
  "guide.step.start": "Mulakan perjalanan",
  "guide.step.start.help": "Mulakan perjalanan yang ditugaskan dan diluluskan sebelum mengutip. Jika belum diluluskan, tunggu penyelia.",
  "guide.step.arrive": "Sahkan ketibaan di pelanggan",
  "guide.step.arrive.help": "Buka pelanggan semasa dan sahkan ketibaan di lokasi. Benarkan akses lokasi dan tunggu pengesahan GPS. Gunakan rekod pelanggan sedia ada; jangan cipta akaun pendua.",
  "guide.step.bill": "Timbang dan keluarkan bil",
  "guide.step.bill.help": "Pilih kaedah timbang, masukkan berat sebenar dan semak pelanggan, barang, harga serta jumlah sebelum mengeluarkan bil.",
  "guide.step.proof": "Lampirkan bukti bayaran jika diperlukan",
  "guide.step.proof.help": "Bil tunai memerlukan bukti bayaran. Bil kredit tidak memerlukan bukti bayaran tunai. Ambil foto atau pilih daripada galeri, semak pratonton dan hantar.",
  "guide.step.finish": "Selesaikan pelanggan, kemudian perjalanan",
  "guide.step.finish.help": "Selepas bil dan bukti wajib disimpan, selesaikan pelanggan ini dan teruskan ke pelanggan seterusnya. Tamatkan perjalanan selepas semua pelanggan diuruskan.",
  "guide.rules": "Bila kelulusan diperlukan?",
  "guide.rule.order": "Ubah urutan pelanggan: kelulusan dahulu",
  "guide.rule.order.help": "Hantar urutan yang diminta dan sebab. Ikut urutan asal sehingga diluluskan. Menghantar permohonan tidak terus mengubah laluan.",
  "guide.rule.advance": "Tiada barang sebelum tiba: tunggu kelulusan",
  "guide.rule.advance.help": "Catat kaedah hubungan, sebab dan bukti. Hantar untuk semakan penyelia; pelanggan kekal dalam laluan sehingga diluluskan.",
  "guide.rule.onsite": "Ketibaan disahkan + bukti tiada barang: boleh langkau",
  "guide.rule.onsite.help": "Selepas sistem mengesahkan ketibaan, hantar sebab dan bukti tiada barang. Memilih “di lokasi” sahaja bukan pengesahan ketibaan.",
  "guide.rule.temporary": "Kutipan pelanggan sementara masih tersedia",
  "guide.rule.temporary.help": "Buka More → Pelanggan Sementara semasa perjalanan sedang berjalan. Cari nama dahulu: pilih pelanggan sedia ada jika ditemui; jika tiada, isi nama dan GPS pelanggan baharu. Penyelia menyemak pelanggan sementara selepas bil dikeluarkan.",
  "guide.rule.transfer": "Pelanggan kenderaan lain: mohon pemindahan",
  "guide.rule.transfer.help": "Mohon pemindahan dan tunggu kelulusan penyelia. Hentian yang sudah tiba, berbil atau selesai tidak boleh dipindahkan.",
  "guide.rule.return": "Kembali kemudian atau ubah tarikh: mohon kelulusan",
  "guide.rule.return.help": "Berikan sebab dan masa atau tarikh yang diminta. Tunggu kelulusan; permohonan tertangguh belum bermaksud perubahan selesai.",
  "guide.blocked": "Jika tidak dapat meneruskan",
  "guide.help.noRoute": "Tiada laluan? Semak tarikh hari ini dan akaun anda, kemudian minta pejabat menyemak kenderaan serta tugasan.",
  "guide.help.noTrip": "Tiada perjalanan dalam Pelanggan Sementara? Mulakan perjalanan yang ditugaskan di Laluan Hari Ini. Perjalanan selesai tidak boleh digunakan untuk kutipan baharu.",
  "guide.help.gps": "GPS gagal? Benarkan akses lokasi dan cuba semula di lokasi pelanggan. Jika masih gagal, hubungi penyelia.",
  "guide.help.waiting": "Menunggu kelulusan bermaksud belum berubah. Ikut aturan asal dan semak keputusan; elakkan permohonan pendua.",
  "guide.help.role": "Gunakan kawalan yang tersedia untuk peranan anda. Kru tanpa kawalan pelaksanaan perlu merujuk pemandu yang ditugaskan atau penyelia.",
  "guide.understood": "Saya telah membaca — teruskan",
  "guide.moreHelp": "Langkah harian, kelulusan dan bantuan",
  "guide.next": "Langkah seterusnya:",
  "guide.next.preview": "Pelan esok ialah pratonton. Gunakan Laluan Hari Ini untuk kutipan.",
  "guide.next.done": "Perjalanan ini selesai.",
  "guide.next.approval": "Tunggu kelulusan pelepasan daripada penyelia.",
  "guide.next.waiting": "Permohonan kembali kemudian menunggu kelulusan. Semak keputusan sebelum meneruskan.",
  "guide.next.start": "Mulakan perjalanan ini.",
  "guide.next.earlier": "Semak perjalanan sebelumnya dan status penyelesaiannya.",
  "guide.next.finishTrip": "Semua pelanggan telah diuruskan. Tamatkan perjalanan ini.",
  "guide.next.refresh": "Muat semula laluan; hubungi pejabat jika pelanggan semasa tidak muncul.",
  "guide.next.arrive": "Pergi ke pelanggan semasa dan sahkan ketibaan di lokasi.",
  "guide.next.bill": "Masukkan berat sebenar dan keluarkan bil kutipan.",
  "guide.next.proof": "Muat naik bukti bayaran tunai.",
  "guide.next.finish": "Semak bil dan bukti yang disimpan, kemudian selesaikan pelanggan ini.",
  "apiError.guide_stale": "Muat semula panduan dan cuba lagi selepas ia berkuat kuasa."
})

Object.assign(messages.zh,{
  "guide.title": "使用指南",
  "guide.welcome": "司机操作指引：出发前请阅读",
  "guide.where": "本指引长期保留在「更多 More → 使用指南」，以后随时可以回来查看。",
  "guide.effective": "以下审批规则从马来西亚时间 2026 年 9 月 14 日凌晨 00:00 开始执行。",
  "guide.workflow": "每天怎样开始收货",
  "guide.step.route": "查看今日路线",
  "guide.step.route.help": "核对日期、分配给自己的车辆和顾客。明天的路线仅供预览，实际收货请使用今日路线。",
  "guide.step.start": "开始执行趟次",
  "guide.step.start.help": "先开始已分配、已批准的趟次，再收货。尚未批准时，等待主管处理。",
  "guide.step.arrive": "到店后确认到达",
  "guide.step.arrive.help": "到达当前顾客地点后，打开该顾客并确认到达。允许手机定位，等待系统验证。旧顾客沿用原有资料，不要重复开户。",
  "guide.step.bill": "称重并开收货单",
  "guide.step.bill.help": "选择称重方式，填写实际重量，核对顾客、货物、价格和金额，再开收货单。",
  "guide.step.proof": "按付款方式提交证明",
  "guide.step.proof.help": "现金单需上传付款证明，赊账单不需现金付款证明。照片可现场拍摄或从相册选择，检查预览后提交。",
  "guide.step.finish": "完成顾客，再结束趟次",
  "guide.step.finish.help": "单据及必需证明保存后，完成当前顾客，再继续下一家。所有顾客处理完后，结束该趟次。",
  "guide.rules": "什么情况需要主管批准",
  "guide.rule.order": "修改顾客顺序：先申请，批准后才生效",
  "guide.rule.order.help": "提交想调整的顺序和原因。等待期间仍按原顺序执行；提交申请不代表已经修改成功。",
  "guide.rule.advance": "还没到店就报告无货：须主管批准才跳过",
  "guide.rule.advance.help": "填写联系方法、无货原因并附上证明，提交主管审核。批准前，该顾客仍保留在原路线中。",
  "guide.rule.onsite": "系统已验证到店：附上无货证明即可跳过",
  "guide.rule.onsite.help": "先由系统验证确实到店，再填写无货原因并提交证明，即可跳过。单纯选择“现场”不等于已通过到店验证。",
  "guide.rule.temporary": "临时增加顾客收货：保留",
  "guide.rule.temporary.help": "在已开始执行的趟次中，打开「More → 临时客户」。先输入名字查找：旧顾客直接选用；确实是新顾客才填写名字及 GPS。临时顾客开单后，由主管审核是否转为正式或一次性户口。",
  "guide.rule.transfer": "其他车辆的顾客：申请转车",
  "guide.rule.transfer.help": "提出转车申请，主管批准后才能转到自己的车辆。已经到达、开单或完成的顾客不能转车。",
  "guide.rule.return": "稍后返回或改日期：提交审批",
  "guide.rule.return.help": "填写原因及希望返回的时间或收货日期，等待主管批准。待审批不代表已更改。",
  "guide.blocked": "遇到不能操作时",
  "guide.help.noRoute": "没有路线：先核对今日日期和登录员工，再请办公室检查车辆及人员分配。",
  "guide.help.noTrip": "临时客户没有可选趟次：先在今日路线开始自己的趟次。已结束的趟次不能继续新增收货。",
  "guide.help.gps": "定位失败：开启手机定位及浏览器定位权限，在顾客现场重试；仍失败请联系主管。",
  "guide.help.waiting": "待主管批准：原安排暂时不变。查看审批结果，不要重复提交同一个申请。",
  "guide.help.role": "按自己岗位显示的功能操作。跟车员工若没有执行按钮，请交由该车驾驶员或主管处理。",
  "guide.understood": "我已阅读，开始使用",
  "guide.moreHelp": "每日操作步骤、审批规则及常见问题",
  "guide.next": "下一步：",
  "guide.next.preview": "这是明日预览，收货请进入今日路线。",
  "guide.next.done": "该趟次已完成。",
  "guide.next.approval": "等待主管批准出发。",
  "guide.next.waiting": "有稍后返回申请待批准，请先查看审批结果。",
  "guide.next.start": "开始执行这趟行程。",
  "guide.next.earlier": "检查前一趟行程及完成状态。",
  "guide.next.finishTrip": "顾客已处理完，可以结束这趟行程。",
  "guide.next.refresh": "刷新路线；若仍没有当前顾客，请联系办公室。",
  "guide.next.arrive": "前往当前顾客，到店后确认到达。",
  "guide.next.bill": "填写实际重量并开收货单。",
  "guide.next.proof": "上传现金付款证明。",
...
Object.assign(messages.en,{"flex.title": "Flexible collection areas", "flex.help": "Open until the owner closes it. Collect priority customers without route order; payment and document checks remain required.", "flex.open": "Open", "flex.closed": "Closed", "flex.close": "Close area", "flex.enable": "Open area", "flex.dispatch": "Dispatch a vehicle", "flex.zone": "Area / Zone", "flex.search": "Search customers", "flex.customer": "Customer / Branch", "flex.vehicle": "Running vehicle / Driver", "flex.noTrip": "Start an approved trip before assigning collections.", "flex.send": "Assign collection", "flex.sent": "Assigned. The customer is now in the driver’s route.", "apiError.flex_closed": "This area is closed. Refresh and use the usual approval process.", "apiError.flex_claimed": "Another vehicle has claimed this customer. Ask the supervisor to reassign it."})

Object.assign(messages.ms,{"flex.title": "Kawasan kutipan fleksibel", "flex.help": "Terbuka sehingga pemilik menutupnya. Kutip pelanggan utama tanpa turutan laluan; semakan bayaran dan dokumen masih diperlukan.", "flex.open": "Terbuka", "flex.closed": "Ditutup", "flex.close": "Tutup kawasan", "flex.enable": "Buka kawasan", "flex.dispatch": "Hantar kenderaan", "flex.zone": "Kawasan / Zon", "flex.search": "Cari pelanggan", "flex.customer": "Pelanggan / Cawangan", "flex.vehicle": "Kenderaan berjalan / Pemandu", "flex.noTrip": "Mulakan perjalanan yang diluluskan sebelum memberi tugasan.", "flex.send": "Tugaskan kutipan", "flex.sent": "Ditugaskan. Pelanggan kini dalam laluan pemandu.", "apiError.flex_closed": "Kawasan ditutup. Muat semula dan ikut proses kelulusan biasa.", "apiError.flex_claimed": "Kenderaan lain telah mengambil tugasan ini. Minta penyelia menugaskan semula."})

Object.assign(messages.zh,{"flex.title": "区域自由收货", "flex.help": "开放后持续生效，直到你手动关闭。可优先收取重要客户；付款和单据记录仍须完整。", "flex.open": "已开放", "flex.closed": "已关闭", "flex.close": "关闭", "flex.enable": "开放", "flex.dispatch": "主管调配车辆", "flex.zone": "分区", "flex.search": "搜索客户", "flex.customer": "客户／分店", "flex.vehicle": "已出车车辆／司机", "flex.noTrip": "请先让车辆按已批准排程出车，再调配收货客户。", "flex.send": "推送收货", "flex.sent": "已推送，客户已显示在对应司机的路线中。", "apiError.flex_closed": "该区域已关闭，请刷新并按正常流程申请。", "apiError.flex_claimed": "另一辆车已接下此客户，请主管调配，避免重复收货。"})

Object.assign(messages.en,{"apiError.system_review_different": "A different supervisor must approve this pending system change.", "apiError.system_review_stale": "Records or plans changed. Reload and review again.", "apiError.system_review_supervisor": "A supervisor with an employee identity must approve.", "apiError.system_review_choose": "Choose whether to update system records.", "apiError.system_review_draft": "The proposed changes are invalid. Reopen the editor.", "apiError.system_review_evidence": "Check the evidence and enter a review reason.", "apiError.system_review_forbidden": "You do not have permission to review this request.", "apiError.system_review_not_found": "Request not found."})

Object.assign(messages.ms,{"apiError.system_review_different": "Perubahan ini perlu diluluskan oleh penyelia lain.", "apiError.system_review_stale": "Rekod atau jadual berubah. Muat semula dan semak.", "apiError.system_review_supervisor": "Penyelia dengan identiti pekerja mesti meluluskan.", "apiError.system_review_choose": "Pilih sama ada mahu ubah rekod sistem.", "apiError.system_review_draft": "Cadangan tidak sah. Buka semula borang.", "apiError.system_review_evidence": "Semak bukti dan isi sebab semakan.", "apiError.system_review_forbidden": "Anda tidak dibenarkan menyemak permintaan ini.", "apiError.system_review_not_found": "Permintaan tidak ditemui."})

Object.assign(messages.zh,{"apiError.system_review_different": "这项系统修改需要另一位主管批准。", "apiError.system_review_stale": "资料或排程已变化，请重新载入并核对。", "apiError.system_review_supervisor": "需要有员工身份的主管批准。", "apiError.system_review_choose": "请选择是否同时修改系统资料。", "apiError.system_review_draft": "修改草稿无效，请重新打开编辑。", "apiError.system_review_evidence": "请核对证明并填写审核原因。", "apiError.system_review_forbidden": "你没有审核这项申请的权限。", "apiError.system_review_not_found": "找不到这项申请。"})
Object.assign(messages.en,{'sales.fixLine':'Item {row}: Check {field}.','sales.duplicateSlip':'Item {row}: The slip number duplicates item {other}.','apiError.sales_lines':'Check each item’s date, slip number, material, weight, price and amount. Slip numbers must be unique within this bill.'})
Object.assign(messages.ms,{'sales.fixLine':'Item {row}: Semak {field}.','sales.duplicateSlip':'Item {row}: Nombor tiket timbang sama dengan item {other}.','apiError.sales_lines':'Semak tarikh, nombor tiket timbang, bahan, berat, harga dan amaun setiap item. Nombor tiket timbang mestilah unik dalam bil ini.'})
Object.assign(messages.zh,{'sales.fixLine':'第{row}行：请检查{field}。','sales.duplicateSlip':'第{row}行：磅单号码与第{other}行重复。','apiError.sales_lines':'请检查各行的日期、磅单号码、货品、重量、单价及金额；同一张结算单的磅单号码不能重复。'})
Object.assign(messages.en,{'salePrices.title':'Selling Materials & Prices','salePrices.help':'Set the selling price for each factory and material. Bills keep their actual recorded prices.','salePrices.savePrice':'Save selling price','salePrices.saved':'Price saved.','salePrices.allFactories':'All factories','salePrices.items':'prices','salePrices.updated':'Updated','salePrices.action':'Action','salePrices.edit':'Edit','salePrices.remove':'Remove','salePrices.empty':'No selling prices set yet.','sales.chooseProduct':'Choose a material or type below','sales.choosePrice':'Choose a price or type below','apiError.sales_price_invalid':'Choose an active factory and enter a material and valid selling price.','apiError.sales_price_not_found':'Selling price not found.'})
Object.assign(messages.ms,{'salePrices.title':'Bahan & Harga Jualan','salePrices.help':'Tetapkan harga jualan bagi setiap kilang dan bahan. Harga sebenar pada bil kekal seperti direkodkan.','salePrices.savePrice':'Simpan harga jualan','salePrices.saved':'Harga disimpan.','salePrices.allFactories':'Semua kilang','salePrices.items':'harga','salePrices.updated':'Dikemas kini','salePrices.action':'Tindakan','salePrices.edit':'Ubah','salePrices.remove':'Buang','salePrices.empty':'Belum ada harga jualan.','sales.chooseProduct':'Pilih bahan atau taip di bawah','sales.choosePrice':'Pilih harga atau taip di bawah','apiError.sales_price_invalid':'Pilih kilang aktif, bahan dan harga jualan yang sah.','apiError.sales_price_not_found':'Harga jualan tidak dijumpai.'})
Object.assign(messages.zh,{'salePrices.title':'卖货货品与价格','salePrices.help':'按工厂设定货品卖价；卖货单仍以实际填写的价格保存。','salePrices.savePrice':'保存卖价','salePrices.saved':'卖价已保存。','salePrices.allFactories':'所有工厂','salePrices.items':'项卖价','salePrices.updated':'更新时间','salePrices.action':'操作','salePrices.edit':'修改','salePrices.remove':'删除','salePrices.empty':'尚未设定卖货价格。','sales.chooseProduct':'选择货品，或在下方手填','sales.choosePrice':'选择卖价，或在下方手填','apiError.sales_price_invalid':'请选择有效工厂，并填写货品与正确卖价。','apiError.sales_price_not_found':'找不到这项卖价。'})
