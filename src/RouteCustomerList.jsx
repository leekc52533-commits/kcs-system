...
function CustomerRow({stop,index,day,route,days,canEdit,editable,onReorder,busy}){
  const[open,setOpen]=useState(false),[adjust,setAdjust]=useState(false),[date,setDate]=useState(day.dispatch_date),[targetRoute,setTargetRoute]=useState(String(route.routeNumber)),[reason,setReason]=useState(''),[saving,setSaving]=useState(false),[error,setError]=useState('')
  ...
  return <li>
    <div className="route-customer-row"><button type="button" className="route-customer-name" aria-expanded={open} aria-controls={`route-customer-${day.id}-${stop.id}`} onClick={()=>setOpen(value=>!value)}><b>{index+1}. {formatBranchId(stop.branchId)} — {stop.branchName||'Unnamed branch'}</b><small>{stop.zoneGroup} · {stop.area}</small></button><span className={`collection-status ${stop.status==='completed'?'done':''}`}>{collectionStatus(stop)}</span>{canEdit&&<div className="route-customer-order"><button type="button" aria-label={`上移 ${stop.branchName}`} disabled={busy||saving||!editable||index===0} onClick={()=>reorder('up')}>↑</button><button type="button" aria-label={`下移 ${stop.branchName}`} disabled={busy||saving||!editable||index===route.stops.length-1} onClick={()=>reorder('down')}>↓</button></div>}</div>
    {error&&<p role="alert">{error}</p>}
    {open&&<div id={`route-customer-${day.id}-${stop.id}`} className="route-customer-detail">
      <p><b>Address：</b><span data-i18n-raw>{stop.address||'Not recorded'}</span></p>
