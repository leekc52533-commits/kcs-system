...
import {formatUnitPrice,validUnitPrice} from '../shared/measurePrecision.js'
...
export default function SalePricesPage({onBack}){
 ...
 <div className="sale-prices-table"><table><thead><tr><th>{t('sales.description')}</th><th data-numeric>{t('sales.unitPrice')}</th><th>{t('salePrices.updated')}</th><th>{t('salePrices.action')}</th></tr></thead><tbody>{data.items.map(item=><tr key={item.id}><td>{item.description}</td><td data-numeric>RM {formatUnitPrice(item.unitPrice)}/kg</td><td>{item.updatedAt}</td><td><button className="sale-price-action" type="button" aria-label={t('salePrices.edit')} title={t('salePrices.edit')} disabled={busy} onClick={()=>{setDraft({id:item.id,description:item.description,unitPrice:item.unitPrice});window.scrollTo({top:0,behavior:'smooth'})}}><PriceActionIcon kind="edit"/></button><button className="sale-price-action" type="button" aria-label={t('salePrices.remove')} title={t('salePrices.remove')} disabled={busy} onClick={()=>{if(window.confirm(t('salePrices.confirmRemove')))remove(item.id)}}><PriceActionIcon kind="remove"/></button></td></tr>)}</tbody></table>{!data.items.length&&<p>{t('salePrices.empty')}</p>}</div>
 </div>
}
