...
export default function SalePricesPage({onBack}){
 ...
 useEffect(()=>{load()},[])
 const save=async e=>{e.preventDefault();setBusy(true);setError('');setMessage('');try{await apiRequest('/api/sales/prices',{method:'POST',body:JSON.stringify({payload:draft})});setDraft({description:'',unitPrice:''});await load();setMessage(t('salePrices.saved'))}catch(e){setError(e.message)}finally{setBusy(false)}}
 ...
 <div className="sale-prices-table"><table><thead><tr><th>{t('sales.description')}</th><th>{t('sales.unitPrice')}</th><th>{t('salePrices.updated')}</th><th>{t('salePrices.action')}</th></tr></thead><tbody>{data.items.map(item=><tr key={item.id}><td>{item.description}</td><td>RM {item.unitPrice}/kg</td><td>{item.updatedAt}</td><td><button type="button" disabled={busy} onClick={()=>{setDraft({description:item.description,unitPrice:item.unitPrice});window.scrollTo({top:0,behavior:'smooth'})}}>{t('salePrices.edit')}</button><button type="button" disabled={busy} onClick={()=>{if(window.confirm(t('salePrices.confirmRemove')))remove(item.id)}}>{t('salePrices.remove')}</button></td></tr>)}</tbody></table>{!data.items.length&&<p>{t('salePrices.empty')}</p>}</div>
 </div>
}
