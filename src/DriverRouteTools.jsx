...
export default function DriverRouteTools({stop,trip,route,busy,run}){
 const{t,language}=useI18n(),[open,setOpen]=useState(false),[date,setDate]=useState(''),[reason,setReason]=useState(''),[reasonCode,setReasonCode]=useState(''),[evidence,setEvidence]=useState({}),[proofBusy,setProofBusy]=useState(false)
 ...
 const evidenceDraft={...evidence,details:reason},evidenceInvalid=evidenceProblem(reasonCode,evidenceDraft)
 ...
 return <div className="driver-route-tools">
  ...
  {untouched(stop)&&stop.dateRequest?.status!=='pending'&&<button data-preview-safe type="button" disabled={busy||proofBusy} onClick={()=>setOpen(!open)}>{t(open?'common.cancel':'routeTrial.requestDate')}</button>}
  {open&&<div><label>{t('routeTrial.targetDate')}<input type="date" value={date} min={route.date} onChange={e=>setDate(e.target.value)}/></label><label>{t('routeTrial.reason')}<select required disabled={busy||proofBusy} value={reasonCode} onChange={e=>{setReasonCode(e.target.value);setEvidence({})}}><option value="">{t('common.select')}</option>{dateRequestReasons.map(r=><option data-i18n-raw key={r.id} value={r.id}>{r[language]||r.en}</option>)}</select></label>{reasonCode&&<label>{['other','time','staff','customer','call','stopped'].includes(reasonCode)?notes.required:notes.optional}<textarea required={reasonCode==='other'} disabled={busy} maxLength={800} value={reason} onChange={e=>setReason(e.target.value)}/></label>}<DateRequestEvidence key={reasonCode} code={reasonCode} value={evidenceDraft} onChange={setEvidence} busy={busy||proofBusy} onBusyChange={setProofBusy}/><small>{t('routeTrial.pendingHelp')}</small><button type="button" disabled={busy||proofBusy||Boolean(evidenceInvalid)||!finalReason||date<=route.date} onClick={submit}>{t('routeTrial.submit')}</button></div>}
 </div>
}
