...
export function DispatchPlanningReview({data}){
 ...
 return <details className="schedule-review"><summary>收货排程需要确认（{items.length+gaps.length}）</summary>{gaps.map((g,i)=><p key={`gap-${i}`}>{g.date} · {formatBranchId(g.branchId)} {g.branchName}：{g.message}</p>)}{[...new Set(gaps.filter(g=>g.kind==='missing').map(g=>g.date))].map(date=><button type="button" key={date} onClick={()=>confirmMissing(date)}>确认补入 {date} 上述应收客户</button>)}{items.map(item=><p key={item.branchId}>{formatBranchId(item.branchId)} {item.branchName} · {item.issue||'按上传路线表同步固定收货星期'} {item.proposal&&<span>建议：{item.proposal.weekdays.join(', ')} {item.proposal.anchorDate||''}</span>}<button type="button" onClick={()=>open(item)}>核对排程</button></p>)}{error&&<p role="alert">{error}</p>}{editing&&<><ScheduleEditor routeOptions={data.days[0]?.routeBoards||[]} item={editing} t={t} routeNumber={route?Number(route):undefined} onClose={()=>setEditing(null)} onSaved={()=>{setEditing(null);refresh()}}/></>}</details>
}
