...
function VehicleMatrix({data,w,language,account,onSelect,toolbarTarget}){
 ...
 const order=normalizeExpenseOrder(saved,ids),columns=['metric',...order,'total'],format=(v,key)=>v==null?'—':Number(v).toLocaleString(undefined,{minimumFractionDigits:key.toLowerCase().includes('weight')?2:0,maximumFractionDigits:2})
 const value=(key,column)=>{if(column==='metric')return w(key==='vehicles'?'departureStatus':key);if(column==='total')return format(data.summary[key],key);const v=vehicles.find(v=>v.id===column);return key==='vehicles'?w(v.status):format(key==='weightKg'?v.quantity:v.detail?.[key],key)}
