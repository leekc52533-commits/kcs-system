...
function VehicleMatrix({data,w,language,account,onSelect}){
 ...
 return <div className="daily-report-matrix">
  <button type="button" className="record-icon-button" title={expenseColumnWords[language]?.title||expenseColumnWords.en.title} aria-label={expenseColumnWords[language]?.title||expenseColumnWords.en.title} onClick={()=>setArrange(true)}><RecordActionIcon kind="columns"/></button><small> {w('local')}</small>
