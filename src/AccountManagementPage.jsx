...
export default function AccountManagementPage({account}){
  ...
  return <div className="page account-management">
    ...
    <form className="account-create" onSubmit={create} noValidate>
      ...
      <label>{t('auth.employeeName')}<select value={createForm.employeeId} onChange={event=>changeCreate('employeeId',event.target.value)} {...fieldAccessibility('create-account-employee-error',createErrors.employeeId)}><option value="">{t('account.selectEmployee')}</option>{employees.map(employee=><option key={employee.id} value={employee.id}>{employee.employeeCode} — {employee.name}</option>)}</select><FieldError id="create-account-employee-error" message={createErrors.employeeId}/></label>
      <label>{t('auth.username')}<input value={createForm.username} onChange={event=>changeCreate('username',event.target.value)} {...fieldAccessibility('create-account-username-error',createErrors.username)}/><FieldError id="create-account-username-error" message={createErrors.username}/></label>
      ...
      <label>{t('account.temporaryPassword')}<PasswordInput value={createForm.password} onChange={event=>changeCreate('password',event.target.value)} {...fieldAccessibility('create-account-password-error',createErrors.password)}/><FieldError id="create-account-password-error" message={createErrors.password}/></label>
