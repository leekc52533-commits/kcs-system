...
const resolvePage=(raw,tab='')=>{const mapped=legacyPages[raw],page=mapped?.[0]||raw,pageTab=tab||mapped?.[1]||'';return page==='location-zone'&&pageTab==='area-zone'?{page:'operations',tab:'area-zone'}:page==='location-zone'?{page:'customers',tab:'settings'}:page==='operations'&&pageTab==='schedules'?{page:'customers',tab:'schedules'}:{page,tab:pageTab}}
...
function DesktopApp({account,onLogout,onChangePassword,onEnterMobile}){
  ...
  const title=t(navigation.find(x=>x[0]===page)?.[2]||pageTitleKeys[page]||'nav.dashboard'),currentUser={name:account.employeeName,preferenceId:account.id||account.username,role:account.role==='owner_admin'?'admin':account.role==='operations_admin'?'supervisor':account.role,systemRole:account.role,permissions:account.permissions||[]},canAccessBuyer=['owner_admin','operations_admin','supervisor','office'].includes(account.role),canActAsCollector=['owner_admin','operations_admin','supervisor'].includes(account.role),visibleNavigation=navigation.filter(item=>item[0]!=='location-zone'&&(!['buyers','sales','unloading-records'].includes(item[0])||canAccessBuyer)&&(item[0]!=='acting-collector'||canActAsCollector))
