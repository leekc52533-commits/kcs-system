...
function DesktopApp({account,onLogout,onChangePassword,onEnterMobile}){
  ...
  const title=t(navigation.find(x=>x[0]===page)?.[2]||pageTitleKeys[page]||'nav.dashboard'),currentUser={name:account.employeeName,preferenceId:account.id||account.username,role:account.role==='owner_admin'?'admin':account.role==='operations_admin'?'supervisor':account.role,systemRole:account.role,permissions:account.permissions||[]},canAccessBuyer=['owner_admin','operations_admin','supervisor','office'].includes(account.role),canActAsCollector=['owner_admin','operations_admin','supervisor'].includes(account.role),visibleNavigation=navigation.filter(item=>(!['buyers','sales'].includes(item[0])||canAccessBuyer)&&(item[0]!=='acting-collector'||canActAsCollector))
