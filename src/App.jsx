...
function DesktopApp({account,onLogout,onChangePassword,onEnterMobile}){
  ...
  const title=t(navigation.find(x=>x[0]===page)?.[2]||pageTitleKeys[page]||'nav.dashboard'),currentUser={name:account.employeeName,preferenceId:account.id||account.username,role:account.role==='owner_admin'?'admin':account.role==='operations_admin'?'supervisor':account.role,systemRole:account.role,permissions:account.permissions||[]},canAccessBuyer=['owner_admin','operations_admin','supervisor','office'].includes(account.role),canActAsCollector=['owner_admin','operations_admin','supervisor'].includes(account.role),visibleNavigation=navigation.filter(item=>(item[0]!=='buyers'||canAccessBuyer)&&(item[0]!=='acting-collector'||canActAsCollector))
...
function Dashboard({go}){
  ...
  useEffect(()=>{if(!summary)return;const card=document.querySelector('.stats article.gps-to-collect-stat');if(!card)return;const open=()=>go('location-zone','locations'),keyboard=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open()}};card.setAttribute('role','button');card.tabIndex=0;card.addEventListener('click',open);card.addEventListener('keydown',keyboard);return()=>{card.removeEventListener('click',open);card.removeEventListener('keydown',keyboard)}},[summary,go])
  const stats=summary?[[summary.branchCount,'dashboard.branch','blue'],[summary.scheduledBranchCount,'dashboard.scheduled','green'],[summary.gpsBranchCount,'dashboard.gps','violet'],[summary.routeReadyCount,'dashboard.ready','orange'],[summary.gpsToCollectCount,'dashboard.gpsToCollect','cyan gps-to-collect-stat']]:[]
