...
export default function GoogleMapPreview({latitude,longitude,onPositionAdjusted,onMapClick,initialCenter=null}){
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps(mapsKey).then(maps=>{
      ...
      if(!map.current){map.current=new maps.Map(container.current,{center:position,zoom:hasPosition?17:12,mapTypeId:'roadmap',gestureHandling:'greedy',draggable:true,scrollwheel:true,disableDoubleClickZoom:false,mapTypeControl:true,zoomControl:false,panControl:false,cameraControl:false,streetViewControl:false,fullscreenControl:false});if(hasPosition)createMarker();map.current.addListener('click',event=>mapClickHandler.current?.({latitude:event.latLng.lat().toFixed(7),longitude:event.latLng.lng().toFixed(7)}))}
