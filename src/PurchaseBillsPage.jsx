...
function BillRow({bill,expanded,toggle}){return <><tr className="bill-row" onClick={toggle}><td>{bill.serviceDateLabel}</td><td><button className="bill-number" onClick={event=>{event.stopPropagation();toggle()}}>{bill.billNumber}</button></td><td><span className={`payment ${bill.paymentMethod.toLowerCase()}`}>{bill.paymentMethod}</span></td><td>{bill.customerName}</td><td>{bill.branchName}</td><td>{bill.issuedBy}</td><td>{bill.crew||'—'}</td><td>{bill.car}</td><td>{bill.totalLabel}</td><td>{bill.proofId?<a href={`/api/purchase-bills/proofs/${bill.proofId}`} target="_blank" rel="noreferrer" onClick={event=>event.stopPropagation()}>View proof</a>:bill.paymentMethod==='Credit'?'—':'Missing'}</td><td>{bill.status==='voided'?<span className="voided">Voided</span>:<span className="issued">Issued</span>}</td></tr>{expanded&&<tr className="bill-detail"><td colSpan="11"><div><b>Bill Items</b><table><thead><tr><th>Item</th><th>Quantity</th><th>Unit</th><th>Price</th><th>Item Total</th></tr></thead><tbody>{bill.items.map((item,index)=><tr key={index}><td>{item.shortForm||item.item}</td><td>{item.quantity}</td><td>{item.unit||'—'}</td><td>{money(item.unitPriceCents)}</td><td>{money(item.itemTotalCents)}</td></tr>)}</tbody></table><small>Issued at {displayDateTime(bill.issuedAt)} · Electronic records cannot be deleted.</small></div></td></tr>}</>}
