...
export default function UnloadingArchivePage({onBack}){
 ...
 <div className="archive-table" ref={ref}><table><thead><tr>{unloadingColumns.map((k,i)=><FilterHeader key={k} label={unloadingLabels[lang][i]} open={open===k} onOpen={()=>setOpen(k)} onClose={()=>setOpen(null)} value={filters[k]??null} options={(data?.filterOptions[k]||['']).map(v=>({value:v,label:v===''?t('sales.blank'):display(k,v)}))} onChange={v=>setFilters({...filters,[k]:v})} sortDirection={sort.key===k?sort.direction:null} onSort={direction=>setSort(direction?{key:k,direction}:{})}/>)}</tr></thead><tbody>{data?.items.map(row=><tr key={row.id}>{unloadingColumns.map(k=><td key={k} data-i18n-raw>{k==='code'?<a href={row.photoUrl} title={t('unloading.photo')} target="_blank" rel="noreferrer">{row.code}</a>:display(k,row[k])}</td>)}</tr>)}</tbody></table>{data&&!data.items.length&&<p className="archive-empty">{t('unloading.empty')}</p>}</div><TableBottomScroll scrollRef={ref}/></div>
}
