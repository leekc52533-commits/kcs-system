...
export default function FlexibleCollectionPanel({routeNumber,date,isOpen=false,disabled=false}){
 ...
 const [expanded,setExpanded]=useState(false),[options,setOptions]=useState({branches:[],trips:[]}),[branch,setBranch]=useState(''),[trip,setTrip]=useState(''),[busy,setBusy]=useState(false),[loading,setLoading]=useState(false),[error,setError]=useState(''),[success,setSuccess]=useState(false),[version,setVersion]=useState(0)
