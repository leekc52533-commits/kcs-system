...
export default function CompactDataTable({items,rowKey,columns,renderDetails,renderActions,emptyLabel,labels,preferenceKey,expandedRowKey,onExpandedRowChange,stickyHorizontalScroll=false,hideExpandColumn=false}){
  ...
  const chooserRef=useRef(null),scrollRef=useRef(null)
  ...
  const toggleExpanded=key=>{if(onExpandedRowChange)onExpandedRowChange(String(expandedRowKey)===key?null:key);else setExpanded(current=>current.has(key)?new Set():new Set([key]))}
