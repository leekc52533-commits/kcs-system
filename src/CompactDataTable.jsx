...
const storageKey=preferenceKey=>`kcs.table-columns.${preferenceKey}`
...
const readColumns=(preferenceKey,columns)=>{
  ...
  try{const raw=window.localStorage.getItem(storageKey(preferenceKey));if(!raw)return defaults;const saved=JSON.parse(raw),allowed=new Set(defaults),required=columns.filter(column=>column.required).map(column=>column.key),visible=saved.filter(key=>allowed.has(key));return [...new Set([...required,...visible])]}catch{return defaults}
}
...
export default function CompactDataTable({items,rowKey,columns,renderDetails,renderActions,emptyLabel,labels,preferenceKey}){
  ...
  const resetColumns=()=>setVisibleKeys(columns.map(column=>column.key))
