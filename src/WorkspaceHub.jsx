import {customerWorkspaceWords} from './customerWorkspaceWords.js'
import {useUi} from './i18n.jsx'
import {useEffect,useState} from 'react'
import WeeklyDispatchPage from './WeeklyDispatchPage.jsx'
import {DataQualityPage} from './DataPages.jsx'
...
function Hub({tabs,validTabs=tabs,initialTab,onTabChange,children,tools,backControl}){
  const ui=useUi()
...
export function LocationGpsZoneHub({currentUser,initialTab,onTabChange}){const{t}=useI18n(),[active,setActive]=useState(initialTab==='locations'?'data-quality':initialTab||'data-quality'),tabs=[['data-quality',t('nav.data')],['address-analysis',t('hub.addressAnalysis')],['recommendations',t('hub.gpsRecommendations')]];const allowed=['admin','supervisor'].includes(currentUser.role);return <Hub tabs={tabs} initialTab={active} validTabs={[...tabs,['legacy-gps',t('nav.gpsMigration')]]} onTabChange={value=>{setActive(value);onTabChange?.(value)}} tools={allowed&&<div className="workspace-tools"><button type="button" className="workspace-tool-icon" aria-label={t('nav.gpsMigration')} title={t('nav.gpsMigration')} onClick={()=>setActive('legacy-gps')}>⇧</button></div>}>{(tab,choose)=>tab==='legacy-gps'?<GpsMigrationPage/>:tab==='data-quality'?<DataQualityPage/>:tab==='address-analysis'?<AddressAnalysisPage onOpenRecommendations={()=>choose('recommendations')}/>:<GpsZoneRecommendationPage currentUser={currentUser}/>}</Hub>}
