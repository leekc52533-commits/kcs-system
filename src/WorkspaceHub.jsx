...
export function LocationGpsZoneHub({currentUser,initialTab,onTabChange}){const{t}=useI18n(),[active,setActive]=useState(initialTab||'locations'),tabs=[['locations',t('hub.locationsGps')],['data-quality',t('nav.data')],['address-analysis',t('hub.addressAnalysis')],['recommendations',t('hub.gpsRecommendations')]];const allowed=['admin','supervisor'].includes(currentUser.role);return <Hub tabs={tabs} initialTab={active} validTabs={[...tabs,['legacy-gps',t('nav.gpsMigration')]]} onTabChange={value=>{setActive(value);onTabChange?.(value)}} tools={allowed&&<div className="workspace-tools"><button type="button" className="workspace-tool-icon" aria-label={t('nav.gpsMigration')} title={t('nav.gpsMigration')} onClick={()=>setActive('legacy-gps')}>⇧</button></div>}>{(tab,choose)=>tab==='legacy-gps'?<GpsMigrationPage/>:tab==='locations'?<MasterDataPage key="locations" embedded currentUser={currentUser} initialTab="gps" allowedTabs={['gps','locations']}/>:tab==='data-quality'?<DataQualityPage/>:tab==='address-analysis'?<AddressAnalysisPage onOpenRecommendations={()=>choose('recommendations')}/>:<GpsZoneRecommendationPage currentUser={currentUser}/>}</Hub>}
