...
export function DispatchScheduleHub({currentUser,onOpenSpecial,initialTab,onTabChange}){const{t}=useI18n(),tabs=[['weekly',t('hub.weekly')],['schedules',t('hub.schedules')],['area-zone',t('hub.areaZone')]];return <Hub title={t('nav.dispatchSchedule')} tabs={tabs} initialTab={initialTab} onTabChange={onTabChange}>{tab=>tab==='weekly'?<WeeklyDispatchPage onOpenSpecial={onOpenSpecial} currentUser={currentUser}/>:tab==='schedules'?<SchedulesPage/>:<ResourcePage embedded fixedTab initialTab="zones" currentUser={currentUser}/>}</Hub>}
