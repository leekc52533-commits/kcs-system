...
function Hub({tabs,validTabs=tabs,initialTab,onTabChange,children,tools,backControl}){
  ...
  return <div className="page workspace-hub"><nav className={`workspace-tabs${backControl?' workspace-tabs--sticky':''}`} aria-label="Workspace sections">{backControl}{tabs.map(([id,label])=><button key={id} className={tab===id?'active':''} onClick={()=>choose(id)}>{label}</button>)}{tools}</nav>{children(tab,choose)}</div>
}
