...
export function NoGoodsButton({stop,onSaved,disabled=false}){
 ...
 const request=stop.arrangementRequests?.find(r=>r.kind==='no_goods'),pending=request?.status==='pending'&&stop.noGoodsApprovalRequired
 ...
 return <div className="no-goods-action">{request&&<p>{t('arrange.no_goods')} · {t('arrange.'+request.status)} <span data-i18n-raw>{request.reviewReason}</span></p>}<button data-preview-safe type="button" disabled={disabled||busy||processing||pending} onClick={()=>setOpen(!open)}>{t(open?'common.cancel':stop.noGoodsApprovalRequired?'arrange.requestNg':'ng.title')}</button>{open&&<form onSubmit={submit}><p>{t(stop.noGoodsApprovalRequired?'arrange.ngHelp':stop.verifiedArrival?'arrange.onsiteHelp':'ng.help')}</p><label>{t('ng.method')}<select value={method} disabled={busy} onChange={e=>setMethod(e.target.value)}>{['phone','whatsapp','sms','onsite'].map(x=><option key={x} value={x}>{t('ng.'+x)}</option>)}</select></label><label>{t('ng.reason')}<textarea required maxLength={1000} value={reason} disabled={busy} onChange={e=>setReason(e.target.value)}/></label><b>{t('ng.proof')}</b><ProofPhotoPicker value={photo} onChange={setPhoto} onBusyChange={setProcessing} disabled={busy}/>{error&&<p role="alert">{error}</p>}<button type="submit" disabled={busy||processing||!photo||!reason.trim()}>{t(busy?'common.processing':stop.noGoodsApprovalRequired?'arrange.requestNg':'ng.submit')}</button></form>}</div>
}
