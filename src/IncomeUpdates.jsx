...
export default function IncomeUpdates({data,error}){
 ...
 return <section className="income-updates"><h2>{w.incomeUpdates}</h2><p>{w.midnight}</p>{(error||failure)&&<p role="alert">{w[(error||failure).code]||w.failed}</p>}{data.items.length>0&&<details open={data.unread>0}><summary>{w.incomeUpdates} ({data.unread})</summary>{data.items.map(n=><article key={n.id}><b>{n.updateDate} · {n.readAt?w.read:w.unread}</b><p>{w.period}: {n.summary.period.start} ～ {n.summary.period.end}</p><p>{w.amount}: <strong>RM {n.summary.amount.toFixed(2)}</strong> · {w.pendingKg}: {formatWeight(n.summary.pendingKg)}</p><p>{w.generated}: {new Date(n.createdAt).toLocaleString(language==='zh'?'zh-MY':language==='ms'?'ms-MY':'en-GB',{timeZone:'Asia/Kuching'})}</p>{n.summary.changed&&<p>{w.changed}</p>}{!n.readAt&&<button disabled={busy!==null||isEmployeePreview()} onClick={()=>read(n.id)}>{w.markRead}</button>}</article>)}</details>}</section>
}
