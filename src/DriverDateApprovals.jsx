...
export function DateRequestReview({item,onSaved,onPlanner,submitUrl}){
 ...
 const[date,setDate]=useState(item.targetDate),[route,setRoute]=useState(''),[scope,setScope]=useState('once'),[reason,setReason]=useState(''),[sunday,setSunday]=useState(false),[checked,setChecked]=useState(false)
 ...
 return <div className="date-request-review">
  ...
  <div className="date-review-actions"><button type="button" className="primary" disabled={busy||(!submitUrl&&!checked)||!date||!chosen?.available||!reason.trim()} onClick={()=>decide('approve')}>{t('dateReview.approve')}</button>{!submitUrl&&<button type="button" disabled={busy||!reason.trim()} onClick={()=>decide('reject')}>{ew.reject}</button>}</div>
 </div>
}
