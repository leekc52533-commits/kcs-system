...
[[ "$SCHEMA" = 69 ]]
STAGE="$(mktemp -d /tmp/kcs-closed-stop-completion-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-closed-stop-completion-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-closed-stop-completion-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 69
