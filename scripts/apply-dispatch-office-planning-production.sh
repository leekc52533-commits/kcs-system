...
test "$SCHEMA" = 58
STAGE="$(mktemp -d "$APP/.dispatch-office-planning-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-dispatch-office-planning-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-dispatch-office-planning-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 58
...
echo OFFICE_DISPATCH_EDIT=ready
echo FUTURE_DATE_AUTO_CREATE=ready
echo ROUTE_WITHOUT_VEHICLE=ready
echo DRIVER_UNAPPROVED_PREVIEW=ready
echo DRIVER_EXECUTION_APPROVAL=required
echo DATE_REVIEW_REUSE_EXISTING=ready
echo DATE_REVIEW_INLINE=ready
echo DATE_REVIEW_TARGET_ROUTE=ready
echo DATE_REVIEW_ONCE_PERMANENT=ready
echo CUSTOMER_SCHEDULE_SYNC=ready
echo FUTURE_EXECUTED_RECORDS=preserved
