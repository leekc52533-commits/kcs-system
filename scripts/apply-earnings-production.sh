...
[[ "$SCHEMA" = 70 || "$SCHEMA" = 71 || "$SCHEMA" = 72 ]]
STAGE="$(mktemp -d /tmp/kcs-earnings-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-earnings-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-earnings-${STAMP}.tar.gz"
RESTARTED=1
systemctl stop kcs-api
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 72
