...
[[ "$SCHEMA" = 63 || "$SCHEMA" = 64 ]]
STAGE="$(mktemp -d /tmp/kcs-driver-approval-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-driver-approval-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-driver-approval-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 64
test "$(sqlite3 "$DB" 'PRAGMA integrity_check;')" = ok
test -z "$(sqlite3 "$DB" 'PRAGMA foreign_key_check;')"
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('driver_arrangement_requests');")" = 1
