...
[[ "$SCHEMA" = 70 || "$SCHEMA" = 71 || "$SCHEMA" = 72 || "$SCHEMA" = 73 || "$SCHEMA" = 74 || "$SCHEMA" = 75 || "$SCHEMA" = 76 || "$SCHEMA" = 77 ]]
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-work-close-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-work-close-${STAMP}.tar.gz"
RESTARTED=1
systemctl stop kcs-api
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 77
...
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('attendance_settings','attendance_settings_history','attendance_records');")" = 3
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('route_collection_access','route_collection_access_events');")" = 2
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='trip_work_close_requests';")" = 1
echo WORK_CLOSE=ready
