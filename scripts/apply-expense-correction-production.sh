...
[[ "$SCHEMA" = 66 || "$SCHEMA" = 67 ]]
STAGE="$(mktemp -d /tmp/kcs-expense-correction-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-expense-correction-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-expense-correction-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 67
test "$(sqlite3 "$DB" 'PRAGMA integrity_check;')" = ok
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='expense_amount_corrections';")" = 1
...
echo OWNER_EXPENSE_AMOUNT_CORRECTION=ready
