...
test "$SCHEMA" = 58
STAGE="$(mktemp -d "$APP/.no-goods-notice-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-no-goods-notice-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-no-goods-notice-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 59
...
echo NO_GOODS_OUTSIDE=ready
echo NO_GOODS_PROOF_REQUIRED=ready
echo NO_GOODS_COLLAPSED_HISTORY=ready
echo NO_GOODS_OFFICE_RESTORE=ready
echo NO_GOODS_RECURRENCE=preserved
