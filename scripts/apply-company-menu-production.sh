...
test "$SCHEMA" = 60 -o "$SCHEMA" = 61
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM auth_accounts WHERE lower(username)='kcadmin' AND is_active=1;")" = 1
...
STAGE="$(mktemp -d "$APP/.company-menu-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-company-menu-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-company-menu-${STAMP}.tar.gz"
...
echo COMPANY_MENU_SHARED=ready
echo COMPANY_MENU_OWNER_ONLY=ready
echo COMPANY_MENU_DOCUMENTS=ready
