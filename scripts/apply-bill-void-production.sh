...
test "$SCHEMA" = 56
STAGE="$(mktemp -d "$APP/.bill-void-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-bill-void-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-bill-void-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 57
...
echo BILL_VOID_MOBILE=ready
echo BILL_VOID_DESKTOP=ready
echo BILL_VOID_APPROVALS=ready
echo BILL_VOID_CASH_REVERSAL=ready
echo BILL_VOID_REISSUE=ready
echo BILL_VOID_HISTORY=preserved
