...
[[ "$SCHEMA" = 66 ]]
STAGE="$(mktemp -d /tmp/kcs-sidebar-colors-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-sidebar-colors-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-sidebar-colors-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 66
...
echo PALE_BLUE_SIDEBAR=ready
echo PURPLE_FILE_ICONS=ready
