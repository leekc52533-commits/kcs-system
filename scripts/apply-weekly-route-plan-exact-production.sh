...
node -e 'const r=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));const monday=r.days[0],sunday=r.days[6];if(!(r.startDate==="2026-09-07"&&r.protectedDays.length===0&&monday&&monday.vehicles.QM630S===19&&sunday&&sunday.vehicles.QAA4293N===13&&sunday.vehicles.QTY5028===0))process.exit(1)' "$REFRESH"
...
LIVE_SUNDAY_QAA="$(sqlite3 "$DB" "SELECT GROUP_CONCAT(branch_code,'|') FROM (SELECT UPPER(CASE WHEN b.jodoo_branch_id GLOB '[0-9]*' THEN 'B'||b.jodoo_branch_id ELSE b.jodoo_branch_id END) branch_code FROM dispatch_days dd JOIN dispatch_trips dt ON dt.dispatch_day_id=dd.id JOIN dispatches d ON d.id=dt.dispatch_id JOIN vehicles v ON v.id=d.vehicle_id JOIN dispatch_stops ds ON ds.dispatch_trip_id=dt.id JOIN branches b ON b.id=ds.branch_id WHERE dd.dispatch_date='2026-09-13' AND UPPER(REPLACE(v.registration_number,' ',''))='QAA4293N' AND ds.status<>'cancelled' ORDER BY dt.trip_number,ds.stop_sequence);")"
test "$LIVE_SUNDAY_QAA" = "B10151|B10142|B10164|B10104|B10204|B10134|B10135|B10167|B10145|B10177|B10144|B10275|B10310"
...
node -e 'const r=JSON.parse(require("fs").readFileSync(process.argv[1],"utf8"));const c=r.templateCountsSunToSat;if(!(r.sourceName==="KCS_7Day_5Vehicle_Route_Plan(2).xlsx [vehicle sheets Arrange + confirmed Sunday alternation]"&&r.allVehiclesMatch===true&&r.sundays.length===1&&r.sundays[0].date==="2026-09-13"&&r.sundays[0].plate==="QAA4293N"&&r.sundays[0].branchCount===13&&c.QAA4293N.join(",")==="0,15,19,17,20,18,13"&&c.QAB1225B.join(",")==="0,27,27,24,26,29,25"&&c.QM3028M.join(",")==="4,23,21,22,21,22,24"&&c.QM630S.join(",")==="8,19,22,29,16,19,20"&&c.QTY5028.join(",")==="13,24,23,19,19,19,18"))process.exit(1)' "$VERIFY"
...
echo "SUNDAY_2026_09_13_VEHICLE=QAA4293N"
echo "SUNDAY_2026_09_13_ROUTE=$LIVE_SUNDAY_QAA"
echo "QAA4293N_SUN_TO_SAT=0,15,19,17,20,18,13"
echo "QAB1225B_SUN_TO_SAT=0,27,27,24,26,29,25"
echo "QM3028M_SUN_TO_SAT=4,23,21,22,21,22,24"
echo "QTY5028_SUN_TO_SAT=13,24,23,19,19,19,18"
echo "SUNDAY_ROUTE_ALTERNATION=true"
