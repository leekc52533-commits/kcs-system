...
[[ "$SCHEMA" = 70 || "$SCHEMA" = 71 || "$SCHEMA" = 72 || "$SCHEMA" = 73 ]]
STAGE="$(mktemp -d /tmp/kcs-income-notices-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-income-notices-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-income-notices-${STAMP}.tar.gz"
RESTARTED=1
systemctl stop kcs-api
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 73
...
echo INCOME_NOTIFICATIONS=ready
