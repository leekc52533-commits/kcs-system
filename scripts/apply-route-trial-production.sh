...
[[ "$SCHEMA" = 54 || "$SCHEMA" = 55 ]]
STAGE="$(mktemp -d "$APP/.trial-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-route-trial-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-route-trial-${STAMP}.tar.gz"
sqlite3 "$DB" ".backup \"$BACKUP\""
test "$(sqlite3 "$BACKUP" 'PRAGMA integrity_check;')" = ok
tar -czf "$FRONTEND_BACKUP" -C "$APP" dist
# Startup applies the additive request-table migration before exposing the new UI.
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 55
...
echo ROUTE_ORDER_TRIAL=2026-09-10_to_2026-09-23_Asia_Kuching
echo NORMAL_ORDER_RESTORED=2026-09-24
echo EMPLOYEE_DATE_REQUESTS=ready
echo SUPERVISOR_DATE_APPROVALS=ready
