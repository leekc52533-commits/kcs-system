...
try{
  ...
  const days=result.days.map(day=>({date:day.dispatch_date,status:day.status,protected:result.protectedDays.some(item=>item.date===day.dispatch_date),stops:day.previewSummary.stopCount,unassigned:day.previewSummary.unassignedCount,vehicles:Object.fromEntries(day.vehicleBoards.map(vehicle=>[String(vehicle.registrationNumber||vehicle.vehicle).toUpperCase().replace(/[^A-Z0-9]/g,''),vehicle.customerCount]))}))
