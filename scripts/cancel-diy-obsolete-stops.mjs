...
try{
 ...
 const preview=cancelDiyObsoleteStops(db)
 if(apply&&preview.items.some(i=>i.status!=='already_cancelled')){
 const directory=path.join(path.dirname(databasePath),'backups');mkdirSync(directory,{recursive:true,mode:0o750})
 const destination=path.join(directory,'kcs-pre-diy-stop-cleanup-'+new Date().toISOString().replace(/[:.]/g,'-')+'.sqlite');await backup(db,destination);console.log('BACKUP='+destination)
 }
 console.log(JSON.stringify(apply?cancelDiyObsoleteStops(db,{apply:true}):preview,null,2))
