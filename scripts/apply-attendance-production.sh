...
[[ "$SCHEMA" = 70 || "$SCHEMA" = 71 || "$SCHEMA" = 72 || "$SCHEMA" = 73 || "$SCHEMA" = 74 || "$SCHEMA" = 75 ]]
STAGE="$(mktemp -d /tmp/kcs-attendance-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-attendance-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-attendance-${STAMP}.tar.gz"
RESTARTED=1
systemctl stop kcs-api
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 75
