...
[[ "$SCHEMA" = 62 || "$SCHEMA" = 63 ]]
STAGE="$(mktemp -d /tmp/kcs-pickup-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-pickup-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-pickup-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 63
test "$(sqlite3 "$DB" 'PRAGMA integrity_check;')" = ok
test -z "$(sqlite3 "$DB" 'PRAGMA foreign_key_check;')"
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('existing_customer_pickups','customer_transfer_requests');")" = 2
