...
[[ "$SCHEMA" = 70 || "$SCHEMA" = 71 || "$SCHEMA" = 72 || "$SCHEMA" = 73 || "$SCHEMA" = 74 || "$SCHEMA" = 75 || "$SCHEMA" = 76 ]]
STAGE="$(mktemp -d /tmp/kcs-route-collection-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-route-collection-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-route-collection-${STAMP}.tar.gz"
RESTARTED=1
systemctl stop kcs-api
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 76
