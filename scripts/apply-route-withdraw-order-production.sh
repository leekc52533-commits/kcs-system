...
STAGE="$(mktemp -d "$APP/.route-withdraw-order-build-XXXXXX")"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-route-withdraw-order-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-route-withdraw-order-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 61
...
echo CASH_FLOAT_UNIFIED_TOOLBAR=ready
echo CASH_FLOAT_DAILY_ARITHMETIC=ready
echo CASH_FLOAT_EXPENSE_ITEMS=ready
echo CASH_DAILY_HISTORY=ready
echo DAILY_DATE_DROPDOWN=ready
echo DAILY_SELECTION_AUTO_CLOSE=ready
echo DAILY_RANGE_EXCEL=ready
echo ROUTE_INDIVIDUAL_WITHDRAW=ready
echo ROUTE_DATE_ORDER_PRESERVED=ready
echo NO_GOODS_EMPLOYEE_PROOF_ACCESS=ready
echo NO_GOODS_TASK_SCOPE_CHECK=ready
echo CASH_LEDGER_CONTINUOUS_HEADER=ready
echo SHARED_HEADER_FORMAT=ready
echo CASH_LEDGER_COLUMN_FILTERS=ready
echo CASH_LEDGER_RANGE_DOWNLOAD=ready
echo CASH_LEDGER_EXPORT_FILTER_SORT=ready
echo CASH_MIDNIGHT_ROLLOVER=ready
echo CASH_BACKDATED_OFFICE_ENTRY=ready
echo CASH_DAILY_UNLOCKED=true
echo CASH_ONLY_SUMMARY=ready
echo SUMMARY_EXPENSE_ITEMS=ready
echo CASH_FLOAT_MORE_MENU=ready
echo EMPLOYEE_SPENDING_SUMMARY=ready
echo ADMIN_EXPENSE_EXCLUDED=true
