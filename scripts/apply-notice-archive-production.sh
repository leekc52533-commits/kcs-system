...
[[ "$SCHEMA" = 64 || "$SCHEMA" = 65 ]]
STAGE="$(mktemp -d /tmp/kcs-notice-archive-build-XXXXXX)"
...
BACKUP="/var/lib/kcs/data/backups/kcs-pre-notice-archive-${STAMP}.sqlite"
FRONTEND_BACKUP="/var/lib/kcs/data/backups/kcs-ui-pre-notice-archive-${STAMP}.tar.gz"
...
test -n "$INTERNAL"
test "$(sqlite3 "$DB" 'SELECT MAX(version) FROM schema_meta;')" = 65
test "$(sqlite3 "$DB" 'PRAGMA integrity_check;')" = ok
test -z "$(sqlite3 "$DB" 'PRAGMA foreign_key_check;')"
test "$(sqlite3 "$DB" "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('employee_notices','employee_notice_receipts');")" = 2
