...
桌面右上角员工姓名只打开 Profile 菜单。菜单显示员工姓名、Username、Employee Code、System Role、Preferred Language、Change Password、按权限显示的 Account Management，以及 Logout。`kcadmin` 始终解析为 `owner_admin`；System Role 不会覆盖 Employee Job Role。
