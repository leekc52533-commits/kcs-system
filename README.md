...
- 当前上线的 Special Requests、Customer/Branch、Schedules、Data Quality、GPS 建议、Employee、Vehicle、Location/Zone、旧 GPS 迁移与 Jodoo 导入页面均纳入路由表面三语回归矩阵。关键主档及 Excel/CSV 区块另外使用实际 React 组件渲染测试；生产源码 CJK 门禁会报告准确文件、行号和残留文字。Zone 改名使用系统内 Modal；正式地点名称仍显示数据库原值，并拒绝新输入的 CJK 地点名称。
